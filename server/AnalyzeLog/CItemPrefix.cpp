...
//////////////////////////////////////////////////
	...
	eIP_ADD_AGILITY_BY_LEVEL,"민첩성 [+1]/레벨 [0]",
	eIP_ADD_CONSTITUTION_BY_LEVEL,"건강 [+1]/레벨 [0]",
	eIP_ADD_WISDOM_BY_LEVEL,"지혜 [+1]/레벨 [0]",
	eIP_ADD_INTELIGENCE_BY_LEVEL,"지식 [+1]/레벨 [0]",
	eIP_ADD_CHARISMA_BY_LEVEL,"카리스마 [+1]/레벨 [0]",
	eIP_ADD_LUCK_BY_LEVEL,"운 [+1]/레벨 [0]",
...
//////////////////////////////////////////////////////////////////////////////
	...
	eIP_WATER_DAMAGE,"물 대미지 [0]~[1], 콜드 [2]Frame",
	eIP_WIND_DAMAGE,"바람 대미지 [0]~[1], 스턴 [2]Frame",
	eIP_TERRA_DAMAGE,"[2]초간 초당 [0]~[1]의 중독 대미지",
	eIP_LIGHT_DAMAGE,"빛 대미지 [0]~[1], [2]초동안 명중,회피 저하 ",
	eIP_DARK_DAMAGE,"어둠 대미지 [0]~[1], 저주 [2]초",
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	eIP_AUTO_RELOAD,"아이템 자동 리로드",
	eIP_DISABLE,	"아이템을 사용할 수 없다.",
	eIP_CAN_NOT_STRIP,	"착용 후 탈의가 불가능하다.",
	eIP_CAN_NOT_ADD_PREFIX,	"추가 인챈트가 불가능하다.",

	eIP_UPGRADE_FIRE_DAMAGE_ATTACK, "불 공격 접두사 1단계 상승",
	eIP_UPGRADE_WATER_DAMAGE_ATTACK, "물 공격 접두사 1단계 상승",
	eIP_UPGRADE_WIND_DAMAGE_ATTACK, "바람 공격 접두사 1단계 상승",
	eIP_UPGRADE_EARTH_DAMAGE_ATTACK, "대지 공격 접두사 1단계 상승",
	eIP_UPGRADE_LIGHT_DAMAGE_ATTACK, "빛 공격 접두사 1단계 상승",
	eIP_UPGRADE_DARK_DAMAGE_ATTACK, "어둠 공격 접두사 1단계 상승",
...
//	아이템 부가 효과 설명
char*
cITEM_PREFIX_MANAGER::getComment(CItemPrefixInfo *_lpPrefixInfo)
{
	...
	if	(iIndex	==	0xffff)
	{
		ERRMSG("코멘트 얻기 에러!!","새로운 접두사가 추가 되었는데 업데이트가 안된거 같으니\n\n엘엔케이에서 제일 멋진 남자에게 문의해 주세요.");
	...
	{
		m_string.Add("%s 스킬 레벨 +%d",l_strHeroJobName[_lpPrefixInfo->m_aValue[1]],_lpPrefixInfo->m_aValue[0]);
	...
	while(1)
	{
		...
		if (!lpStream && !bFirst)	break;
		bFirst		=	FALSE;

		if (lpStream)	m_string.Add(lpStream);
		...
		if (bIsAcquireSymbol && iIndex>0)
				m_string.Add(_ms("+%d",iIndex));
		else	m_string.Add(_ms("%d",iIndex));
...
{
	int		iIndex	=	getIndex(_lpUnique->m_wEffect);
	...
	{
		m_string.Add("%s 스킬 레벨 +%d",l_strHeroJobName[_lpUnique->m_aValue[1]],_lpUnique->m_aValue[0]);
	...
	while(1)
	{
		...
		iIndex		=	m_decoder.getNumber();
		iIndex		=	_lpUnique->m_aValue[iIndex];
