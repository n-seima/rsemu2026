...
// Overrides
	...
	protected:
	virtual void OnCustomDrawNotify(NMHDR* lParam, LRESULT* pResult );	//NM_CUSTOMDRAW handler
...
protected:
	...
	LRESULT	OnPostLbuttonMsg(UINT nFlags, LPARAM point);			//Called from OnLbuttonDown using PostMessage
...
//	void	Init(int nBitmapW=0, BOOL bShowBmp4SubMenu=FALSE, HBITMAP hBitmap=0, BOOL bStretchBmp=TRUE);
	CBmpMenu(int nBitmapW=0, BOOL bShowBmp4SubMenu=FALSE, HBITMAP hBitmap=0, BOOL bStretchBmp=TRUE);
