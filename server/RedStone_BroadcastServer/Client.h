...
public:
	...
	cUserCustomDataDefine	m_clPersonalData;
	int					m_iDBIndex;
	DWORD				m_dwSaveGap;
...
public:
	...
	void				setPartySerial(int _iSerial=-1);
	void				setGuildSerial(int _iSerial , int _GuildDisplayIndex = 0xffff);
	void				setFieldSerial(int _iSerial);
	void				setAvatarIndex(int _iIndex);
	//	일반 Get & Set Method
...
public:
	...
	CClient				User[dBC_MAX_USER_COUNT];
...
#define	mCM	CClientManager::GetInstance()
extern	CClientManager & UserManager;
extern	BOOL			g_bIsTestServer;
