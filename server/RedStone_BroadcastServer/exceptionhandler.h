...
const	s4	c_stEH_CodeInfoFileNameLength	=	512;
...
//	단일 쓰레드에서만 사용하도록 한다.
	BOOL		__cdecl		addStaticLog(char *_lpstrFormat, ...);	//	로그 추가(계속 누적됨)
	void					outputStaticLog(char *_lpstrFileName);	//	로그 출력
	BOOL		__cdecl		addLog(char *_lpstrFormat, ...);		//	로그 추가(아래 임시 로그로 사용)
...
//	단일 쓰레드에서만 사용하도록 한다.
	...
	void					remainCodeInfo(char *_lpstrFileName,int _iLineIndex);
	void					clearRemainCodeInfo();
	//	현재 파일과 라인을 남긴다. rpt 파일조차 남기지 않고 죽을때 메모리에서 바로 확인해 보기 위해 필요

private:
	CRITICAL_SECTION		m_cs;
	int						m_iCurrentLine;
	char					m_strCurrentFile[c_stEH_CodeInfoFileNameLength];
...
#define	mRCI()			g_eh.remainCodeInfo(__FILE__,__LINE__);	//	현재 코드 파일명과 라인 위치를 남긴다.
#define	mCRCI()			g_eh.clearRemainCodeInfo();	//	현재 코드 파일명과 라인 위치를 남긴다.
