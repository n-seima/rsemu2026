...
{
	mciSendCommand(LOWORD(1), MCI_CLOSE, 0, (DWORD)NULL);

	KillTimer(IDT_APPEARING);
	KillTimer(IDT_WAITING);
	SetTimer(IDT_DISAPPEARING,m_dwDelayBetweenHideEvents,NULL);
	if(!(point.x > 10  && point.x < 30  &&  point.y > 10  &&  point.y < 30)){	// x버튼을 누룰때는 그냥닫는다
...
void CTaskbarNotifier::OnPaint()
{
	...
	dc.DrawText(szBuffer,-1,m_rcText, DT_VCENTER | DT_WORDBREAK | DT_END_ELLIPSIS);
