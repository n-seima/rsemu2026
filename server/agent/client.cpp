...
CClient::AddRecvPacket(char * _cData,int _iSize)	//	packet을 더한다.
{
	...
	//	쪼개지지 않았다고 가정한다.
	if	(GetTickCount() - m_dwLastRecvUpdateTime >= 1000)
	{
		m_dwRecvSizePerSec = m_dwRecvTmpSizePerSec;
		m_dwRecvTmpSizePerSec = _iSize;
		m_dwLastRecvUpdateTime = GetTickCount();
	}else
	{
		m_dwRecvTmpSizePerSec += _iSize;
	}
...
int CClient::CompleteSendPacket(int _iSize)
{
	...
	//	쪼개지지 않았다고 가정한다.
	if	(GetTickCount() - m_dwLastSendUpdateTime >= 1000)
	{
		m_dwSendSizePerSec = m_dwSendTmpSizePerSec;
		m_dwSendTmpSizePerSec = _iSize;
		m_dwLastSendUpdateTime = GetTickCount();
	}
	else
	{
		m_dwSendTmpSizePerSec += _iSize;
...
{	for(int _next = m_wNext;_next<dMAX_CLIENT_SERVER_COUNT;_next++)
...
{	m_wNext = 0;		m_wCount = 0;
	for(int i=0;i<dMAX_CLIENT_SERVER_COUNT;i++)
	{
		User[i].Kill();
	}
}
...
{	CCritical	CS(&csDeque);
	...
	if(_wSerial>=dMAX_CLIENT_SERVER_COUNT)		return;
...
{	CCritical	CS(&csDeque);
	...
	WORD	wResult = 0xffff;
	if(m_wCount >= dMAX_CLIENT_SERVER_COUNT)	{ NextNext(); return wResult; }
...
{	
	if(_wSerial >=dMAX_CLIENT_SERVER_COUNT)
		return NULL;
