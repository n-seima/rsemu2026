...
static void store(void)
{
	...
	int	iRemainSize	=	inFileSize;

	while (iRemainSize) 
	{
		int	iReadSize	=	min(BLOCKSIZE,iRemainSize);

		fread(tempBLOCK, iReadSize,1,inFile);
		fwrite_crc((BYTE*)tempBLOCK, iReadSize, outFile);  

		OriginalSize += iReadSize;

		cNUX::FileRate	=	ftell(inFile)*100/inFileSize;

		iRemainSize	-=	iReadSize;

		if	(cNUX::ProcessPress)
			cNUX::ProcessPress();
	}
// 	while (iRemainSize) 
// 	{
// 		int	iReadSize	=	min(DICSIZ,iRemainSize);
// 
// 		fread((char *)buffer, iReadSize,1,inFile);
// 		fwrite_crc(buffer, iReadSize, outFile);  
// 
// 		OriginalSize += iReadSize;
// 
// 		cNUX::FileRate	=	ftell(inFile)*100/inFileSize;
...
//	outFile	=	NULL;
	...
	{	int read	=	read_header();
		...
		{	SetCurrentDirectory(processFolder);
			...
			{	if (!cFOLDER::Create(folderName,TRUE)) return cMSG::Error("Error in cNUX::PressExtract","Don't Create \"%s\" Folder",folderName);
				if (files)
				{	char folder[512];
