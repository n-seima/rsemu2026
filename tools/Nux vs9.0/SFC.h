...
//#define	dDEBUG_MODE						//		디버깅 모드 이다. 조건문이 많아 지는 대신 에러검색이 쉬워쥔다.준다.
...
#define cKILL(buff) if (buff) {delete buff		;buff=NULL;}	// 클래스 삭제..
#undef  ATTEMPT
#define ATTEMPT(x)	{ if (FAILED(x)) goto generic_error; }		//	에러 처리..
#undef  RELEASE
...
extern	char		_logFolder[512];						//	프로그램 실행 폴더

extern	char*		gCpuSignature;										//	CPU이름..
extern	void		memset(WORD *dest,WORD value,unsigned int size);	//	16비트 용..
extern	inline int	random(int range);

extern	void*		(*fastcpy)(void *pDest ,const void *pSrc,unsigned int size);

extern	BOOL		_makeShotcut(char* src, char* lnkName, char* lnkPath);
extern	char		*_ms(char *str,...);					//	문장을 만든다.
