...
CSharedMemory::create(char *_lpstrName,DWORD _dwBufferSize)
{
	...
	m_hMemMap	=	::CreateFileMappingA(INVALID_HANDLE_VALUE,	// 파일 맵의 핸들, 초기에 INVALID_HANDLE_VALUE를 설정한다.
										NULL,					// 보안 속성
	...
	m_hMutex	=	::CreateMutexA(NULL, FALSE,strName);
...
// open the file mapping
    m_hMemMap	=	::OpenFileMappingA(FILE_MAP_ALL_ACCESS,FALSE,strName);

    if	(!m_hMemMap)	return FALSE;
...
// get the handle of mutex
	...
	m_hMutex	=	::OpenMutexA(MUTEX_ALL_ACCESS, FALSE,strName);
