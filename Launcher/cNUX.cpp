...
BOOL		cNUX::bMERGE				=	FALSE;
BOOL		cNUX::bMERGEFOLDER			=	FALSE;

BOOL		cNUX::bSTOP					=	FALSE;

cNUX_INFO	l_infoNux;
void		(*l_fpExtractProgress)(void *lpInfo)	=	NULL;
void		(*l_fpPressProgress)(void *lpInfo)		=	NULL;
...
BOOL		bAPPEND						=	FALSE;
const		DWORD	BLOCKSIZE			=	1024*100;
...
//		if (cNUX::l_fpPressProgress) cNUX::l_fpPressProgress();
	}
...
void fwrite_crc(BYTE *p, int n, FILE *f)
{	if (fwrite(p, n,1, f) < 1) cMSG::Error("Decode Error!!","Unable to write");
...
cNUX::IsCanceled()
{
	return	l_infoNux.IsCanceled();
}

BOOL
cNUX::IsPaused()
{
	return	l_infoNux.IsPaused();
}

void
cNUX::SetExtractFunc(void (*extract)(void *lpInfo))
{
	l_infoNux.Reset();
	l_fpExtractProgress	=	extract;
}

void
cNUX::SetPressFunc(void (*press)(void *lpInfo))
{	
	l_fpPressProgress	=	press;
}

BOOL
cNUX::PressCreate(char *fn,BOOL merge)
{
	bMERGE	=	merge;

	if (outFile) return cMSG::Error("Error!! in cNUX::PressCreate","Achive File Already Opened!!");
	bStop	=	FALSE;
	bSTOP	=	FALSE;
	bAPPEND	=	FALSE;
	...
	strcpy_s(tempArc,_ms("%s\\%s",processFolder,tempName));

	SetFileAttributes(tempArc,FILE_ATTRIBUTE_ARCHIVE);
	DeleteFile(tempArc);

	fopen_s(&outFile,tempArc, "wb");

	if	(!outFile)
		return cMSG::Error("Compress Error!!","'%s' File Create Error!!",fn);
...
BOOL
cNUX::PressAppend(char *fn,BOOL merge)
{	bMERGE	=	merge;

	if	(outFile)
		return cMSG::Error("Error!! in cNUX::PressCreate","Achive File Already Opened!!");
...
DWORD
cNUX::PressClose()
{	if (!outFile) return cMSG::Error("Error!! in PressFolder","Achive File not Created!!");
...
{	if (bStop) return FALSE;
	...
	if (!outFile) return cMSG::Error("Error!! in cNUX::PressFile","Achive File not Created!!");

	long headerpos, arcpos;

	strcpy_s(filename,1024,_exportFileName(fn));

	if ((fopen_s(&inFile,fn, "rb")) == NULL)
		return PressFolder(fn,TRUE,TRUE);

	strcpy_s(cNUX::CurrentFile,filename);
...
//	skip();
	...
	FileRate	=	100;
//	if (l_fpPressProgress) l_fpPressProgress();
...
{	if (bStop) return FALSE;
	...
	strcpy_s(filename,_exportFileName(fn));
...
{	if (bStop			)	return FALSE;
	...
	{	cSTRING	*str	=	strs->Pick(0);
		...
		if (includeCurrent)	strcpy_s(temp,_exportFolderName(fn));
		else
		{	strcpy_s(temp,fn);
			PressAllFile();
		}
...
BOOL
cNUX::PressExtract(char *fn,char *destFolder,char *strLogFile,cSTRINGS *files)
{	if (arcFile) return cMSG::Error("Error!! in cNUX::PressExtract","Achive File Aleady Opened!!",fn);	//	화일을 찾을 수 없다.

	l_infoNux.Reset();

	int	iSaveItemCount	=	0;
	bStop		=	FALSE;
	bSTOP		=	FALSE;
	ProcessRate	=	0;
	ProcessSize	=	0;
...
//	초기화
	...
	strcpy_s(l_infoNux.m_strDestFolder,destFolder);

	strcpy_s(ArcName,fn);

	fopen_s(&arcFile,fn, "rb");

	if (!arcFile) return cMSG::Error("Error!! in cNUX::PressExtract","'%s' File not Open",fn);	//	화일을 찾을 수 없다.
	...
	{	fclose(arcFile);
		...
		if (ispress==ISMERGEFILE || ispress==ISMERGEFOLDER)	return MergeExtract(fn,destFolder,strLogFile,files);
		return	cMSG::Error("Error!! in cNUX::PressExtract","'%s' File not NUX file",fn);	//	화일을 찾을 수 없다.
	}
	...
	if (destFolder)
		if(SetCurrentDirectory( destFolder) == 0) 
			if (!cFOLDER::Create(destFolder,TRUE)) return cMSG::Error("Error!! in cNUX::PressExtract","Don't Create \"%s\" Folder",destFolder);
...
//	outFile	=	NULL;
	...
	l_infoNux.m_dwBeginTime		=	timeGetTime();
	l_infoNux.m_dwTotalSize		=	TotalSize;
	l_infoNux.m_dwProgressSize	=	0;
	l_infoNux.m_dwProgressTime	=	0;
	l_infoNux.m_dwBps			=	0;
	l_infoNux.m_iProgressRate	=	0;
	l_infoNux.m_iFileCount		=	FileCount;
	l_infoNux.m_iCurrentFile	=	0;

	cFILE	fileLog;

	if (strLogFile)
	{
		fileLog.Open(strLogFile,"wb");
		fileLog.Write(&FileCount,4);
	}
	...
	{	int read	=	read_header();
		...
		{	if (outFile)	fclose(outFile);
			...
			return cMSG::Error("Error in cNUX::PressExtract","CRC Error!!");	//	CRC Error
		}
		if (read == 2)
		{
			SetCurrentDirectory(processFolder);
			...
			{
				if	(!cFOLDER::Create(folderName,TRUE))
					return cMSG::Error("Error in cNUX::PressExtract","Don't Create Follow Folder\n\n%s",folderName);
			...
			sprintf_s(l_infoNux.m_strDestFolder,"%s\\%s",destFolder,folderName);

			if (strLogFile)
			{
				fileLog.Write4(1);
				fileLog.Write4(strlen(folderName)+1);
				fileLog.Write(folderName,strlen(folderName)+1);
				iSaveItemCount++;
			}
		...
		strcpy_s(l_infoNux.m_strCurrentFile,filename);

		if (!outFile)
		{
			SetCurrentDirectory(processFolder);

			return cMSG::Error("Error in cNUX::PressExtract","'%s' File Create Error!!",filename);
		}

		if (strLogFile)
		{
			fileLog.Write4(0);
			fileLog.Write4(strlen(filename)+1);
			fileLog.Write(filename,strlen(filename)+1);
			iSaveItemCount++;
		}
		...
		{	if (outFile)	fclose(outFile);
			...
			return cMSG::Error("Error in cNUX::PressExtract","'%s' File header Error!!",filename);
		}
		...
		{	//	fprintf(stderr, "There's an extended header of size %u.\n",ext_headersize);
			...
			{	if (outFile)	fclose(outFile);
				...
				return cMSG::Error("Error in cNUX::PressExtract","File read Error!!");
			}
		...
		l_infoNux.m_dwFileSize	=	OriginalSize;
		...
		{	n = (WORD)((OriginalSize > DICSIZ) ? DICSIZ : OriginalSize);
			...
			{
				if (fread((char *)buffer, n,1, arcFile)	!=	1)
				{	if (outFile)	fclose(outFile);
					outFile	=	NULL;
			...
			l_infoNux.m_dwProgressSize		=	ProcessSize+fsize;
			l_infoNux.m_dwProgressFileSize	=	fsize;
			l_infoNux.Update();

			if (TotalSize<=d1MEGA)	ProcessRate	=	(ProcessSize+fsize)*100/TotalSize;
			else					ProcessRate	=	(ProcessSize+fsize)/(TotalSize/100);

			if	(l_fpExtractProgress)
				l_fpExtractProgress((void *)&l_infoNux);
		}
		...
		l_infoNux.m_dwProgressSize		=	ProcessSize;
		l_infoNux.m_dwProgressFileSize	=	cFILE::Size(outFile);
		l_infoNux.Update();

		if (l_fpExtractProgress)	l_fpExtractProgress((void *)&l_infoNux);
		...
		{	if (outFile)	fclose(outFile);
			...
			return cMSG::Error("Error in PressExtract","CRC error(last piece)");
		}
	}
	...
	{
		fileLog.Rewind();
		fileLog.Write(&iSaveItemCount,4);
	...
	l_infoNux.m_dwProgressSize		=	TotalSize;
	l_infoNux.m_dwProgressFileSize	=	l_infoNux.m_dwFileSize;
	l_infoNux.Update();
...
{	if (outFile) return cMSG::Error("Error!! in cNUX::MergeCreate","Achive File Already Opened!!");
	...
	tmpnam_s(strTempName);
	strcpy_s(tempArc,_ms("%s\\%s",processFolder,strTempName));

	fopen_s(&outFile,tempArc, "wb");
	if (!outFile) return cMSG::Error("Merge Error!!","templete file creation Error!!",tempArc);
...
{	if (outFile) return cMSG::Error("Error!! in cNUX::MergeCreate","Achive File Already Opened!!");
	...
	fopen_s(&outFile,fn, "rb+");
	if (!outFile) return cMSG::Error("Error in cNUX::MergeAppend!!","'%s' File Append Error!!",fn);
...
{	if (!outFile) return cMSG::Error("Error!! in PressFolder","Achive File not Created!!");
	...
	int	loc	=	ftell(outFile);

	if (!bMERGEFOLDER)	qsort((void *)Info,FileCount,sizeof(Info[0]) ,Compare_Name);

	fwrite(Info	,sizeof(Info[0])*FileCount,1,outFile);

	fwrite(&appendPos		,sizeof(DWORD  ),1,outFile);

	if (bMERGEFOLDER)	fwrite(&ISMERGEFOLDER	,sizeof(DWORD  ),1,outFile);
...
{	if (!outFile) return;
	...
	strcpy_s(Info[FileCount].FName,"folder");
...
{	if (bStop		)	return FALSE;
	if (!outFile	)	return cMSG::Error("Error!! in cNUX::MergeFile","Achive File not Created!!");
	if (IsFolder(fn))	return cMSG::Error("Error!! in cNUX::MergeFile","'%s' file not file",fn);;

	FILE *fp;

	fopen_s(&fp,fn,"rb");

	if (!fp)		return cMSG::Error("Error!! in cNUX::MergeFile","'%s' File not found!!",fn);

	strcpy_s(Info[FileCount].FName,_exportFileName(fn));
...
BOOL
cNUX::MergeExtract(char *fn,char *destFolder,char *strLogFile,cSTRINGS *files)
{	if (arcFile) return cMSG::Error("Error!! in cNUX::MergeExtract","Achive File Aleady Opened!!",fn);	//	이미 열려 있다.
...
//	초기화
	...
	if (!arcFile) return cMSG::Error("Error!! in cNUX::MergeExtract","'%s' File Not Open",fn);			//	화일을 찾을 수 없다.
	...
	{	fclose(arcFile);
		arcFile	=	NULL;
		if (ismerge==ISPRESSFILE)	return PressExtract(fn,destFolder,strLogFile,files);
		return	cMSG::Error("Error!! in cNUX::MergeExtract","'%s' File Not Merge File!!",fn);			//	합쳐진 화일이 아니다.
	}

	if (destFolder)
		if(SetCurrentDirectory( destFolder) == 0) 
			if (!cFOLDER::Create(destFolder,TRUE)) 
				return cMSG::Error("Error!! in cNUX::MergeExtract","Don't Create \"%s\" Folder",destFolder);
	...
	fread(&TotalSize,4,1,arcFile);			//	전체 사이즈
	...
	for (i=0;i<FileCount;i++,l_infoNux.m_iCurrentFile++)
	{	DWORD	size;

		fseek(arcFile,Info[i].Locate,SEEK_SET);

		if (_stricmp(Info[i].FName,"folder")==0)
		{	fread(tempBLOCK,Info[i].Size,1,arcFile);

			if (destFolder)
					SetCurrentDirectory( destFolder);
			else	SetCurrentDirectory(processFolder);

			if (!IsFolder(tempBLOCK))
			{	if (!cFOLDER::Create(tempBLOCK,TRUE)) return cMSG::Error("Error in cNUX::MergeExtract","Don't Create \"%s\" Folder",tempBLOCK);
			...
			{
				fileLog.Write4(1);
				fileLog.Write4(Info[i].Size);
				fileLog.Write(tempBLOCK,Info[i].Size);
			}
			continue;
		}

		SetFileAttributes(Info[i].FName,FILE_ATTRIBUTE_ARCHIVE);
		DeleteFile(Info[i].FName);

		FILE	*fp;
		fopen_s(&fp,Info[i].FName,"wb");
		strcpy_s(CurrentFile,Info[i].FName);
		strcpy_s(l_infoNux.m_strCurrentFile,CurrentFile);

		if (!fp)
		{
			cMSG::Error("Error in cNUX::MergeExtract","File Create Error!!");
			continue;
		}
...
//		MSGOUT(CurrentFile);
		...
		{
			fileLog.Write4(0);
			fileLog.Write4(strlen(Info[i].FName)+1);
			fileLog.Write(Info[i].FName,strlen(Info[i].FName)+1);
		}
		...
		l_infoNux.m_dwFileSize			=	size;
		l_infoNux.m_dwProgressFileSize	=	0;
		...
		for (j=0;j<(size/BLOCKSIZE);j++)
		{
			...
			l_infoNux.m_dwProgressSize		+=	BLOCKSIZE;
			l_infoNux.m_dwProgressFileSize	+=	BLOCKSIZE;
			l_infoNux.Update();
		...
		l_infoNux.m_dwProgressSize		+=	(size%BLOCKSIZE);
		l_infoNux.m_dwProgressFileSize	=	size;
		l_infoNux.Update();
	...
	if (strLogFile)	fileLog.Close();

	l_infoNux.m_dwProgressSize		=	l_infoNux.m_dwTotalSize;
...
cNUX_INFO::Update()
{
	...
	m_dwProgressTime		=	(dwCurrentTime-m_dwBeginTime)/1000;

	if	(m_dwBeginCheckTime	!=	0)
	{
		if	(dwCurrentTime	>=	m_dwBeginCheckTime+1000*10)
		{
			int	iSize			=	m_dwProgressSize-m_dwBeginCheckSize;
			m_dwBps				=	iSize / 10;
			...
			if	(m_dwBps)
			{
				int	iRemainSize		=	m_dwTotalSize-m_dwProgressSize;
				m_dwRemainTime		=	iRemainSize/m_dwBps+1;

				if	(m_iProgressRate==	1000)
					m_dwRemainTime	=	0;
			}
		}
	}

	if	(m_dwRemainTime	&&	m_dwRemainTime	!=	0xffffffff	&&	m_dwOldTime/1000	!=	dwCurrentTime/1000)
		--m_dwRemainTime;

	m_dwOldTime				=	dwCurrentTime;

	if	(m_dwBeginCheckTime	==	0	&&	m_dwProgressTime	>=	5)
	{
		m_dwBeginCheckTime	=	dwCurrentTime;
		m_dwBeginCheckSize	=	m_dwProgressSize;
	}

	if	(m_dwTotalSize		!=	0)
		m_iProgressRate		=	(int)(((UINT64)m_dwProgressSize*1000)/m_dwTotalSize);
	if	(m_dwFileSize		!=	0)
		m_iProgressFileRate	=	(int)(((UINT64)m_dwProgressFileSize*1000)/m_dwFileSize);
//	if	(m_dwProgressTime	!=	0)
//		m_dwBps				=	m_dwProgressSize / m_dwProgressTime;
}