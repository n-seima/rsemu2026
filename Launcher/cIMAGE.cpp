...
void
cSPRITE::uploadImage(int _iCount,WORD *_lpSpriteData,int *_lpiOffset)
{
	Count			=	_iCount;

	_free(p16DATA);
	_free(spriteOFFSET);
	p16DATA			=	_new(WORD,_lpiOffset[_iCount],"sprite");
	spriteOFFSET	=	_new(int,_iCount+1,"sprite offset");

	memcpy(p16DATA,_lpSpriteData,_lpiOffset[_iCount]*2);
	memcpy(spriteOFFSET,_lpiOffset,sizeof(int)*(_iCount+1));
}
...
{	cFILE	*file;
	...
	if (!file)	return FALSE;//ERRMSG("Error in cANM::Load - '%s' file not found",fn);

	strncpy_s(FName,fn,sizeof(FName));
...
cIMAGE::LOAD(char *fn,BOOL change)
{
	...
	if (_stricmp(_exportExt(fn),"tga")==0)	temp	=	new cTGA();
	else
	if (_stricmp(_exportExt(fn),"bmp")==0)	temp	=	new cBMP();
	else
	if (_stricmp(_exportExt(fn),"pcx")==0)	temp	=	new cPCX();
	else	return NULL;
...
{	cFILE *file;
	...
	strncpy_s(Name,_exportName(fn),sizeof(Name)-2);
...
cOBJ_ANM::PutScale(int x,int y,int _iScale)
{
	...
	if (m_iObjectType	==	dOBJECT_TYPE_ANM)	cSPRITE::Put(x,y,1,_iScale,_iScale);
}
