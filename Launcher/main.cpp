...
{
	strcpy_s(g_strCommandLine,GetCommandLine());

#ifdef	_FOR_JAPAN
	g_strMessage	=	new char* [1024];

	for (int i=0;i<1024;i++)
		g_strMessage[i]	=	new char [256];

	{
		// Load the user-defined resource.
		HRSRC	hTextData	=	::FindResource(_hInst, "TEXT_JAPAN", "CUSTOM");
		HGLOBAL	hRes		=	::LoadResource(_hInst, hTextData);
		DWORD	dwSize		=	::SizeofResource(_hInst, hTextData);

		int		iCount		=	0;

		if	(hRes	!=	NULL)
		{
			char*	lpData	=	(char*)::LockResource(hRes);

			cDECODER	decoder;

			decoder.UploadBuffer(lpData,NULL);

			while(1)
			{
				char	*lpstrText	=	decoder.GetStream();

				if	(!lpstrText || strcmp(lpstrText,"stream not Found!!") == NULL)
					break;

				strcpy(g_strMessage[iCount++],lpstrText);
			}
		}
		else
		{
...

	{
		...
		if	(lpstrKeyword && _stricmp(lpstrKeyword,"serviceid") == NULL)
		{
			...
			strcpy_s(g_strUserID,lpstrUserID);

			decoder.GetWord();
			char*	lpstrToken		=	decoder.GetWord();

			strcpy_s(g_strTokken,lpstrToken);
		}
		else
		{
#ifdef	_FOR_USA
...
#ifdef	_FOR_USA
		strcpy_s(g_strAddress,"113.130.71.109");
#endif
	}
