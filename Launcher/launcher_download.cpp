...
cLauncher::progress(cDOWNLOAD *_lpInfo)
{
	...
	{
		strcpy_s(m_strPatchStatusText,dMSG_NOT_EXIST_PATCH_DATA);
...
//		SetDlgItemText(m_hProgressWnd,IDC_DOWNLOAD_TEXT,dMSG_CONFIRM_VERSION_INFO);
	...
	else
	{
		for (int i=strlen(_lpInfo->m_strUrl)-1;i>=0;i--)
			...
			{
				strcpy_s(strText,_lpInfo->m_strUrl+i+1);
				break;
			}

		strcpy_s(m_strPatchStatusText,dMSG_DOWNLOAD_PATCH_DATA);
		...
		sprintf_s(m_strCurrentDownLoadFileInfo,"%s(%d.%.2dMB/%d.%.2dMB)",strText,iReceivedSizeHigh,iReceivedSizeLow,iDataSizeHigh,iDataSizeLow);
...
//		iReceivedSizeLow	=	iReceivedSizeLow*100/1024;
		...
		sprintf_s(m_strDownLoadInfo,"%s... %d/%d files(%d.%.2dMB Received)",
...
//	m_dwBps가 킬로바이트 단위이다.
	if	(_lpInfo->m_dwBps/1024>0)
	{
		...
		sprintf_s(m_strDownloadSpeed,"%s : %d.%.3d MB/Sec",dMSG_TRANS_SPEED,iValue1,iValue2);
	}
	else
	{
		int	iValue1	=	(int)_lpInfo->m_dwBps;

		sprintf_s(m_strDownloadSpeed,"%s : %d KB/Sec",dMSG_TRANS_SPEED,iValue1);
	}
}
...
#ifdef	_DEBUG
//			sprintf_s(strText,"ftp://211.189.122.82/redstone_patch%.4d.nux",iPatchVersion);
//			sprintf_s(strText,"ftp://redstone.feelamint.com/redstone/patch/redstone_patch%.4d.nux",iPatchVersion);
			sprintf_s(strText,"%s%.4d.nux",m_strPatchDataAddress,iPatchVersion);
#else
			sprintf_s(strText,"%s%.4d.nux",m_strPatchDataAddress,iPatchVersion);
#endif
			...
			strcpy_s(m_strPatchStatusText,dMSG_INSTALL_DOWNLOAD_PATCH_DATA);
