...
/*	16bit	이미지 읽어 오기..						*/
	...
	static void	PutImageScale	(int x,int y,int xs,int ys,WORD *img,int _iXScale,int _iYScale);
	static void	PutImage		(int x,int y,int xs,int ys,BYTE *img,WORD *plt);
...
extern			WORD*	PCX24TO16(BYTE *buffer,int xs,int ys);
