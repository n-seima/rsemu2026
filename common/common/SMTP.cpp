...
bool cSmtp::ConnectServer(const char server[], const unsigned short port)
{
    assert(server);
...
SOCKET cSmtp::_connectServerSocket(const char server[],
...
const char* const cSmtp::GetSenderEmail() 
{
...
void cSmtp::SetSenderEmail(const char name[], const char email[])
{
	assert(name);
	assert(email);
    ...
    if (m_pcFromName)
        delete [] m_pcFromName;
    m_pcFromName = new char[size + s + 5];
	sprintf(m_pcFromName, "%s<%s>",name,email);
}
void cSmtp::SetSenderEmail(const char email[])
{
    assert(email);
    int s=strlen(email);
    if (m_pcFromEmail)
