...
static DWORD SS0[256],SS1[256],SS2[256],SS3[256];
...
/************************ Block Encryption *************************/
void cSEED::SeedEncrypt(BYTE *pbData, DWORD *pdwRoundKey)
{
...
void cSEED::SeedDecrypt(BYTE *pbData, DWORD *pdwRoundKey)
{
    DWORD L0, L1, R0, R1, T0, T1, *K=pdwRoundKey;
...
void cSEED::SeedEncRoundKey(DWORD *pdwRoundKey, BYTE *pbUserKey)
{
...
{
	FILE *fp;
	fp = fopen("seed.dat","rb");
	fseek(fp,4l,SEEK_SET);
	fread(SS0,256*sizeof(DWORD),1,fp);
	fread(SS1,256*sizeof(DWORD),1,fp);
	fread(SS2,256*sizeof(DWORD),1,fp);
	fread(SS3,256*sizeof(DWORD),1,fp);
	fclose(fp);
}

void cSEED::EncryptOR(BYTE *data,int size,BYTE	key)
{
	for(int i=0;i<size;i++)
	{	*(data+i) ^= key;	}
}

void cSEED::DecryptOR(BYTE *data,int size,BYTE	key)
{
	for(int i=0;i<size;i++)
	{	*(data+i) ^= key;	}
}
