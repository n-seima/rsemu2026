...
//	스크롤바 버튼
	{
		...
		if	(m_iType	==	dIMAGE_SCROLL_BAR_VERTICAL)	
			...
			m_button.addImageBar(dBM_SCROLL_BAR_INCREASE_BUTTON,m_iButtonWidth+m_iBodyCapSize*2,0,m_iNormalDownButton,m_iActiveDownButton,m_iPressedDownButton,0xffff,0xffff);
	}	//	스크롤바 버튼
...
//	스크롤바 몸통 그리기
	{
		...
		if	(m_iType	==	dIMAGE_SCROLL_BAR_VERTICAL)	
		{
			int	iSize		=	m_lpSprite->Get16(m_iScrollBarBody)[1];
			int	iScale		=	max(iBodySize*100/iSize,1);
			int	iScaleSize	=	iScale*iSize/100;

			if	(iScaleSize	!=	iBodySize)
				iScale++;
			
			m_lpSprite->Put(m_pos.x,m_pos.y+iBodyPos+m_iButtonHeight+m_iBodyCapSize,m_iScrollBarBody,100,iScale);
		}
		else
		{
			int	iSize		=	m_lpSprite->Get16(m_iScrollBarBody)[0];
			int	iScale		=	max(iBodySize*100/iSize,1);
			int	iScaleSize	=	iScale*iSize/100;
			...
			m_lpSprite->Put(m_pos.x+iBodyPos+m_iButtonWidth+m_iBodyCapSize,m_pos.y,m_iScrollBarBody,iScale,100);
		}
	}
}	//	CImageScrollBar::draw()
