...
LPCTSTR		lpszAppName			= "BUILDINGEDITOR";
LPCTSTR		lpszTitle			= "건물 에디터";
LPCTSTR		lpszToolbar			= "건물 에디터 툴바";
...
int			g_iBlockSize	=	1;
BOOL		g_aBuildBlock[dMAX_BUILDING_BLOCK];
...
{	Lock			=	FALSE;
	BGAnm			=	0xffff;
	memset(g_aBuildBlock,0,sizeof(g_aBuildBlock));
}
...
{	GetCurrentDirectory(250,curDir);
	...
	if (!sdDATA.Load("data\\blocks.sd"))				return FALSE;
...
{	HMENU	hMENU;
	...
	RemoveMenu(hMENU,ID_MENUSTOP		,MF_BYCOMMAND);
	for (i=0;i<dMAX_ANM	;i++)	RemoveMenu(hMENU,IDM_CHANGEANM		+i,MF_BYCOMMAND);
	for (i=0;i<10		;i++)	AppendMenu(hMENU,MFT_STRING,IDM_CHANGEANM+i		,ANM[i].Name);
	CheckMenuRadioItem(hMENU,IDM_CHANGEANM,IDM_CHANGEANM+cANM::AnmCount-1,IDM_CHANGEANM+cANM::CurAnm,MF_BYCOMMAND);

	hMENU	=	GetMenu(hWND);

	if (cIMG::Count	==	0)	EnableMenuItem(hMENU,1,MF_BYPOSITION|MFS_GRAYED);
	else					EnableMenuItem(hMENU,1,MF_BYPOSITION|MFS_ENABLED);
}
...
{	BOOL	repaint	=	FALSE;
	...
	{	case	VK_ADD		:	_WORKWND.ZoomIn()	;repaint|=1;break;
		...
		case	'X'			:	if (workMode==dWORK_SET_FRAME)	{	repaint|=7	;cANM::PlusCurrentFrame();	}
								if (workMode==dWORK_SET_SPRITE)	{	repaint|=3	;cANM::PlusCurrentImage();	}
								break;
...
//		ScreenToClient(hWND,(POINT *)&_cfg.border);
		_cfg.border.y	=	y;
	...
	{	cRECT rect;
		...
		int	x			=	_cfg.border.x;
//		_cfg.border.y	=	rect.y1;
		_cfg.border.MousePos(hWND);
//		ScreenToClient(hWND,(POINT *)&_cfg.border);
		_cfg.border.x	=	x;
//		_cfg.border.y	-=	TOOLBAR.height()+3;
...
{	OPENFILENAME	ofn;
	...
	ofn.hwndOwner		=	hWND;
	ofn.lpstrFilter		=	"건물 데이터 {*.rbd}\0*.rbd\0\0";
	...
	{	case WM_CREATE		:
		...
		case WM_COMMAND		:
            ...
            {	case	IDM_LOAD		:
				...
				case	IDM_ADD_ANM			:
				case	IDM_ADD_ANM_EFFECT	:
				case	IDM_CLOSED_DOOR		:
				case	IDM_OPEN_DOOR		:
				{	HMENU	hMENU	=	GetSubMenu(GetMenu(_MAIN.hWND),1);
					CheckMenuItem(hMENU,IDM_CHANGEANM+cANM::CurAnm,MF_BYCOMMAND|MF_UNCHECKED);

					hMENU	=	GetSubMenu(GetMenu(_MAIN.hWND),2);
					CheckMenuRadioItem(hMENU,IDM_ADD_ANM,IDM_ADD_ANM+3,LOWORD( wParam ),MF_BYCOMMAND);
					cANM::SetCurrentAnm(LOWORD( wParam )-IDM_ADD_ANM+10);
					_MAIN.changeMode(dWORK_SET_FRAME);
					_MAIN.Draw();
					break;
				}
				...
				default					:
					if (LOWORD( wParam ) >= IDM_CHANGEANM && LOWORD( wParam ) < IDM_CHANGEANM+cANM::AnmCount)
					{	HMENU	hMENU	=	GetSubMenu(GetMenu(_MAIN.hWND),2);
						CheckMenuItem(hMENU,IDM_ADD_ANM+cANM::CurAnm-10,MF_BYCOMMAND|MF_UNCHECKED);

						hMENU	=	GetSubMenu(GetMenu(_MAIN.hWND),1);
						CheckMenuRadioItem(hMENU,IDM_CHANGEANM,IDM_CHANGEANM+cANM::AnmCount-1,LOWORD( wParam ),MF_BYCOMMAND);
						cANM::SetCurrentAnm(LOWORD( wParam )-IDM_CHANGEANM);
						_MAIN.changeMode(dWORK_SET_FRAME);
						_MAIN.Draw();
						break;
					}
