...
BOOL
cANMATION::isBack(int anm,int direct,int frame)
...
BOOL
cANMATION::Load(char *fn,BOOL loadpalette,cNUX *file)
{	FILE *fp;

	fp	=	fopen(fn,"rb");

	if (!fp)	return MSGOUT("Error in cANMATION::Load - '%s' file not found",fn);
...
BOOL
cANMATION::Load(FILE *fp,BOOL loadpalette)
{	int i,j;

	fread(&SADHEADER,sizeof(SADHEADER),1,fp);
#ifdef dDEBUG_MODE
	if (strcmp(SADHEADER.Reg,REGSADHEADER)!=0)
		return MSGOUT("Error in cANMATION::Load - header not match!!");
#endif
...
int
cANMATION::GetSprite(int anm,int direct,int frame)
{	return pANM[anm].Sprite[FlipDirect[bFLIP][pANM[anm].DirectCount][direct]][frame];
}

void
cANMATION::PutReg(int x,int y,int anm,int direct,int frame,int rate)
...
void
cANMATION::Put(int x,int y,int anm,int direct,int frame,int xrate,int yrate,int effect,int alpha)
{	int		fliped	=	IsFlip[bFLIP][pANM[anm].DirectCount][direct];
	int		index	=	pANM[anm].Sprite[FlipDirect[bFLIP][pANM[anm].DirectCount][direct]][frame];
	if (effect	<	dPUT_NORMAL)	alpha	=	effect;
...
void
cANMATION::PutFx(int x,int y,int anm,int direct,int frame,int effect)
{	int		fliped	=	IsFlip[bFLIP][pANM[anm].DirectCount][direct];
	int		index	=	pANM[anm].Sprite[FlipDirect[bFLIP][pANM[anm].DirectCount][direct]][frame];
...
void
cANMATION::Put(WORD *plt,int x,int y,int anm,int direct,int frame,int xrate,int yrate,int effect,int alpha)
{	int		fliped	=	IsFlip[bFLIP][pANM[anm].DirectCount][direct];
	int		index	=	pANM[anm].Sprite[FlipDirect[bFLIP][pANM[anm].DirectCount][direct]][frame];
	if (effect	<	dPUT_NORMAL)	alpha	=	effect;
...
void
cANMATION::PutFx(WORD *plt,int x,int y,int anm,int direct,int frame,int effect)
{	int		fliped	=	IsFlip[bFLIP][pANM[anm].DirectCount][direct];
	int		index	=	pANM[anm].Sprite[FlipDirect[bFLIP][pANM[anm].DirectCount][direct]][frame];
...
void
cANMATION::GetRect(int anm,int direct,int frame,cRECT *rect)
{	int	index	=	pANM[anm].Sprite[FlipDirect[bFLIP][pANM[anm].DirectCount][direct]][frame];
