...
{	srand( (unsigned)time( NULL ) );
	...
	cMSG::hCONSOLEWND	=	FindWindow("CONSOLEBOX","Console Box");
...
{
	int	iResult	=	strcmp(((cFILENAME *)a)->Name,((cFILENAME *)b)->Name);
	return iResult;
...
#ifdef dDEBUG_MODE
	if (!result)	return MSGOUT("Error in cNUX::Open - '%s' file not found",fn);
#endif
...
#ifdef dDEBUG_MODE
	if (index >= Count)
		return MSGOUT("Error in cNUX::Open - out of range %d [0~%d]",index,Count);
#endif
...
char	cMSG::msgBuffer[256];
HANDLE	cMSG::hCONSOLE		=	0;
HWND	cMSG::hCONSOLEWND	=	NULL;
...
{	if (hCONSOLEWND)
	{	FILE *fp;
		...
		{	va_list		argptr;
			...
			SendMessage(hCONSOLEWND,WM_CONSOLETEXT,sizeof(msgBuffer),0);
		}
	}
