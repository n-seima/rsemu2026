...
{	SCROLLINFO	si;
	...
	if (cIMG::Count <= 0 && _MAIN.workMode != dWORK_SET_CRASH)
	{	PAINTSTRUCT ps;
		HDC		hDC		=	GetDC(hWND);
	...
	for (j=0;j<ImgPerCol ;j++)	
		...
		{	if (count >= cIMG::Count) break;
			...
			if (_MAIN.workMode==dWORK_SET_SPRITE)
			{
				...
				if (img->isSelect)	cDRAW::Fill	(_BLUE		,bxp,byp,bxp+imgXS-2,byp+imgYS-2);
				else				cDRAW::Fill	(0			,bxp,byp,bxp+imgXS-2,byp+imgYS-2);
			}
			else
			{
				if (count	==	0)			cDRAW::Fill	(_LTGREEN	,bxp,byp,bxp+imgXS-2,byp+imgYS-2);
				else
				if (g_aBuildBlock[count])	cDRAW::Fill	(_GREEN		,bxp,byp,bxp+imgXS-2,byp+imgYS-2);
				else						cDRAW::Fill	(0			,bxp,byp,bxp+imgXS-2,byp+imgYS-2);
			}
		...
		{	if (_MAIN.workMode == dWORK_SET_SPRITE)
			...
			else
			{
				...
				if (pos!=0xffff && !pressKey )
				{
					...
					{	g_aBuildBlock[pos]	=	1-g_aBuildBlock[pos];
						_MAIN.Draw();
					}
					else
					if (pos!=cIMG::Current)
