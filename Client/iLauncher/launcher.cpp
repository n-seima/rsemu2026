...
// #define	dTEST	0	//	얘는 주석으로 처리 해야 정상이 된다.
...
#elif	_FOR_KOREA
 	lpstrLoginServerIP	=	cSOCKET::GetIP("login.redgem.co.kr");
#endif

	if	(lpstrLoginServerIP	==	NULL)
		lpstrLoginServerIP	=	g_strAddress;

//  	cMSG::Put("try","connect %s",lpstrLoginServerIP);
...
cLauncher::receiveNotice(void *_lpData)
{
	...
	strcpy(m_strPatchDataAddress,lpPacket->strDownloadAddress);
	strcpy(m_strNotice,lpPacket->strMessage);
	...
	if	(_iStatus	==	ePS_ReadyToLaunch)
	{
		if	(m_iServerStatus		==	eSS_ON)
			if	(m_isOperationSuccess	&&	m_iLastestVersion	!=	0)
			{
				...
				if	(iResult	==	ERROR_SUCCESS)
					reg.SetValue(m_iLastestVersion,"Version");	
...
cLauncher::downloadLoginServerIP()
{
	...
	else
	{
		...
		if	(decoder.Upload(m_patch.m_strFileName,NULL))
		{
			...
			strcpy(g_strAddress,lpstrAddress);
...
cLauncher::launch()
{
	...
	if	(lpstrJapanHangame)
	{
		...
		strcpy(strConnectIdCompare,"hangamejapan");
		...
		sprintf(strCommand,"%s%03d%s%03d%s Mongsran%d",strConnectIdCompare,strlen(strId),strId,strlen(strPass),strPass,timeGetTime());
	}
	else
	if	(lpstrPortalParam)
	{
		...
		strcpy(strConnectIdCompare,"portal_redstoneID");
	...
	if	(lpstrParam)
	{
		...
		strcpy(strId,lpstrParam+strlen("redstoneID"));

		sprintf(strCommand,"redstoneID%03d%s Mongsran%d",strlen(strId),strId,timeGetTime());
	}
	else
	if	(g_iServiceID)	//	OGP 용
	{
		...
		sprintf(strCommand,"by_ogp_launcher##@!%s",decoder.GetCurStream());
	}
	else
	{
		sprintf(strCommand,"Mongsran%d",timeGetTime());
	}
