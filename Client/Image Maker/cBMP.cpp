...
{	pKILL(p16IMAGE);
	...
	if (BPP > 8)
	{	p16IMAGE	=	new WORD [rWidth*Height+2];
		...
		{
			for (j=0;j<rWidth*3;j+=3)
			{
				if (i == Height-1&& j >= (rWidth-2)*3)
				{
					int aa=0;
					aa++;
				}
				p16IMAGE[ip++]	=	RGB24To16(data[addr+j+2],data[addr+j+1],data[addr+j ]);
			}
		}
	...
	{	int	ip	=	4,i;
		pPLT	= new WORD	[256];
		p8IMAGE	= new BYTE	[rWidth*Height+4];
