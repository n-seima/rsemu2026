...
#define	NEW(size,type)	(type *)VirtualAlloc(NULL,(size)*sizeof(type),MEM_COMMIT,PAGE_READWRITE) 
#define	FREE(addr)		{ if (addr) VirtualFree(addr,0,MEM_RELEASE);addr = NULL;}
...
public:
	...
	static	BOOL		SaveBox(char *fname,char *filter,char *defaultExt,char *folder=NULL,char *title="Save data");
	static	BOOL		LoadBox(char *fname,char *filter,char *defaultExt,char *folder=NULL,char *title="Load data");
...
public:
	...
	static	cSTRINGS*	GetFolders		(char *root,BOOL includeSub=TRUE);		//	폴더들을 얻어 온다.
...
{
#ifdef dDEBUG_MODE
	char	*data				=	new char [size + 12];

	*(DWORD *)(data+0		)	=	0xfefefefe;
	*(DWORD *)(data+4		)	=	size;		//	사이즈
	*(DWORD *)(data+size+8	)	=	0xfefefefe;	//	사이즈
...
sFREE(char *pointer)
{
	...
	if (pointer)
	{	char	*real	=	pointer-8;
		DWORD	header	=	*(DWORD *)(real);
		DWORD	size	=	*(DWORD *)(real+4);
		DWORD	tail	=	*(DWORD *)(real+size+8);

		if (header	!=	0xfefefefe)	ERRMSG("Error!!","specific pointer head offset damaged!!");
		if (tail	!=	0xfefefefe)	ERRMSG("Error!!","specific pointer tail offset damaged!!");
		pKILL(real);
	}
#else
	pKILL(pointer);
