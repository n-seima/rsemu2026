...
zoomTable zoomXTable[1024];
zoomTable zoomYTable[768];
...
//	확대해서 찍어!!
void
cDRAW::PutImageScale(int x,int y,int iScale,WORD *image)
{
#ifdef dDEBUG_MODE
	if (!image)
	{	MSGOUT("Error!! in cDRAW::PutImage - Valid Image data");
	...
	if (iScale	==	1)
	{
		cDRAW::PutImage(x,y,image);
		return;
	}

	int		offset	=	0,i,j;
	int		iWidth	=	image[0],iHeight=	image[1];
	WORD	*img	=	&image[2];

	for (i=0;i<iHeight;i++,offset+=iWidth)
		for (j=0;j<iWidth;j++)
			Fill(img[offset+j],x+j*iScale,y+i*iScale,x+j*iScale+iScale,y+i*iScale+iScale);
}	//	cDRAW::PutImageScale(int x,int y,int iScale,WORD *img)

//
//	확대해서 찍어!!
void
cDRAW::PutImageScale(int x,int y,BYTE *image,WORD *plt,int iScale)
{
#ifdef dDEBUG_MODE
	if (!image)
	{	MSGOUT("Error!! in cDRAW::PutImage - Valid Image data");
	...
	for (i=0;i<ys;i++,addr+=WIDTH,offset+=width)
		for (j=0;j<xs;j++)
			Fill(plt[img[offset+j]],x+j*iScale,y+i*iScale,x+j*iScale+iScale,y+i*iScale+iScale);
}
...
{	int   x1, y1, x2, y2 ,xs ,ys;
	...
	{	count	=	sprite[bufferPoint++];
		...
		{	xPoint	+=	sprite[bufferPoint++];
			...
			if (xPoint			< 0		) beginX= -xPoint;			//	왼쪽 클리핑..
			if (xPoint+ size	> WIDTH	) endX	= WIDTH-xPoint;		//	오른쪽 클리핑..
			if (endX			> beginX) memset(p2SCREEN+addr+beginX+xPoint,color,endX - beginX);
...
{	int   x1, y1, x2, y2 ,xs ,ys;
	...
	{	count	=	sprite[bufferPoint++];
		...
		{	xPoint	-=	sprite[bufferPoint++];
			...
			memset(p2SCREEN+addr+tail-size+1+beginX,color,endX-beginX);
	...
	{	count	=	sprite[bufferPoint++];
		...
		{	xPoint	-=	sprite[bufferPoint++];
			...
			memset(p2SCREEN+addr+xPoint-size+1,color,size);
...
//
//	x1,y1~x2,y2 안에 있는 이미지를 안티 얼라이싱 시킨다.
//
void
cDRAW::AA(int x1,int y1,int x2,int y2)
{
	int		x,y;
	WORD	wColor,wTopColor,wLeftColor,wRightColor,wDownColor;
	int		iHalfColor;

	if (x1 <= 0			) x1 = 1;
	if (x2 >= WIDTH-1	) x2 = WIDTH-2;
	if (y1 <= 0			) y1 = 1;
	if (y2 >= HEIGHT-1	) y2 = HEIGHT-2;

	int		address	=	y1*WIDTH;

	for (y = y1;y < y2;y++,address+=WIDTH)
		for (x = x1;x < x2;x++)
		{
			wColor			=	p2SCREEN[address+x];
			iHalfColor		=	(wColor&d2HM_MASK)>>1;

			wTopColor		=	p2SCREEN[address+x-WIDTH];
			wDownColor		=	p2SCREEN[address+x+WIDTH];
			wLeftColor		=	p2SCREEN[address+x-1];
			wRightColor		=	p2SCREEN[address+x+1];

			p2SCREEN[address+x-WIDTH]	=	MIX2BYTE(wColor,wTopColor,6,22);;
			p2SCREEN[address+x+WIDTH]	=	MIX2BYTE(wColor,wDownColor,6,22);
			p2SCREEN[address+x-1]		=	MIX2BYTE(wColor,wLeftColor,6,22);;
			p2SCREEN[address+x+1]		=	MIX2BYTE(wColor,wRightColor,6,22);;
/*
			if (wColor	!=	wTopColor	)
				p2SCREEN[address+x-WIDTH]	=	((wTopColor&d2HM_MASK)>>1)+iHalfColor;

			if (wColor	!=	wDownColor	)
				p2SCREEN[address+x+WIDTH]	=	((wDownColor&d2HM_MASK)>>1)+iHalfColor;

			if (wColor	!=	wLeftColor	)
				p2SCREEN[address+x-1]		=	((wLeftColor&d2HM_MASK)>>1)+iHalfColor;

			if (wColor	!=	wRightColor	)
				p2SCREEN[address+x+1]		=	((wRightColor&d2HM_MASK)>>1)+iHalfColor;
				*/
		}
}
...
//
void
cDRAW::AA(int x1,int y1,int x2,int y2,WORD checkColor)
{
	int		x,y;
	WORD	wColor,wTopColor,wLeftColor,wRightColor,wDownColor;
	int		iHalfColor;
	...
	for (y = y1;y < y2;y++,address+=WIDTH)
		for (x = x1;x < x2;x++)
		{
			...
			p2SCREEN[address+x-WIDTH]	=	MIX2BYTE(wColor,wTopColor,10,22);;
			p2SCREEN[address+x+WIDTH]	=	MIX2BYTE(wColor,wDownColor,10,22);
			p2SCREEN[address+x-1]		=	MIX2BYTE(wColor,wLeftColor,10,22);;
			p2SCREEN[address+x+1]		=	MIX2BYTE(wColor,wRightColor,10,22);;

			p2SCREEN[address+x-WIDTH-1]	=	MIX2BYTE(wColor,p2SCREEN[address+x-WIDTH-1],10,22);;
			p2SCREEN[address+x-WIDTH+1]	=	MIX2BYTE(wColor,p2SCREEN[address+x-WIDTH+1],10,22);;
			p2SCREEN[address+x+WIDTH-1]	=	MIX2BYTE(wColor,p2SCREEN[address+x+WIDTH-1],10,22);
			p2SCREEN[address+x+WIDTH+1]	=	MIX2BYTE(wColor,p2SCREEN[address+x+WIDTH+1],10,22);
