...
cSetArca::InitData(HWND hDlg)
{
	...
	//	바닥에 딸어지는 아이템 데이터/리스트
	{
		...
		s_listDropItemList.AddColumn("Category", 6 * 16);
...
cSetArca::DecodeTextScriptFile(char *_lpstrFn)
{
	...
	while(1)
	{
		...
		if (DecodeTextScript(strOneLine+iPoint))
		{
			...
			if (iSerial	==	0xffff)
			{
				ERRMSG("Treasure chest name is duplicate.","%s",s_makeArca.m_strName);
				iOverlappedCount++;
			}
		}
	}

	cMSG::Put("Data file analysis results", "%d treasure chests were added.\n\n%d treasure chest names were duplicated.", g_arcaManager.getCount(), iOverlappedCount);
	...
	{	
			...
			if (((iIndex-iItemBorder)	%	3)	==	0)
			{
				...
				else
				{
					...
					if (lpItem->m_wItemType	==	0xffff)
						return	ERRMSG("error","There is no item of type [%s]!",lpStream);
				}
			}
		...
		switch(iIndex)
		{
			...
			case	14	:
			{
				...
				if (!lpSkill)
					return	ERRMSG("Error!!","There is no trap named '%s'!!",lpStream);
...
cSetArca::Loading(HWND hDlg)
{
	...
	if (!cFILE::LoadBox(strFn, "Text file {*.txt}\0*.txt\0\0", "txt", NULL, "Load treasure box script data file"))
	{
		SetLogFolder();
		return;
	}
...
cSetArca::RemoveArca(HWND hDlg)
{
	...
	if (cMSG::YESNO("Delete the selected treasure chest information", "Are you sure you want to delete it?") == IDNO)
		return;
