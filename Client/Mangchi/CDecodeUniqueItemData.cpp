...
cSET_BASIC_ITEM::DecodeUniqueScriptFile(HWND _hDlg)
{
	...
	if (cBASIC_ITEM::GetUniqueCount() > 0)
		if (cMSG::YESNO(_ms("'%s' currency will be decoded.", strFn), "Do you want to initialize existing unique item data before decoding?") == IDYES)
...
/*
	...
	cMSG::Put("Unique item data parsing results", "You added %d unique items.\n\nThere was a problem with %d items.\n\n%d item names were duplicated", cBASIC_ITEM::s_iCount - iCount, iErrorCount, iOverlappedItemCount);
