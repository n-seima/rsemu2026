...
cMAIN::exportItemInfo(HWND hwnd)	// 09.07.20 수정.
{
	...
	file.writeStreamCR("Number Item Name Minimum Damage Maximum Damage");
//		file.writeStreamCR("%s%-45s%-14s%-14s","Number", "Item name", "Minimum damage", "Maximum damage");
...
//		file.writeStreamCR("%-14d%-45s%-14d%-14d%-14d",g_aBasicItem[i].m_iSerial,g_aBasicItem[i].m_strName,g_aBasicItem[i].m_wMinDamage,g_aBasicItem[i].m_wMaxDamage,g_aBasicItem[ i].m_wKind);
	...
	file.writeStreamCR("Index Prefix1 Prefix2 Minimum value Maximum number Minimum time (seconds) Maximum time (seconds)");
//	file.writeStreamCR("%-14s%-25s%-25s%-14s%-14s%-14s%-14s", "index", "prefix 1", "prefix 2", "minimum number", "maximum number", "Minimum time (seconds)","Maximum time (seconds)");
...
//		file.writeStreamCR("%-14d%-25s%-25s%-14d%-14d%-14d%-14d",i,lpPrefix->m_str1stPrefix,lpPrefix->m_str2stPrefix,lpPrefix->m_aValue[0][0],lpPrefix ->m_aValue[0][1],lpPrefix->m_aValue[1][0],lpPrefix->m_aValue[1][1]);
	...
	cMSG::Put("Item information extraction complete","item Info.txt\nitem Prefix Info.txt\n\nPlease check both files.");
...
cMAIN::exportExpReward(HWND hwnd)
{
	...
	file.Open("Experience reward list.txt","wb");
	...
	cMSG::Put("ok","Experience reward extraction completed");
...
cMAIN::exportMonsterInfo(HWND hwnd)
{
	...
	file.writeString("Field number\tField name\tSerial\tName\tProfession\tName\tMinimum level\tMaximum level\tType\tGrade\tAverage attack power\tAverage defense\tAverage health\tInflight Resistance\tWater Resistance\tEarth Resistance\tWind Resistance\tLight Resistance\tDark Resistance\tAverage Experience\n");
...
cMAIN::exportAllMapDropItemInfo(HWND hwnd)
{
	...
	for (int iLevel=0;iLevel<100;iLevel++)
	{
		...
		file.writeString("Item Type\tField\n");
		...
		for (i=0;i<eIK_END;i++)
		{
			...
			for (int j=0;j<lpInfo->m_iCount;j++)
			{
				...
				if	(lpFieldInfo->m_aiMonsterCount[iLevel])
					file.writeString("\t%d\t%d",lpFieldInfo->m_iField,lpFieldInfo->m_aiMonsterCount[iLevel]);
			}
...
cMAIN::exportNpcInfo(HWND hwnd)
{
	...
	cSTRINGS	*lpFiles	=	cFOLDER::GetFiles("*.rmd");
	file.writeStreamCR("%s\t%s\t%s\t%s","Field number","Name","CoordinateX","CoordinateY");
	if	(!lpFiles)
...
void cMAIN::exportActorKarma(){
	...
	file.writeStreamCR("File name: %-25s Map name: %-25s", g_lpActmap->m_strFName, g_lpActmap->m_strName);
	...
	for(int i=0;i<dMAX_ACTOR_COUNT && iCnt<iCount;++i){
		...
		file.writeStreamCR("Number: %4d Name: %-15s Occupation: %-15s Grade: %-25s Coordinates: X=%3d, Y=%3d",
			pAc->m_iSerial, pAc->getName(), pJobName, pToolName,pAc->m_pos.x/dTILE_WIDTH,pAc->m_pos.y/dTILE_HEIGHT);
		file.writeStreamCR("%-25s","List of lines");
		for(int j=0;j<pAc->m_pCC->m_wConversationCount;++j){
			...
			file.writeStreamCR("\t\t%-25s","Selection list");
			for(int k=0;k<pCs->m_wContentsCount;++k){
				...
				file.writeStreamCR("\t\t\t\t%-25s","Processing contents");
				for(int l=0;l<pKc->getKarmaCount();++l){
					...
					file.writeStreamCR("\t\t\t\t\tActivation probability [%4d%%] Check period [%4d] Activation count limit [%4d] Number of active triggers [%4d]",
						pK->m_wActivateChance, pK->m_wActivePeriod,pK->m_wActiveCount, pK->m_bf7ActivateTriggerCount);
					file.writeStreamCR("\t\t\t\t\t[trigger](condition[%s])",pChAnd);
					...
					file.writeStreamCR("\t\t\t\t\t[Reaction](Activated when conditions are not met[%s])", pChNot);
					...
					for(int n=0;n<pK->m_wReactionCount;++n){
						...
						if(bSpeech){
							file.writeStreamCR("\t\t\t\t\tMove to conversation[%3d. %s].",pRg->m_aValue[0]+1, pAc->m_pCC->m_pConversation[pRg->m_aValue[ 0]].getTitle());
						}else{
...
void cMAIN::exportFieldKarma(){
	...
	SendMessage(hProg, PBM_SETPOS, i, 0);
		...
		file.writeStreamCR("Number: %3d\tName: %-25s", i, pK->m_pstrTitle);
...
void cMAIN::exportMonsterKarma(){
	...
	for(int i=0;i<dMAX_ACTOR_COUNT && iCnt < iCount;++i){
		...
		file.writeStreamCR("Number: %4d Name: %-15s Grade: %-25s ", pCs->m_wSerial, pCs->m_strName, pToolName);
		file.writeStreamCR("\tEvent");
		for(int j=0;j<4;++j){
...
void cMAIN::exportAreaKarma(){
	...
	for(int i=0;i<iCount;++i){
		...
		if(!pAi) continue; 
		SendMessage(hProg, PBM_SETPOS, i, 0);
		file.writeStreamCR("Number: %3d. Name: %-25s", pAi->m_wSerial, pAi->m_strName);
		WORD wKind = pAi->m_wKind;
		
		switch(wKind)
		{
		case dAREA_NORMAL :
			file.writeStreamCR("\tArea: General area");
			break;
		case dAREA_NOTICE_BOARD	:
			file.writeStreamCR("\tArea: Bulletin Board");
			file.writeStreamCR("\tContent: %s",pAi->m_pString);
			break;
		case	dAREA_DOOR :
			{
				file.writeStreamCR("\tArea: Door");
				cAreaDoor *lpDoor =	(cAreaDoor	*)pAi;
				lpDoor->m_bDetectTrapLevel;
				file.writeStreamCR("\tGeneration frequency: %5d\t\t\t\tGeneration probability: %5d",lpDoor->m_wRegenPeriod,lpDoor->m_wGenChance);
				file.writeStreamCR("\tKeep Time: %5d\t\t\t\tSecret Level: %5d",lpDoor->m_wUpkeepPeriod,lpDoor->m_bSecretLevel);
				file.writeStreamCR("\tDurability: %5d\t\t\t\tMinimum level for destruction: %5d",lpDoor->m_iDurability,lpDoor->m_wDestroyMinimumLevel);
				file.writeStreamCR("\tMagic/Physical Resistance: %5d",lpDoor->m_iDefense);
				file.writeStreamCR("\tUnlock level: %5d\t\t\t\tUnlock duration: %5d",lpDoor->m_bUnlockLevel,lpDoor->m_wUpkeepDismantlingTime);
				file.writeStreamCR("\tDoor Destroyable[%s]", ((lpDoor->m_bf1IsDestroyable == 1) ? "O" : "X" ));
				file.writeStreamCR("\tTrap level: %5d\t\t\t\tTrap target: %s",lpDoor->m_wTrapLevel,g_strTrapTarget[lpDoor->m_wTrapTarget]);
				file.writeStreamCR("\tTrap detection level: %5d\t\t\t\tTrap disarming level: %5d",lpDoor->m_bDetectTrapLevel,lpDoor->m_bDisarmTrapLevel);
				file.writeStreamCR("\tTrap Skill 1: %-25s\t\tActivation probability: %3d%%",g_trapSkillManager.m_aTrapSkill[lpDoor->m_wTrapSkill1].m_strName,lpDoor->m_wTrapSkill1ActiveChance);
				file.writeStreamCR("\tTrap Skill 2: %-25s\t\tActivation probability: %3d%%",g_trapSkillManager.m_aTrapSkill[lpDoor->m_wTrapSkill2].m_strName,lpDoor->m_wTrapSkill2ActiveChance);
				cSecretDungeon	*lpSD	=	g_sdManager.get(lpDoor->m_wLinkSecretDungeon);
				file.writeStreamCR("\tConnected secret dungeon: %s",lpSD->m_strName);
				
				
			}
			break;
		case dAREA_PORTAL :
			file.writeStreamCR("\tArea: Entrance");
			file.writeStreamCR("\tShape : %s", g_strPortalShape[pAi->m_gateShape]);
			file.writeStreamCR("\tEntrance Direct: %s", g_strPortalDirect[pAi->m_gateDirect]);
			file.writeStreamCR("\tConnected location: %s", pAi->m_strMoveGateName);
			file.writeStreamCR("\tFile to load: %s", pAi->m_pString);
			
			break;
		case dAREA_MONSTER_ACTIVE :
			file.writeStreamCR("\tArea: Monster activity area");
			break;
		case dAREA_START_AREA :
			file.writeStreamCR("\tArea: Start area");
			break;
		case dAREA_NAMED_AREA :
			file.writeStreamCR("\tArea: Name display area");
			break;
		case dAREA_GUILD_BATTLE	:
			file.writeStreamCR("\tArea: Guild battle area");
			break;
		case dAREA_OX_AREA_O :
			file.writeStreamCR("\tArea: OX Quiz Area (O)");
			break;
		case dAREA_OX_AREA_X :
			file.writeStreamCR("\tArea: OX Quiz Area (X)");
			break;
		case dAREA_REVIVE_AREA :
			file.writeStreamCR("\tArea: Resurrection location");
			break;
		case dAREA_HUNTING_AREA :
			file.writeStreamCR("\tArea: Hunting Area");
			break;
		case dAREA_SECRET_DUNGEON_EXIT :
			file.writeStreamCR("\tArea: Bidun Exit");
			break;
			
		case dAREA_TRAP	:
			{
				file.writeStreamCR("\tArea: Trap");
				cAreaTrap	*lpTrap			=	(cAreaTrap	*)pAi;
				file.writeStreamCR("\tLevel: %5d\t\t\t\tTrap release duration: %d", lpTrap->m_wTrapLevel, lpTrap->m_wTrapRefreshTime);
				file.writeStreamCR("\tDetection level: %5d\t\t\t\tDisarm level: %d", lpTrap->m_wDetectTrapLevel, lpTrap->m_wDisarmTrapLevel);
				file.writeStreamCR("\tTrap Skill 1: %-25s\t\tActivation probability: %3d%%",g_trapSkillManager.m_aTrapSkill[lpTrap->m_wTrapSkill1].m_strName,lpTrap->m_wTrap1ActiveChance);
				file.writeStreamCR("\tTrap Skill 2: %-25s\t\tActivation probability: %3d%%",g_trapSkillManager.m_aTrapSkill[lpTrap->m_wTrapSkill2].m_strName,lpTrap->m_wTrap2ActiveChance);
				file.writeStreamCR("\tTrap target: %s",g_strTrapTarget[lpTrap->m_wTrapTarget]);
			}
			break;
		case dAREA_EVENT_AREA :
			{
				file.writeStreamCR("\tArea: Event Area");
					...
					for(int k=0;k<pCs->m_wContentsCount;++k){
						...
						for(int l=0;l<pKc->getKarmaCount();++l){
							...
							for(int n=0;n<pK->m_wReactionCount;++n){
								...
								char	*lpText		=	cKARMA_WND::KarmaTextOut(pRg,&bSpeech, FALSE);
								if(bSpeech){
									file.writeStreamCR("\t\t\t\t\tMove to conversation[%3d. %s].",pRg->m_aValue[0]+1, pAi->m_cc.m_pConversation[pRg->m_aValue[0 ]].getTitle());
								}else{
		...
		case dAREA_ARCA	:
			...
			file.writeStreamCR("\tArea: Treasure Chest");
			file.writeStreamCR("\tTreasure chest name: %s",lpArca->m_strName);
			}
			break;
		case dAREA_GUILD_CREST :
			file.writeStreamCR("\tArea: Guild Sentence");
			break;
		}
