...
{	int dest=0;
	...
	for (UINT i=0;i<strlen(str);i++)	if (str[i]=='\\'||	str[i]=='/') dest=i+1;
...
BOOL
ustrncpy(char *dest,char *src,int size,BOOL _bIsJapan)
{
	int	index	=	0;
	int	len		=	strlen(src);
	...
	while(1)
	{
		...
		if	(_bIsJapan && dest[index] == -80)
			bIsNot2ByteChar	=	TRUE;

		if	(dest[index] & 0x80	&&	bIsNot2ByteChar	==	FALSE)
		{
...
// 10.01.08
char*	
cSTRING::MinusBack(char *src, char *minusback)
{
	int Index = 0;
	char *tempstr2 = src;
	...
	while(1)
	{
		if(*src == *minusback)
		{
			strncpy(tempStr,tempstr2, Index);
			tempStr[Index] = NULL;
			return tempStr;			
		}
		src++;
...
{	return stricmp(((cSORT_STRING_ITEM *)a)->m_strString,((cSORT_STRING_ITEM *)b)->m_strString);
...
{	cSORT_STRING_ITEM	*pStrings;
	...
	for (i=0;i<iCount;i++)
	{	strcpy(pStrings[i].m_strString,str->String);
...
cFILE::writeString(char *str,...)
{
	...
	va_start(argptr, str);
	vsprintf(strMessage,str,argptr);
	va_end(argptr);
...
{	char	curDir[512];
	...
	if (folder)
	{	Change(folder);
		strcpy(s_strFolder,folder);

		if (dest)
		{	strcpy(s_strFolder,_ms("%s\\%s",folder,dest));
			strcpy(s_destFolder,dest);
		}
	}
...
void _cdecl
cMSG::Copy2ClipBoard(char *mainmsg,...)
{
	HANDLE		hMem;
	LPVOID		lpData;
	va_list		argptr;
...
BYTE	KEY[256];
BYTE	TOUCHKEY[256];
BYTE	CLICKEDKEY[256];	//	키보드 눌렀다 땠다

BYTE	LEFTDBLCLICK	=	FALSE;
BYTE	RIGHTDBLCLICK	=	FALSE;
BYTE	MDBLCLICK		=	FALSE;	//임시..
BYTE	LBUTTON			=	FALSE;
BYTE	RBUTTON			=	FALSE;
BYTE	MBUTTON			=	FALSE;	//임시..
BYTE	TOUCHLBUTTON	=	FALSE;
BYTE	TOUCHRBUTTON	=	FALSE;
BYTE	TOUCHMBUTTON	=	FALSE;	//임시..
...
/*
	if ( KEY[dKEY_MBUTTON] && !oldkey[dKEY_MBUTTON])		//임시
	{	if (timeGetTime() < idleTime2 + 300 )
		{
			if (LastPos.x	==	MOUSE.x	&&	LastPos.y	==	MOUSE.y)
				MDBLCLICK	=	TRUE;
...
GetMoneyString(LONGLONG _llMoney)
{
	...
	if (iUk && iMan && iGold)
	{
		sprintf(s_strGold, "%d%s %d%s %d", iUk, "billion", iMan, "ten thousand", iGold);
	}
	else
		if (iUk && iMan)
		{
			sprintf(s_strGold, "%d%s %d%s", iUk, "billion", iMan, "ten thousand");
		}
		else
			if (iUk && iGold)
			{
				sprintf(s_strGold, "%d%s %d", iUk, "billion", iGold);
			}
			else
				if (iUk)
				{
					sprintf(s_strGold, "%d%s", iUk, "billion");
				}
				else
					if (iMan && iGold)
					{
						sprintf(s_strGold, "%d%s %d", iMan, "Only", iGold);
					}
					else
						if (iMan)
						{
							sprintf(s_strGold, "%d%s", iMan, "Only");
						}
	else
	{
