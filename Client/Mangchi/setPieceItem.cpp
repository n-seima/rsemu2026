	...
	for(int i=iIndex;i<s_deqFilterIndex.size();++i)
	{
		...
		if (s_deqFilterIndex[i]	==	s_iSelectPieceItem)
		{
			...
			if (lpPieceItem->m_wEquipLevel)
				cTEXT::Put		(x+1+36	,y+4+18	,WHITE,_ms("%s [wearing restriction level LV %d]",g_strItemCategory[lpItem->m_wKind],lpPieceItem->m_wEquipLevel));
			else
		...
		else
		{
			...
			if (lpPieceItem->m_wEquipLevel)
				cTEXT::Put		(x+36	,y+3+18	,LTGRAY,_ms("%s [wearing restriction level LV %d]",g_strItemCategory[lpItem->m_wKind],lpPieceItem->m_wEquipLevel));
			else
...
cSetPieceItem::RemovePieceItem(HWND _hDlg)//09.08.20
{
	...
	if (cMSG::YESNO("Remove fragment item", "Do you want to remove the selected fragment item?") == IDNO)
		return;
...
cSetPieceItem::DecodeTextScriptFile(char *_lpstrFn)
{
	...
	while(1)
	{
		...
		if (DecodeTextScript(strOneLine+iPoint))
		{
			...
			if (iSerial	==	0xffff)
			{
				ERRMSG("Piece item name is duplicated.","%s",s_makePieceItem.m_strName);
				iOverlappedCount++;
			}
		}
	}

	cMSG::Put("Data file analysis results", "%d piece items were added.\n\n%d piece item names were duplicated.", g_pieceItemManager.getCount() - 1, iOverlappedCount);
...
cSetPieceItem::DecodeTextScript(char *_lpstrText)
{
		...
		switch(iIndex)
		{
			...
			case	3	:
			{
				if (strstr(lpStream,":"))
				{
					...
					if	(!lpItemSet)
						return	ERRMSG("Error!!","There is no item set called '%s'",lpStream);
					...
					if (s_makePieceItem.m_bf4SetItemIndex	>=	lpItemSet->getItemCount())
						return	ERRMSG("Error!!","Set '%s' has %d items registered.",lpItemSet->getItemCount());
				}
				else
				{
	...
	if (iReversionPieceCount	!=	iReversionPieceCounter)
		return	ERRMSG("Create piece item","Error!! There is a difference between the set number of untradable pieces and the actual number of untradable pieces.\n\n%s) %d:%d",s_makePieceItem.m_strName,iReversionPieceCount,iReversionPieceCounter);

	if (iPieceCounter			!=	s_makePieceItem.m_wPieceCount)
		return	ERRMSG("Piece item production","Error!! There is a difference between the set number of pieces and the actual number.\n\n%s) %d:%d",s_makePieceItem.m_strName,iPieceCounter,s_makePieceItem.m_wPieceCount);
...
cSetPieceItem::LoadingTextScript(HWND _hDlg)
{
	...
	if (!cFILE::LoadBox(strFn, "Text file {*.txt}\0*.txt\0\0", "txt", NULL, "Load fragment item text script data file"))
	{
		SetLogFolder();
		return;
	}
