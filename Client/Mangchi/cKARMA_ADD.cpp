...
cKARMA_WND::DrawTriggerWnd(HWND hwnd)
{
	...
	if (g_EditKarma.m_wTriggerCount <= 0)
	{
		...
		cTEXT::Put(x + 1, y + 2, 0, "There is no specially set situation. The operation will be executed regardless of the situation.");
		cTEXT::PopShadow();
	}
...
cKARMA_WND::DrawReactionWnd(HWND hwnd)
{
	...
	{
		cTEXT::PushShadow(FALSE);
		cTEXT::Put(x + 1, y + 2, 0, "No operation has been set.");
		cTEXT::PopShadow();
	}
