...
extern	BOOL		ustrncpy(char *dest,char *src,int size,BOOL _bIsJapan);
...
public:
	...
	int				getMinuteValue(int _iCorrectYear=2000);	//	=2000을 넣고 싶지만... 이제 와서 넣으면 무슨 사고가 터질지 몰라 안 넣는다.
...
public:
	...
	// 10.01.08
	static	char*	MinusBack(char *src, char *minusback);
};
...
public:
	...
	BOOL	_cdecl		writeString(char *str,...);
...
public:
	...
	static	void _cdecl	Copy2ClipBoard(char *mainmsg,...);
...
public:
	...
	inline	int			getLastIndex()		/// 09.07.17
	{
		if(m_iLimitCount == 0 ) return -1;	// 요고 수정..
	...
	inline void			setRookie(int _iRookie)			/// 09.07.17
	{
		m_iRookie = _iRookie;

	}
	inline void			setCount(int _iCount)			/// 09.07.17
	{
		m_iCount = _iCount;
...
extern	char*	GetMoneyString(LONGLONG _llMoney);
