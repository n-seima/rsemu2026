...
cMakeSecretDungeon::InitData(HWND _hDlg)
{
...

	{
		...
		s_listFloor.AddColumn("Floor", 6 * 4, eLCOM_CENTER);
		s_listFloor.AddColumn("Experience correction", 6 * 13, eLCOM_CENTER);
		s_listFloor.AddColumn("Monster Regen", 6 * 13, eLCOM_CENTER);
	}

	{
		...
		for (i=1;i<=dMAX_FLOOR_COUNT_FOR_SECRET_DUNGEON;i++)
			cCOMBOBOX::Add(hFloorCount, _ms("%2dfloor", i), i);

		cCOMBOBOX::Reset(hSecretDungeonSerial);

		for (i=1;i<=dMAX_SERIAL_FOR_GUILD_DUNGEON;i++)
			cCOMBOBOX::Add(hSecretDungeonSerial, _ms("%2dsecond dungeon", i), i);
	}
...
cMakeSecretDungeon::UpdateFloorList()
{
	...
	for (int i = 0; i<s_makeDungeon.m_bf8FloorCount;i++)
	{
		...
		s_listFloor.AddItem(0, _ms("%dfloor", i + 1), i);
