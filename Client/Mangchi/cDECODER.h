...
#define	dMAX_STREAM_LENGTH		2048*4
...
class cDECODER
{
		...
		BOOL			m_bIsJapan,m_bIsThai;
		BOOL			m_bIsForcedJapan,m_bIsForcedThai;

		cKEYWORD_ITEM	*m_lpKeyWord;
		char			*m_pSTREAM;

public:
		static	BOOL	s_bIsThai;
		...
		void			setForcedJapan(BOOL _bSet)	{m_bIsForcedJapan	=	_bSet;}
		void			setForcedThai(BOOL _bSet)	{m_bIsForcedThai	=	_bSet;}
		...
		void			setJapan(BOOL _bIsSet)
		{
			m_bIsJapan	=	_bIsSet;
		}
		void			setThai(BOOL _bIsSet)
		{
			m_bIsThai	=	_bIsSet;
		}

		BOOL			findCharWithRange(char ch,int _iRange);
		BOOL			isEmptyLine();
		...
		char			*GetStream();
		char			*GetStream(char cEndSymbol,char cEndSymbol2=0);

		char			*GetWord();
		BOOL			FindWord(char *cmpWord);
		int				GetNumber(BOOL _bIsWantMessage=TRUE);
