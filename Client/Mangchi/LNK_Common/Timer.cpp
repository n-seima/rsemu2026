...
cClock::getTerm(cClock *_lpTime)
{
	...
	if	(earlyTime.m_u2Year		>	lateTime.m_u2Year)
		bIsOverTime	=	TRUE;
	else
	if	(earlyTime.m_u2Month	>	lateTime.m_u2Month)
		bIsOverTime	=	TRUE;
	else
	if	(earlyTime.m_u2Day		>	lateTime.m_u2Day)
		bIsOverTime	=	TRUE;
	else
	if	(earlyTime.m_u2Hour		>	lateTime.m_u2Hour)
		bIsOverTime	=	TRUE;
	else
	if	(earlyTime.m_u2Minute	>	lateTime.m_u2Minute)
		bIsOverTime	=	TRUE;
	else
	if	(earlyTime.m_u2Second	>	lateTime.m_u2Second)
		bIsOverTime	=	TRUE;
	...
	while(1)
	{
		if	(earlyTime.m_u2Year	==	lateTime.m_u2Year	&&	earlyTime.m_u2Month	==	lateTime.m_u2Month		&&	earlyTime.m_u2Day		==	lateTime.m_u2Day)
		{
		...
		if	(earlyTime.m_u2Day > GetLastDayInMonth(earlyTime.m_u2Year,earlyTime.m_u2Month))
		{
			earlyTime.m_u2Day		=	1;
			earlyTime.m_u2Month++;
...
cClock::increaseDay(s4 _s4Day)
{
	for (;_s4Day;_s4Day--)
	{
		...
		if	(m_u2Day > GetLastDayInMonth(m_u2Year,m_u2Month))
		{
			m_u2Day	=	1;
			m_u2Month++;
...
cClock::increaseHour(s4 _s4Hour,int _iHourPerDay)
{
	...
	for (;iIncreaseHour;iIncreaseHour--)
	{
		m_u2Hour++;

		if	(m_u2Hour	>=	_iHourPerDay)
		{
			m_u2Hour	=	0;
			m_u2Day++;
...
cTimer::cTimer(void)
{
	this->m_bQueryPerformance	= TRUE;
	
	m_iCountperseconds.QuadPart	= 0;
...
{
	// 고해상도 타이머 지원여부
	if( !QueryPerformanceFrequency( &m_iCountperseconds ) )
	{
		m_bQueryPerformance = FALSE;	
...
float cTimer::getSPF()
{
	if( !m_bQueryPerformance ) // timeGetTime
	{
		m_dCurrentTime	= timeGetTime();
	...
	{	
		QueryPerformanceCounter( &m_iCurrentCount );
		m_fTimeDelta = (float)( m_iCurrentCount.QuadPart - m_iLastCount.QuadPart ) / (float)m_iCountperseconds.QuadPart;
		m_iLastCount.QuadPart = m_iCurrentCount.QuadPart;
	}
