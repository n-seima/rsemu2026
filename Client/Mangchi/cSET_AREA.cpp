...
cRECT					cSET_AREA::s_Rectangle;
BOOL					cSET_AREA::s_isEditArea;
BOOL					cSET_AREA::s_isSpecialArea;
CConversationContainer	cSET_AREA::s_cc;

cRECT		l_SetAreaBoxRect;

void
cSET_AREA::InitSetAreaBox(HWND hDlg)
{
	HWND	hComboAreaKind	=	GetDlgItem(hDlg,IDC_AREA_KIND);
	HWND	hComboSeason	=	GetDlgItem(hDlg,IDC_COMBO_SEASON);
	HWND	hComboSwapArea	=	GetDlgItem(hDlg,IDC_COMBO_SWAP_AREA);
	...
	if (s_isSpecialArea)	//	특수 영역이다.
	{
		HWND	hCombo	=	GetDlgItem(hDlg,IDC_POLL);
		HWND	hOk		=	GetDlgItem(hDlg,IDOK);

		int		kind	=	dAREA_RANGE;

		if(s_isEditArea)
			if (g_lpSelectArea->isHeroRange())	kind	=	dAREA_HERO_RANGE;
		...
		{
			CheckDlgButton(hDlg,IDC_POLL_AREA,TRUE);

			if (g_lpActmap->m_pArea->m_iCount	<=	2)
			{
			...
			for (int i=2,count =2;i<dMAX_AREA && count < g_lpActmap->m_pArea->m_iCount;i++)
			{
				...
				{
					if (g_lpActmap->m_pArea->m_aArea[i].m_wKind	<	dAREA_RANGE)
						cCOMBOBOX::Add(hCombo,g_lpActmap->m_pArea->m_aArea[i].m_strName,i);
				}
			}

			if (s_isEditArea	&&	g_lpSelectArea->m_wKind	==	dAREA_RANGE)
			{
				cCOMBOBOX::SelectByData(hCombo,g_lpSelectArea->x1);
				SetDlgItemInt(hDlg,IDC_RANGE,g_lpSelectArea->x2,FALSE);
			}
			else
			{
				cCOMBOBOX::Select(hCombo,0);
				SetDlgItemInt(hDlg,IDC_RANGE,100,FALSE);
			}
		}
	...
	if (s_isEditArea)		//	수정하는거다.
	{
		SetDlgItemText(hDlg,IDC_NAME,g_lpSelectArea->m_strName);

		s_cc.copy(&g_lpSelectArea->m_cc);

		cCOMBOBOX::SelectByData(hComboAreaKind,g_lpSelectArea->m_wKind);

		Reset(hDlg,g_lpSelectArea);
	}
	else
	{
		if (g_lpActmap->m_pArea->m_iCount >= dMAX_AREA)
		{
			ERRMSG(_ms("Area Count Full (max %d)",dMAX_AREA));
			EndDialog(hDlg,IDCANCEL);
		}

		SetDlgItemText(hDlg,IDC_NAME,_ms("Area %d",g_lpActmap->m_pArea->m_iNewSerial-1));
		...
		cCOMBOBOX::SelectByData(hComboAreaKind,0);
	}
...
BOOL
cSET_AREA::SetArea(HINSTANCE hinst,HWND hwnd,cRECT *rect,BOOL edit,BOOL special)
{
	s_isEditArea	=	edit;
	s_isSpecialArea	=	special;

	if (s_isEditArea)
	{
		if (g_lpSelectArea->m_wKind	>=	dAREA_RANGE)
			s_isSpecialArea	=	TRUE;
	}
	else
		if (rect)	memcpy(&s_Rectangle,rect,sizeof(cRECT));

	if (g_iScale	!=	100)
	{
		cRECT	rect;

		rect.Set(s_Rectangle.x1,s_Rectangle.y1,s_Rectangle.x2,s_Rectangle.y2);
		...
		s_Rectangle.x1	=	g_lpActmap->m_pos.x*dTILE_WIDTH /dSHARE_TILE+rect.x1*100/g_iScale;
		s_Rectangle.y1	=	g_lpActmap->m_pos.y*dTILE_HEIGHT/dSHARE_TILE+rect.y1*100/g_iScale;
		s_Rectangle.x2	=	g_lpActmap->m_pos.x*dTILE_WIDTH /dSHARE_TILE+rect.x2*100/g_iScale;
		s_Rectangle.y2	=	g_lpActmap->m_pos.y*dTILE_HEIGHT/dSHARE_TILE+rect.y2*100/g_iScale;
	}

	if(DialogBox( hinst, "SET_AREA",hwnd,(DLGPROC)s_procSetArea) == IDOK)
		cSET_AREA_EDIT::ResetAreData();
	return	TRUE;
}

void
cSET_AREA::InitTrapAreaForm(HWND hDlg,cAREA_INFO *lpArea,cRECT *_lpRect)
{
	...
	MoveWindow(hDlg,_lpRect->x1,_lpRect->y1,_lpRect->x2-_lpRect->x1,314,TRUE);
...
//	포탈 영역 초기화
void
cSET_AREA::InitPortalAreaForm(HWND hDlg,cAREA_INFO *lpArea,cRECT *_lpRect)
{
...
void
cSET_AREA::InitArcaAreaForm(HWND hDlg,cAREA_INFO *_lpArea,cRECT *_lpRect)
{
...
//	포탈 영역 초기화
void
cSET_AREA::InitDoorAreaForm(HWND hDlg,cAREA_INFO *lpArea,cRECT *_lpRect)
{
	int		i;
...
//	ShowWindow(GetDlgItem(hDlg,IDC_OPEN			),SW_SHOW);
	...
	MoveWindow(hDlg	,l_SetAreaBoxRect.x1,l_SetAreaBoxRect.y1
					,l_SetAreaBoxRect.x2-l_SetAreaBoxRect.x1
					,l_SetAreaBoxRect.y2-l_SetAreaBoxRect.y1
					,TRUE);
...
void
cSET_AREA::Reset(HWND hDlg,cAREA_INFO *lpArea)
{
	if (s_isSpecialArea)
		return;
	...
	ShowWindow(GetDlgItem(hDlg,IDC_AREA_CHECK_NPC	),SW_HIDE);		// 09.08.12
	ShowWindow(GetDlgItem(hDlg, IDC_TEXT_DOOR_DEFENSE_PERCENT), SW_HIDE);	//09.08.13
	ShowWindow(GetDlgItem(hDlg, IDC_DOOR_DEFENSE_PERCENT), SW_HIDE);	//09.08.13
	// 10.02.25 오브젝트 변경
	...
	// insu add
	ShowWindow(GetDlgItem(hDlg,IDC_STATIC_HUNTING_ICON), SW_HIDE);
	ShowWindow(GetDlgItem(hDlg,IDC_STATIC_HUNTING_LEVEL), SW_HIDE);
	ShowWindow(GetDlgItem(hDlg,IDC_HUNTING_ICON_X), SW_HIDE);
	ShowWindow(GetDlgItem(hDlg,IDC_HUNTING_ICON_Y), SW_HIDE);
	ShowWindow(GetDlgItem(hDlg,IDC_HUNTING_LEVEL_LOW), SW_HIDE);
	ShowWindow(GetDlgItem(hDlg,IDC_HUNTING_LEVEL_HIGH), SW_HIDE);
	// insu add end
	...
	switch(lpArea->m_wKind)
	{
		...
		case	dAREA_HUNTING_AREA	:
			{
				ShowWindow(GetDlgItem(hDlg,IDC_STATIC_HUNTING_ICON), SW_SHOW);
				ShowWindow(GetDlgItem(hDlg,IDC_STATIC_HUNTING_LEVEL), SW_SHOW);
				ShowWindow(GetDlgItem(hDlg,IDC_HUNTING_ICON_X), SW_SHOW);
				ShowWindow(GetDlgItem(hDlg,IDC_HUNTING_ICON_Y), SW_SHOW);
				ShowWindow(GetDlgItem(hDlg,IDC_HUNTING_LEVEL_LOW), SW_SHOW);
				ShowWindow(GetDlgItem(hDlg,IDC_HUNTING_LEVEL_HIGH), SW_SHOW);
				MoveWindow(hDlg, rect.x1, rect.y1, rect.x2-rect.x1, 200, TRUE);
				
				cAreaHunting*	lpHunting	=	(cAreaHunting*)lpArea;
				SetDlgItemText(hDlg,IDC_HUNTING_ICON_X	,_ms("%d", lpHunting->m_dwIconX));
				SetDlgItemText(hDlg,IDC_HUNTING_ICON_Y	,_ms("%d", lpHunting->m_dwIconY));
				SetDlgItemText(hDlg,IDC_HUNTING_LEVEL_LOW	,_ms("%d", lpHunting->m_dwLevel_Low));
				SetDlgItemText(hDlg,IDC_HUNTING_LEVEL_HIGH	,_ms("%d", lpHunting->m_dwLevel_High));
			}
			break;
		// insu add end
		...
		case	dAREA_EVENT_AREA:
		{
			...
			MoveWindow(hDlg,rect.x1,rect.y1,rect.x2-rect.x1,226,TRUE);			// 170->226
...
void
cSET_AREA::SetSpecialArea(HWND hDlg,int kind)
{
	if (!s_isSpecialArea)
		return;
...

	{
		...
		for (int i=2,count =2;i<dMAX_AREA && count < g_lpActmap->m_pArea->m_iCount;i++)
		{
			...
			{
				if (g_lpActmap->m_pArea->m_aArea[i].m_wKind	<	dAREA_RANGE)
					cCOMBOBOX::Add(hCombo,g_lpActmap->m_pArea->m_aArea[i].m_strName);
			}
		}
...
BOOL
cSET_AREA::GetAreaData(HWND hDlg)
{
	cAREA_INFO	areaInfo;
	...
	if (s_isSpecialArea)
	{
		...
		if (IsDlgButtonChecked(hDlg,IDC_POLL_AREA))	kind	=	dAREA_RANGE;
		else										kind	=	dAREA_HERO_RANGE;

		data	=	cCOMBOBOX::GetData(hCombo);
		range	=	GetDlgItemInt(hDlg,IDC_RANGE,&temp,FALSE);

		if	(s_isEditArea)
		{
			g_lpSelectArea->m_wKind	=	kind;
			g_lpSelectArea->x1		=	data;
			g_lpSelectArea->x2		=	range;
		}
		else
		{
			char	name[64];

			strcpy(name,_ms("%s[r%d]",g_lpActmap->m_pArea->m_aArea[data].m_strName,range));

			int area	=	g_lpActmap->m_pArea->GetArea(name);

			if (area	!=	0xffff)
				return	ERRMSG(dMSG_ERROR_ADD_AREA,dMSG_EXIST_SAME_NAME_AREA);

			area		=	g_lpActmap->m_pArea->Add(name,kind,data,range);

			//	똑 같은 이름이 있나 확인
			g_lpActmap->m_pArea->Select(area);
		}
...
//	영역 정보 얻어옴
	{
		...
		switch(areaInfo.m_wKind)
		{
			...
			case	dAREA_HUNTING_AREA	:
			{
				cAreaHunting*	lpHunting = (cAreaHunting*)&areaInfo;				

				lpHunting->m_dwIconX = GetDlgItemInt(hDlg,IDC_HUNTING_ICON_X, NULL, FALSE);
				lpHunting->m_dwIconY = GetDlgItemInt(hDlg,IDC_HUNTING_ICON_Y, NULL, FALSE);
								
				lpHunting->m_dwLevel_Low = GetDlgItemInt(hDlg,IDC_HUNTING_LEVEL_LOW, NULL, FALSE);
				lpHunting->m_dwLevel_High = GetDlgItemInt(hDlg,IDC_HUNTING_LEVEL_HIGH, NULL, FALSE);
				break;
			}
			// insu add end
			case	dAREA_NORMAL		:
...
//	중복되는 이름의 영역이 있는지 확인
	{
		...
		{
			//	똑 같은 이름이 있나 확인
//			if (areaInfo.m_wKind	!=	dAREA_PORTAL	&&	areaInfo.m_wKind	!=	dAREA_START_AREA)
//			{
				if(stricmp(name,"")==0)	
...
//			}
		...
		else
		{
			...
			if (areaInfo.m_wKind	!=	dAREA_PORTAL	&&	areaInfo.m_wKind	!=	dAREA_START_AREA)
			{
				if(stricmp(name,"")			==	0		)	return	ERRMSG(dMSG_ERROR_ADD_AREA,dMSG_VALID_AREA_NAME);
				if (g_lpActmap->m_pArea->GetArea(name)	!=	0xffff	)	return	ERRMSG(dMSG_ERROR_ADD_AREA,dMSG_EXIST_SAME_NAME_AREA);

				area	=	g_lpActmap->m_pArea->Add(name,&s_Rectangle);
			}
			else
			{
				if(stricmp(name,"")==0)	area	=	g_lpActmap->m_pArea->Add("portal",&s_Rectangle);
				else					area	=	g_lpActmap->m_pArea->Add(name,&s_Rectangle);
			}
...
//	g_lpSelectArea->m_wMoveGate	=	areaInfo.m_wMoveGate;
	...
	switch(g_lpSelectArea->m_wKind)
	{
		...
		case	dAREA_TRAP			:
		case	dAREA_ARCA			:
		case	dAREA_HUNTING_AREA	: 	// insu add
		{
			memcpy(g_lpSelectArea->m_strMoveGateName,areaInfo.m_strMoveGateName,sizeof(areaInfo.m_strMoveGateName));
			break;
		}
...
LRESULT CALLBACK 
cSET_AREA::s_procSetArea(HWND hDlg,UINT uMsg,WPARAM wParam,LPARAM lParam)
{
	switch(uMsg)
	{
		case WM_INITDIALOG	:
		{
			l_SetAreaBoxRect.OutFrame(hDlg);
			InitSetAreaBox(hDlg);
			// insu add
			SendMessage(GetDlgItem(hDlg,IDC_HUNTING_LEVEL_LOW), EM_LIMITTEXT, 4, 0);
			SendMessage(GetDlgItem(hDlg,IDC_HUNTING_LEVEL_HIGH), EM_LIMITTEXT, 4, 0);
			// insu add end
			break;
		}
...
//			g_MAPWND.Draw();
		...
		case WM_COMMAND :
            ...
            {	case	IDOK			:
				...
				case	IDC_POLL_AREA	:
					SetSpecialArea(hDlg,dAREA_RANGE);
					break;

				case	IDC_POLL_HERO	:
					SetSpecialArea(hDlg,dAREA_HERO_RANGE);
					break;
				...
				case	IDC_SET_EVENT	:
				{
					...
					CConversationContainer*	lpData	=	CSetDialog::Init(hDlg,strName,&s_cc,TRUE);
