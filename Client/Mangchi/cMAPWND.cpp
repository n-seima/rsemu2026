...
cMAPWND::UpdateCommand(int iMenu)
{
	switch(iMenu)
	{
		...
		case	IDM_LINK_TO_EVENT_AREA	:
		{
			...
			{
				cMSG::Put("Event Area and Link","Link Failed - No Object Selected");
				break;
			}

			if	(iObject-0x8000	==	0)
			{
				cMSG::Put("Event area and link","Link failure - Selected object serial number is 0.");
				break;
			}
			...
			while(1)
			{
				...
				if	(iArea	==	0xffff)
				{
					cMSG::Put("Event area and link","Link failure");
					break;
				}
				...
				if	(lpArea->isEventArea()	||	lpArea->isGuildCrest())
				{
					...
					cMSG::Put("Event area and link","'%s' area and link successful",lpEventArea->m_strName);
