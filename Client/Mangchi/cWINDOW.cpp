...
{	hWND = CreateToolbarEx(	hwnd,WS_CHILD | CCS_ADJUSTABLE  | WS_VISIBLE | WS_BORDER ,//| TBSTYLE_FLAT,
	...
	for (index = 0; index < nBitmaps; index++) 	
		lpButtons[index].iBitmap += stdidx;

	SendMessage(hWND, TB_ADDBUTTONS,nBitmaps, (LONG)lpButtons);
	RECT rt;
	SendMessage(hWND, TB_GETITEMRECT, 26, (LPARAM)&rt);
	HWND hStatic = CreateWindow("static", NULL, WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL, rt.left + 267,
    				rt.top+4, 68, (rt.bottom-rt.top-2), hWND, (HMENU)0, hINST, NULL);
	SetParent(hStatic, hWND);

	HFONT hFont = CreateFont(13, 0, 0, 0, 10, 0, 0, 0,HANGEUL_CHARSET, 0, 0, 0, VARIABLE_PITCH | FF_ROMAN, TEXT("Dotum"));
	SendMessage(hStatic, WM_SETFONT, (WPARAM)hFont, MAKELPARAM(FALSE,0));
	SetWindowText(hStatic, "OBJECT ID");
	
	SendMessage(hWND, TB_GETITEMRECT, 27, (LPARAM)&rt);
	HWND hEdit = CreateWindow("edit", NULL, WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL, rt.left + 120,
				rt.top +1, 60, (rt.bottom-rt.top-2), hWND, (HMENU)IDM_OBJECTFIND, hINST, NULL);
	SetParent(hEdit, hWND);
	SendMessage(hEdit, WM_SETFONT, (WPARAM)hFont, MAKELPARAM(FALSE,0));

	m_hStatic2 = CreateWindow("static", NULL, WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL, rt.left + 300,
						rt.top+4, 100, (rt.bottom-rt.top-2), hWND, (HMENU)0, hINST, NULL);
	SetParent(m_hStatic2, hWND);
	SetWindowText(m_hStatic2, "Object Num :");

	m_hStatic2 = CreateWindow("static", NULL, WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL, rt.left + 405,
						rt.top+4, 50, (rt.bottom-rt.top-2), hWND, (HMENU)0, hINST, NULL);
	SetParent(m_hStatic2, hWND);
...
HTREEITEM
cTREEVIEW::Add(char *text,int _iData)
{
	if	(!hWND)
		return NULL;
	...
	tv.item.mask			= TVIF_TEXT | TVIF_SELECTEDIMAGE | TVIF_PARAM;
	tv.item.pszText			= text;
	tv.item.lParam			= _iData;

	return	TreeView_InsertItem( hWND, &tv );
}

HTREEITEM
cTREEVIEW::Add(HTREEITEM hParent,char *text,int _iData)
{
	if	(!hWND)
		return NULL;
...
{	
	...
	hWND	=	CreateWindowEx( 0, "STATIC",str, WS_CHILD | WS_VISIBLE|ws, x, y, width, ys, hwnd,NULL,hinst, NULL );
 	...
 	HGDIOBJ hFont	=	(HGDIOBJ )SendMessage(hMASTER,WM_GETFONT,0, 0 );
	...
	MoveWindow(hWND,x,y,size.cx,ys,TRUE);
...
int
cLISTBOX::GetIndexByText(HWND hwnd, char* pText)			// 09.08.07
{

	for(int i=0;i<cLISTBOX::GetCount(hwnd);++i)
	{
		if(strcmp(cLISTBOX::GetText(hwnd,i) , pText) == 0){
			return i;
		}
...
//
BOOL
cSCROLL_BAR::VScroll(WORD wScroll, int _linestep)
{	SCROLLINFO si;
	...
	switch( wScroll )
	{	case SB_LINEDOWN :
			if ( m_iHeightPos<=	(int)(si.nMax - si.nPage)	)	m_iHeightPos += _linestep;		// 09.12.17 수정
			break;

		case SB_LINEUP :
			if ( m_iHeightPos>	0				)	m_iHeightPos -= _linestep;		// 09.12.17 수정
			break;
