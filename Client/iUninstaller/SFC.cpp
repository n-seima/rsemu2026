...
GetProgrameFilesFolder( LPSTR strBuffer, int buffersize) 
{
	...
	if	(ERROR_SUCCESS != reg.QueryValue(strCommonFilesDir, "ProgramFilesDir", &strsize)) 
	{
		return FALSE;
	}
...
GetExcuteFileName()
{
	return	GetCommandLine(0);
}

char *
GetCommandLine(int locate)
{
	static	char	strTemp[1024];
	char	strCommandLine[1024];
	int		iStringPos=0,index=0;
	BOOL	bString	=	FALSE;

	strcpy(strCommandLine,GetCommandLine());

	for (DWORD i = 0 ; i < strlen(strCommandLine);i++)
	{
		if (strCommandLine[i] == ' ' && !bString)	//	공백을 만났다.
		{
			if (index	==	locate)
			{
				strTemp[iStringPos]	=	0;
				return	strTemp;
			}
		...
		if (strCommandLine[i] == '\"')	//	따옴표를 만났다.
		{
			if (!bString)	bString			=	TRUE;
			else			bString			=	FALSE;

			continue;
		}
		
		strTemp[iStringPos++]	=	strCommandLine[i];
	}
...
{	
	...
	{
		if	(create)
			cFOLDER::Create(s_strFolder,TRUE);
		if	(change)
			SetCurrentDirectory(s_strFolder);
