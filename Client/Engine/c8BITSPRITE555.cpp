...
c8BITSPRITE555::PutSprite()
{
	...
	if (_x<s_rectClippingArea.x1 || _x+Xs > s_rectClippingArea.x2)
	for(;Y1<Y2;Y1++)
	{	
		count	=	_p8SPRITE[bufferPoint++];
		XPoint	=	_x;
		...
		for(i=0;i<count;i++)
		{
			...
			for (j=beginX;j<endX;j++)
				*(p2SCREEN+addr+XPoint+j)=_pPLT[*(_p8SPRITE+bufferPoint+j)];
...
{	int   X1, Y1, X2, Y2 ,Xs ,Ys;
	...
	{	count	=	_p8SPRITE[bufferPoint++];
		...
		{	XPoint	+=	_p8SPRITE[bufferPoint++];
			...
			if (endX			> 0)	memset16(p2SCREEN+addr+XPoint+beginX,_Color,size);
	...
	{	count	=	_p8SPRITE[bufferPoint++];
		...
		{	XPoint	+=	_p8SPRITE[bufferPoint++];
			...
			memset16(p2SCREEN+addr+XPoint+j,_Color,size);
			bufferPoint	+=	size;
			XPoint		+=	size;
		}
		addr+=WIDTH;
		...
		{	XPoint	+=	_p8SPRITE[bufferPoint++];
			...
			for (j=beginX;j<endX;j++)
				p2SCREEN[addr+XPoint+j]
					=((p2SCREEN[addr+XPoint+j]&d2HM_MASK_555)+(_pPLT[_p8SPRITE[bufferPoint+j]]&d2HM_MASK_555))>>1;
...
{	if (_Alpha<=0 ) return;
	...
	{	count	=	_p8SPRITE[bufferPoint++];
		...
		{	XPoint	+=	_p8SPRITE[bufferPoint++];
			...
			for (j=beginX;j<endX;j++)
				p2SCREEN[addr+XPoint+j]	=	MIX2BYTE555(_pPLT[_p8SPRITE[bufferPoint+j]],p2SCREEN[addr+XPoint+j],_Alpha,_DestAlpha);
...
c8BITSPRITE555::PutSpriteFX()
{
	...
	{	count	=	_p8SPRITE[bufferPoint++];
		...
		{	XPoint	+=	_p8SPRITE[bufferPoint++];
			...
			for (j=beginX;j<endX;j++)
				p2SCREEN[addr+XPoint+j]	=	FXMIX555(_pPLT[_p8SPRITE[bufferPoint+j]],p2SCREEN[addr+XPoint+j],_Effect);
...
{	int   X1, Y1, X2, Y2 ,Xs ,Ys;
	...
	{	count	=	_p8SPRITE[bufferPoint++];
		...
		{	XPoint	-=	_p8SPRITE[bufferPoint++];
			...
			for (j=beginX;j<endX;j++) *(p2SCREEN+addr+XPoint-j)=_pPLT[*(_p8SPRITE+bufferPoint+j)];
	...
	{	count	=	_p8SPRITE[bufferPoint++];
		...
		{	XPoint	-=	_p8SPRITE[bufferPoint++];
			...
			for (j=0;j<size;j++) *(p2SCREEN+addr+XPoint-j)=_pPLT[*(_p8SPRITE+bufferPoint+j)];
			bufferPoint	+=	size;
			XPoint		-=	size;
		}
		addr+=WIDTH;
...
void
c8BITSPRITE555::fPutSpriteColor()
{	int   X1, Y1, X2, Y2 ,Xs ,Ys;
	int   XPoint=0,i,j,beginX,endX;
	int   count,bufferPoint=8;
	int   size;
...
{	if (_Alpha<=0 ) return;
	...
	{	count	=	_p8SPRITE[bufferPoint++];
		...
		{	XPoint	-=	_p8SPRITE[bufferPoint++];
			...
			for (j=beginX;j<endX;j++)
				p2SCREEN[addr+XPoint-j]	
					=	MIX2BYTE555(_pPLT[_p8SPRITE[bufferPoint+j]],p2SCREEN[addr+XPoint-j],_Alpha,_DestAlpha);
...
c8BITSPRITE555::PutSpriteR()
{
	...
	_y		=	_y-(iDY*_YRate/100);	//	위치 교정..


	if	(_x + XLen < 0 || _x >= WIDTH || _y + YLen < 0 || _y > HEIGHT )
		return;	//	이미지가 화면에 없다.

	MakeSpriteRTable(_x,_y,Xs,Ys,XLen,YLen);	//	,iXG,iYG

	addr	=	max(_y,0)*WIDTH+_x;
	Ys		-=	iGY;

	for (int iYL=Ys,_y=0;iYL;++_y,--iYL)
	{	
		tempBP	=	bufferPoint;	//	버퍼 포인트 백업.. 

		for (Yp=zoomYTable[_y].size;Yp;--Yp)
		{
			if	(iYG	>	0)
			{
				addr		+=	WIDTH;
			...
			for(i=count;i;--i)
			{
				...
				for (int iLoop=size,j=0;iLoop;--iLoop,++j)
				{
					WORD	wColor;
					...
					for (int k=zoomXTable[j+Xp].size;k;)
					{
						--k;
						*(p2SCREEN+addr+zoomXTable[j+Xp].pos+k)	=	wColor;
					}
				}
...
void
c8BITSPRITE555::PutSpriteRColor()
{
	if (_XRate==100	&& _YRate==100	)
	{
		PutSpriteColor();
		return;
	}
				...
				for (j=0;j<size;j++)
				{
					...
					for (int k=0;k<zoomXTable[j+Xp].size;k++)
						*(p2SCREEN+addr+zoomXTable[j+Xp].pos+k)
							=((p2SCREEN[addr+zoomXTable[j+Xp].pos+k]&d2HM_MASK_555)+(wColor&d2HM_MASK_555))>>1;
				}
		...
		for (Yp=0;Yp<zoomYTable[_y].size;Yp++)
		{
			...
			for(i=0;i<count;i++)
			{
				...
				for (j=0;j<size;j++)
				{
					...
					for (int k=0;k<zoomXTable[j+Xp].size;k++)
						p2SCREEN[addr+zoomXTable[j+Xp].pos+k]
							=	MIX2BYTE555(wColor,p2SCREEN[addr+zoomXTable[j+Xp].pos+k],_Alpha,_DestAlpha);
				}
		...
		for (Yp=0;Yp<zoomYTable[_y].size;Yp++)
		{
			...
			for(i=0;i<count;i++)
			{
				...
				for (j=0;j<size;j++)
				{
					...
					for (int k=0;k<zoomXTable[j+Xp].size;k++)
						p2SCREEN[addr+zoomXTable[j+Xp].pos+k]
							=	FXMIX555(wColor,p2SCREEN[addr+zoomXTable[j+Xp].pos+k],_Effect);
				}
	...
	{	
		...
		for (Yp=0;Yp<zoomYTable[_y].size;Yp++)
		{
			...
			for(i=0;i<count;i++)
			{
				...
				for (j=0;j<size;j++)
					for (int k=0;k<zoomXTable[j+Xp].size;k++)
						*(p2SCREEN+addr+zoomXTable[Xp+j].pos-k-1)=_pPLT[*(_p8SPRITE+bufferPoint+j)];
...
void
c8BITSPRITE555::fPutSpriteRColor()
{	
	int	i,j;
	int Xs,Ys,XLen,YLen;				//	이미지 사이즈(Xs,Ys)  확대된 이미지의 사이즈(XLen,YLen)
	int count,bufferPoint=8,size;		//	디코딩.. count 이미지 수 bufferPoint 버퍼 포인트 size 이미지 사이즈
				...
				for (j=0;j<size;j++)
					for (int k=0;k<zoomXTable[j+Xp].size;k++)
						*(p2SCREEN+addr+zoomXTable[Xp+j].pos-k-1)
						=((*(p2SCREEN+addr+zoomXTable[Xp+j].pos-k-1)&d2HM_MASK_555)+(_pPLT[*(_p8SPRITE+bufferPoint+j)]&d2HM_MASK_555))>>1;
		...
		{	
			...
			{	
				...
				for (j=0;j<size;j++)
					for (int k=0;k<zoomXTable[j+Xp].size;k++)
						p2SCREEN[addr+zoomXTable[Xp+j].pos-k-1]	
							=	MIX2BYTE555(_pPLT[_p8SPRITE[bufferPoint+j]],p2SCREEN[addr+zoomXTable[Xp+j].pos-k-1]	,_Alpha,_DestAlpha);
		...
		{	
			...
			{	
				...
				for (j=0;j<size;j++)
					for (int k=0;k<zoomXTable[j+Xp].size;k++)
						p2SCREEN[addr+zoomXTable[Xp+j].pos-k-1]
							=	FXMIX555(_pPLT[_p8SPRITE[bufferPoint+j]],p2SCREEN[addr+zoomXTable[Xp+j].pos-k-1],_Effect);
