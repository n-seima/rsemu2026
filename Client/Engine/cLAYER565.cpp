		...
		{	XPoint	+=	_lpLAYER[bufferPoint++];
			...
			for (j=beginX;j<size;j++)
				p2SCREEN[addr+XPoint+j]
					=((p2SCREEN[addr+XPoint+j]&d2HM_MASK_565)>>1)+_Color;
...
{	if (_Alpha<=0 ) return;
	...
	_Color	=	GET2BYTEALPHAVALUE565(_Color,_Alpha);
	...
	{	count	=	_lpLAYER[bufferPoint++];
		...
		{	XPoint	+=	_lpLAYER[bufferPoint++];
			...
			for (j=beginX;j<size;j++)
				p2SCREEN[addr+XPoint+j]	=	_Color+GET2BYTEALPHAVALUE565(p2SCREEN[addr+XPoint+j],_DestAlpha);
...
cLAYER565::PutLayerFX()
{
	...
	int red,green,blue;
	red		=	_Color>>dGET_RED_SHIFT565;
	green	=	(_Color>>5)&dGET_GREEN_MASK565;
	blue	=	_Color&31;
	...
	{	count	=	_lpLAYER[bufferPoint++];
		...
		{	XPoint	+=	_lpLAYER[bufferPoint++];
			...
			for (j=beginX;j<size;j++)
				p2SCREEN[addr+XPoint+j]	=	FXMIX565(red,green,blue,p2SCREEN[addr+XPoint+j],_Effect);
...
void
cLAYER565::fPutLayerAB()
{	if (_Alpha<=0 ) return;
	if (_Alpha>=32) cLAYER555::fPutLayer();
	int   X1, Y1, X2, Y2 ,Xs ,Ys;
	int   XPoint=0,i,j,beginX,endX;
	int   count,bufferPoint=8;
	int   size;
	...
	{	count	=	_lpLAYER[bufferPoint++];
		...
		{	XPoint	-=	_lpLAYER[bufferPoint++];
			...
			for (j=0;j<endX;j++)
				p2SCREEN[addr+XPoint-j]	
					=	_Color+GET2BYTEALPHAVALUE565(p2SCREEN[addr+XPoint-j],_DestAlpha);
				...
				for (j=0;j<size;j++)
					for (int k=0;k<zoomXTable[j+Xp].size;k++)
						*(p2SCREEN+addr+zoomXTable[j+Xp].pos+k)
							=((p2SCREEN[addr+zoomXTable[j+Xp].pos+k]&d2HM_MASK_565)>>1)+_Color;
		...
		{	count	=	_lpLAYER[bufferPoint++];	//	이미지 카운트
			...
			{	Xp	+=	_lpLAYER[bufferPoint++];
				...
				for (j=0;j<size;j++)
					for (int k=0;k<zoomXTable[j+Xp].size;k++)
						p2SCREEN[addr+zoomXTable[j+Xp].pos+k]
							=	_Color+GET2BYTEALPHAVALUE565(p2SCREEN[addr+zoomXTable[j+Xp].pos+k],_DestAlpha);
		...
		{	count	=	_lpLAYER[bufferPoint++];	//	이미지 카운트
			...
			{	Xp	+=	_lpLAYER[bufferPoint++];
				...
				for (j=0;j<size;j++)
					for (int k=0;k<zoomXTable[j+Xp].size;k++)
						p2SCREEN[addr+zoomXTable[j+Xp].pos+k]
							=	FXMIX565(red,green,blue,p2SCREEN[addr+zoomXTable[j+Xp].pos+k],_Effect);
				...
				for (j=0;j<size;j++)
					for (int k=0;k<zoomXTable[j+Xp].size;k++)
						p2SCREEN[addr+zoomXTable[Xp+j].pos-k-1]
							=	((p2SCREEN[addr+zoomXTable[Xp+j].pos-k-1]&d2HM_MASK_565)>>1)+_Color;
		...
		{	count	=	_lpLAYER[bufferPoint++];	//	이미지 카운트
			...
			{	Xp	+=	_lpLAYER[bufferPoint++];
				...
				for (j=0;j<size;j++)
					for (int k=0;k<zoomXTable[j+Xp].size;k++)
						p2SCREEN[addr+zoomXTable[Xp+j].pos-k-1]
							=	_Color+GET2BYTEALPHAVALUE565(p2SCREEN[addr+zoomXTable[Xp+j].pos-k-1],_DestAlpha);
		...
		{	count	=	_lpLAYER[bufferPoint++];	//	이미지 카운트
			...
			{	Xp	+=	_lpLAYER[bufferPoint++];
				...
				for (j=0;j<size;j++)
					for (int k=0;k<zoomXTable[j+Xp].size;k++)
						p2SCREEN[addr+zoomXTable[Xp+j].pos-k-1]
							=	FXMIX565(red,green,blue,p2SCREEN[addr+zoomXTable[Xp+j].pos-k-1],_Effect);
