...
cExceptionHandler::addStaticLog(char *_lpstrFormat, ...)
{
	...
	if	(s_iStaticLogPointer+iLength >= dSTATIC_LOG_BUFFER_SIZE)
		return	FALSE;
...
cExceptionHandler::UnhandledExceptionFilter(PEXCEPTION_POINTERS pExceptionInfo )
{
#ifndef	_DEBUG
	{
		...
		tfp	=	fopen(s_strLogFileName,"rb");

		if	(tfp)
		{
			dwSize	= filelength(fileno(tfp));
			fclose(tfp);
		}
