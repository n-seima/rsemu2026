...
{	_Free(m_p16IMAGE);
	...
	else	return g_eh.addStaticLog("Error!! this .TGA File '%d' Bits Format",TGA_HEADER.Bits);
...
{	FILE	*fp;
	...
	if(fp == NULL) return FALSE;//ERRMSG("'%s' file not Found!!");
	...
	if (m_p16IMAGE)	CONVERTIMAGE(m_p16IMAGE);
...
{	if (!overWrite)	if (IsFile(fn)) return FALSE;
	...
	if (!image	) return g_eh.addStaticLog("Error in cTGA::Save - value 'image' are valid!!",fn);

	cFILE	file;

	if (!file.Open(fn,"wb")) return g_eh.addStaticLog("Error in cTGA::Save - '%s' file open(write) error!!",fn);
...
cTGA::Save(char *fn,WORD *image,int width,int height,BOOL overWrite)
{
	...
	if (!image	)
		return g_eh.addStaticLog("Error in cTGA::Save - value 'image' are valid!!",fn);
