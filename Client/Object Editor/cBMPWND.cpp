...
{	
	char	strName[512];
	if	(bGETSHADOW)
		return loadShadow(fname);

	int		iResult	=	dLOAD_IMAGE_FILE_OK;

	void	*temp;
	char	*strExt	=	_exportExt(fname);

	strcpy(strName,fname);
	...
	if	(strExt	==	NULL)
	{
		...
		{
			temp	=	new cTGA();
			sprintf(strName,"%s.tga",fname);
		}
		else
		if	(_BMPWND.curExt	==	2)
		{
			temp	=	new cBMP();
			sprintf(strName,"%s.bmp",fname);
		}
		else
		if	(_BMPWND.curExt	==	3)
		{
	...
	if	(stricmp(_exportExt(fname),"tga")==0)
		temp	=	new cTGA();
	else
	if	(stricmp(_exportExt(fname),"bmp")==0)
		temp	=	new cBMP();
	else
	if	(stricmp(_exportExt(fname),"pcx")==0)
		temp	=	new cPCX();
	else
		return dLOAD_IMAGE_FILE_OPEN_ERROR;

	if	(!((cIMAGE *)temp)->Load(strName))
		return	dLOAD_IMAGE_FILE_OPEN_ERROR;
	if	(((cIMAGE *)temp)->BPP	!=	8)
		return	dLOAD_IMAGE_FILE_NOT_8BIT_IMAGE;
	
//		return ERRMSG("Error in cBMPWND::loadBmp","'%s' 화일이 8bit 이미지 데이터가 아닙니다.\n\n오브젝트 데이터는 8비트 여야 합니다.",fname,cANM::BPP);
	...
	{	
		if (bQUICKOPEN)
		{
			MSGOUT("Error in cBMPWND::loadBmp - '%s' 화일이 %dbit 이미지 데이터가 아닙니다.",fname,cANM::BPP);

			return dLOAD_IMAGE_FILE_NOT_SAME_BPP;
		}
	
		ERRMSG("Error in cBMPWND::loadBmp","'%s' 화일이 %dbit 이미지 데이터가 아닙니다.",fname,cANM::BPP);
			...
			{	if (cANM::PLT[i] != pBMP->pPLT[i])
				{
					...
					iResult	=	dLOAD_IMAGE_FILE_NOT_SAME_PALETTE;
...
{	OPENFILENAME	ofn;
	...
	if (loadBmp(BMPName)	==	dLOAD_IMAGE_FILE_OK) return FALSE;
...
{	int index	=	0,fpos=0,size=strlen(_BMPWND.BMPName),count=0;
	...
	{	index =	0;
		...
		while(str->String)
		{	if (loadBmp(str->String)	==	dLOAD_IMAGE_FILE_OK) totalFile++;
			str	=	str->pNEXT;
		}
		...
		{	char cur[256];
			...
			{	bQUICKOPEN	=	TRUE;
				...
				{	SendMessage( hPROGRESS, PBM_SETPOS, j, 0 );
					...
					if (loadBmp(file->String)	==	dLOAD_IMAGE_FILE_OK)	totalFile++;
...
BOOL
cBMPWND::Program(int image,int shadow,BOOL includeSub,char *srcFolder,char *dest)
{
	int count	=	0;
	...
	BOOL	tempQuick	=	bQUICKOPEN;

	char	*strImageType[]	=	{	"pcx","bmp","tga"};
	char	*strFileType[]	=	{	"rso","rfo"};

	int		iRsoCount	=	0;
	int		iRfoCount	=	0;

	cSTRINGS	*imageFolders;

	char	strImageFolder[1024],strShadowFolder[1024];

	sprintf(strImageFolder,"%s/image",srcFolder);
	sprintf(strShadowFolder,"%s/shadows",srcFolder);

	FILE	*errorOut;

	errorOut	=	fopen("image error.txt","wb");

	includeSub	=	TRUE;
	
	if (includeSub)
	{
		imageFolders	=	cFOLDER::GetFolders(strImageFolder);

		cSTRING	*folder	=	imageFolders->Pick(0);
		...
		{
			char*		strFolderName	=	strstr(folder->String,"image")+6;

			cSTRINGS*	files			=	cFOLDER::GetFiles(_ms("*.%s",strImageType[image]));//	화일들을 얻어 온다.

			cANM::Reset();

			if(strstr(strFolderName,"ani"))		isAnimationObject	=	TRUE;
			else								isAnimationObject	=	FALSE;
			if(strstr(strFolderName,"door"))	isDoorObject		=	TRUE;
			else								isDoorObject		=	FALSE;
			...
			if (files)
			{
				...
				SetCurrentDirectory(srcFolder);

				for (int j=0;j<files->Count;j++)
				{
					char	strImageFileName[512],strShadowFileName[512];

					sprintf(strImageFileName,"image/%s/%s",strFolderName,file->String);
					sprintf(strShadowFileName,"shadows/%s/%s",strFolderName,file->String);

					_setExt(strImageFileName,strImageType[image]);
					_setExt(strShadowFileName,strImageType[shadow]);

					int	iResult	=	loadBmp(strImageFileName);

					if (iResult	==	dLOAD_IMAGE_FILE_OK)
					{	
						totalFile++;

						if (IsFile(strShadowFileName))
						{
							bGETSHADOW	=	TRUE;
							int iResult	=	loadBmp(strShadowFileName);
							
							if (iResult	!=	dLOAD_IMAGE_FILE_OPEN_ERROR)
							{
								iResult++;
							}
							bGETSHADOW	=	FALSE;
						...
						if (!isAnimationObject	&&	!isDoorObject)
						{
							char	strFileName[256];

							SetCurrentDirectory(dest);

							if (strstr(folder->String,"안밟히는것"))
							{
								sprintf(strFileName,"%.3d%s",iRsoCount,file->String);
								_setExt(strFileName,"rso");

								ObjectAttr		=	dOBJECT_NORMAL;
								_cfg.objectType	=	dOBJECT_STATIC;

								if (cANM::SaveSAD(strFileName))	iRsoCount++;
							}
							else
							{
								sprintf(strFileName,"%.3d%s",iRfoCount,file->String);
								_setExt(strFileName,"rfo");

								ObjectAttr		=	dOBJECT_RAPE;
								_cfg.objectType	=	dOBJECT_RANDOM;

								if (cANM::SaveSAD(strFileName))	iRfoCount++;
							}
					...
					else
					{
						switch (iResult)
						{
							case	dLOAD_IMAGE_FILE_FAILED			:
								fprintf(errorOut,"알수 없는 문제 - %s\n",strImageFileName);
								break;

							case	dLOAD_IMAGE_FILE_OPEN_ERROR		:
								fprintf(errorOut,"오픈 에러 - %s\n",strImageFileName);
								break;

							case	dLOAD_IMAGE_FILE_NOT_8BIT_IMAGE	:
								fprintf(errorOut,"8비트 이미지가 아니네 - %s\n",strImageFileName);
								break;

							case	dLOAD_IMAGE_FILE_NOT_SAME_BPP	:
								fprintf(errorOut,"BPP가 틀려요  - %s\n",strImageFileName);
								break;

							case	dLOAD_IMAGE_FILE_NOT_SAME_PALETTE	:
								fprintf(errorOut,"팔레트가 틀려요 - %s\n",strImageFileName);
								break;
						}
					}
				...
				if (isAnimationObject	||	isDoorObject)
				{
					...
					if (isAnimationObject)	g_iObjectType	=	dOBJECT_TYPE_ANM;
					if (isDoorObject	)	g_iObjectType	=	dOBJECT_TYPE_DOOR;


					if (strstr(folder->String,"안밟히는것"))
					{
						if (isDoorObject)	sprintf(strFileName,"%.3d%s",iRsoCount,strstr(strFolderName,"door")+3);
						else				sprintf(strFileName,"%.3d%s",iRsoCount,strstr(strFolderName,"ani")+3);
						_setExt(strFileName,"rso");
					...
					{
						if (isDoorObject)	sprintf(strFileName,"%.3d%s",iRfoCount,strstr(strFolderName,"door")+3);
						else				sprintf(strFileName,"%.3d%s",iRfoCount,strstr(strFolderName,"ani")+3);
	...
	cSTRINGS	*files	=	cFOLDER::GetFiles(_ms("*.%s",strImageType[image]));
	...
	if (files)
	{
		...
		for (int i=0;i<files->Count;i++)
		{
			...
			char	shadowFN[512];
			strcpy(shadowFN,file->String);
			_setExt(shadowFN,strImageType[shadow]);
			...
			strcpy(shadowFN,file->String);
//			_setExt(shadowFN,strFileType[fileType]);
...
//	격자 만들기..
	...
	else	Row	[RowCount++	]=x1,Line[LineCount++]=y1,Row	[RowCount++	]=x2,Line[LineCount++]=y2;
