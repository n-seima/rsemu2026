...
void
cPCX::decode(BYTE *buffer, int bpl, cFILE *file)
{   int index, data, num;

    for(index = 0; index < bpl;) 
	{	data = fgetc(file->fp);

        if (data >= 0xc0)
		{	num	=	data & 0x3f;
			data = fgetc(file->fp);
			while(num--)	buffer[index++] = data;
		}
...
*/
	...
	{	BYTE	*buffer	=	new BYTE [Width * Height * 3];
		...
		decode(buffer, Width * Height * 3, &file);
	...
	{	p8IMAGE	=	new BYTE [Width * Height+4];
		...
		*(WORD *)(p8IMAGE+2	)	=	Height;
		...
		decode(p8IMAGE+4, Width * Height, &file);
