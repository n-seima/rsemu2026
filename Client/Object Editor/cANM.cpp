...
int		ObjectAttr			=	dOBJECT_RAPE;

char	*regANISET			=	"하늘 에니메이션 셋팅 데이터";
char	*regSDHEADER		=	"하늘 스프라이트 데이터";
char	*regSADHEADER		=	"붉은 보석 오브젝트 데이터";
char	*regSADHEADER2		=	"붉은 보석 오브젝트 데이터 2";
int		g_iObjectType		=	dOBJECT_TYPE_NORMAL;
...
int		cIMG::s_iMaxSpriteWidth	;
int		cIMG::s_iMaxSpriteHeight;
int		cIMG::s_iMaxShadowWidth	;
int		cIMG::s_iMaxShadowHeight;
...
cANM::SetAnmType(int type)
{
	...
	for (int i=0;i<dMAX_ANM;i++)
		ANM[i].ClearData(type);
...
{
	if (type != ANM[anm].anmtype) 
		ANM[anm].ClearData(type);
}
...
cANM::SaveAniSet(char *fn)
{
	...
	if (fn==NULL)
		strcpy(FName,_ms("%s.asd",ANMName));
	else
		strcpy(FName,fn);
...
{	FILE *fp;
	...
	fwrite(&CrashBox,sizeof(CrashBox),1,fp);
	fwrite(&g_iObjectType,4,1,fp);
	...
	SADHEADER.byAlpha	=	_WORKWND.Alpha+1;

	SADHEADER.ImageCount=	cIMG::Count;
	SADHEADER.AnmType	=	AnmType;
	SADHEADER.CrashType	=	_cfg.objectType;
	SADHEADER.bOutline	=	bSAVEOUTLINE;
	SADHEADER.bShadow	=	TRUE;
...
{	FILE *fp;
	...
	if (!fp)	return cMSG::Put("스프라이트 에니메이션 화일 오픈 에러!!","'%s' 화일을 찾을 수 없습니다.",fn);
	...
	_WORKWND.Alpha	=	SADHEADER.byAlpha;

	if (SADHEADER.byAlpha	==	0)
			_WORKWND.Alpha	=	dPUT_NORMAL;
	else	_WORKWND.Alpha	--;
	
	AnmType			=	SADHEADER.AnmType;
	_cfg.objectType	=	SADHEADER.CrashType;
	bSAVEOUTLINE	=	SADHEADER.bOutline;
	bSAVESHADOW		=	SADHEADER.bShadow;
	bHALF			=	SADHEADER.bHalf;
	BPP				=	SADHEADER.BPP;

	if (SADHEADER.BPP==8)	fread(PLT,256,2,fp);
	...
	fread(&CrashBox,sizeof(CrashBox),1,fp);
	fread(&g_iObjectType,4,1,fp);

	if (g_iObjectType	<	0	||	g_iObjectType	>	dOBJECT_TYPE_BRIDGE)		g_iObjectType	=	dOBJECT_TYPE_NORMAL;
	...
	if (s_iFPS	>=	32	)	s_iFPS	=	8;
	if (s_iFPS	<=	0	)	s_iFPS	=	8;
