...
public:
	...
	//	프로그램
	static	int			_iImageType,_iShadowType,_iFileType;
	static	BOOL		_isIncludeSubfolder;
	static	char		_strSaveFolder[512];
	static	char		_strImageFolder[512];

	static	void		Program();
	static	void		initPROGRAM(HWND hDlg);
	static	void		launchPROGRAM(HWND hDlg);
	static	LRESULT	CALLBACK	procPROGRAM(HWND,UINT,WPARAM,LPARAM);
