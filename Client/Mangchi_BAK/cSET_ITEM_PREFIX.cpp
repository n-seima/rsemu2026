...
{
	"물리 데미지 1차",	ePOE_PHYSICAL_DAMAGE_1TH,
	"물리 데미지 2차",	ePOE_PHYSICAL_DAMAGE_2TH,
	"물리 데미지 3차",	ePOE_PHYSICAL_DAMAGE_3TH,
	"불 데미지 1차",	ePOE_FIRE_DAMAGE_1TH,
	"불 데미지 2차",	ePOE_FIRE_DAMAGE_2TH,
	"불 데미지 3차",	ePOE_FIRE_DAMAGE_3TH,
	"물 데미지 1차",	ePOE_WATER_DAMAGE_1TH,
	"물 데미지 2차",	ePOE_WATER_DAMAGE_2TH,
	"물 데미지 3차",	ePOE_WATER_DAMAGE_3TH,
	"바람 데미지 1차",	ePOE_WIND_DAMAGE_1TH,
	"바람 데미지 2차",	ePOE_WIND_DAMAGE_2TH,
	"바람 데미지 3차",	ePOE_WIND_DAMAGE_3TH,
	"대지 데미지 1차",	ePOE_EARTH_DAMAGE_1TH,
	"대지 데미지 2차",	ePOE_EARTH_DAMAGE_2TH,
	"대지 데미지 3차",	ePOE_EARTH_DAMAGE_3TH,
	"빛 데미지 1차",	ePOE_LIGHT_DAMAGE_1TH,
	"빛 데미지 2차",	ePOE_LIGHT_DAMAGE_2TH,
	"빛 데미지 3차",	ePOE_LIGHT_DAMAGE_3TH,
	"어둠 데미지 1차",	ePOE_DARK_DAMAGE_1TH,
	"어둠 데미지 2차",	ePOE_DARK_DAMAGE_2TH,
	"어둠 데미지 3차",	ePOE_DARK_DAMAGE_3TH,
	"방어력 1차",		ePOE_DEFENSIVE_POWER_1TH,
	"방어력 2차",		ePOE_DEFENSIVE_POWER_2TH,
	"방어력 3차",		ePOE_DEFENSIVE_POWER_3TH,
	"기본 능력치 1차",	ePOE_BASIC_POWER_1TH,
	"기본 능력치 2차",	ePOE_BASIC_POWER_2TH,
	"기본 능력치 3차",	ePOE_BASIC_POWER_3TH,
	"체력 증가 1차",	ePOE_INCREASE_HP_1TH,
	"체력 증가 2차",	ePOE_INCREASE_HP_2TH,
	"체력 증가 3차",	ePOE_INCREASE_HP_3TH,
	"CP 증가 1차",		ePOE_INCREASE_CP_1TH,
	"CP 증가 2차",		ePOE_INCREASE_CP_2TH,
	"CP 증가 3차",		ePOE_INCREASE_CP_3TH,
	"원소 저항 1차",	ePOE_ELEMENTS_RESISTANCE_1TH,
	"원소 저항 2차",	ePOE_ELEMENTS_RESISTANCE_2TH,
	"원소 저항 3차",	ePOE_ELEMENTS_RESISTANCE_3TH,
	"이상 저항 1차",	ePOE_STRANGE_STATUS_RESISTANCE_1TH,
	"이상 저항 2차",	ePOE_STRANGE_STATUS_RESISTANCE_2TH,
	"이상 저항 3차",	ePOE_STRANGE_STATUS_RESISTANCE_3TH,
	"가시 돋힘 1차",	ePOE_THORN_1TH,
	"가시 돋힘 2차",	ePOE_THORN_2TH,
	"가시 돋힘 3차",	ePOE_THORN_3TH,
	"end",				dCODE_END,
};
...
//	접두사 리스트
	{
		...
		s_listPrefix.AddColumn("식별코드"	,6*11);
		s_listPrefix.AddColumn("1차 접두사"	,6*20);
		s_listPrefix.AddColumn("접두사 타입",6*32);
	}	//	접두사 리스트
...
cSET_ITEM_PREFIX::DrawPrefixInfo(HWND _hWwnd)
{
	...
	cTEXT::Put(iX,iY,WHITE,_ms("        접두사 : %s/%s",lpPrefix->m_str1stPrefix,lpPrefix->m_str2stPrefix));

	if (lpPrefix->m_aValue[0][0])
	{
		if (lpPrefix->m_aValue[0][0]==lpPrefix->m_aValue[0][1])
				cTEXT::Put(iX,iY+=16,WHITE,_ms("         수치1 : %d",lpPrefix->m_aValue[0][0]));
		else	cTEXT::Put(iX,iY+=16,WHITE,_ms("         수치1 : %d~%d",lpPrefix->m_aValue[0][0],lpPrefix->m_aValue[0][1]));
	}

	if (lpPrefix->m_aValue[1][1])
	{
		if (lpPrefix->m_aValue[1][0]==lpPrefix->m_aValue[1][1])
				cTEXT::Put(iX,iY+=16,WHITE,_ms("         수치2 : %d",lpPrefix->m_aValue[1][0]));
		else	cTEXT::Put(iX,iY+=16,WHITE,_ms("         수치2 : %d~%d",lpPrefix->m_aValue[1][0],lpPrefix->m_aValue[1][1]));
	}
	if (lpPrefix->m_sValue3)		cTEXT::Put(iX,iY+=16,WHITE,_ms("         수치3 : %d",lpPrefix->m_sValue3));

	if (lpPrefix->m_wPrefixForm==0)	cTEXT::Put(iX,iY+=16,WHITE,"   접두사 형태 : 명사");
	else							cTEXT::Put(iX,iY+=16,WHITE,"   접두사 형태 : 형용사");
	cTEXT::Put(iX,iY+=16,WHITE,_ms("     드랍 레벨 : %d",lpPrefix->m_wDropLevel));

	char	*strEnable[]	=	{"×","○"};
	cTEXT::Put(iX,iY+=16,WHITE,_ms("생성 가능 위치 : 1차 %s,2차 %s,3차 %s",
	...
	switch(lpPrefix->m_wComputePriceMethod)
	{
		case	0	:
			if (lpPrefix->m_wPriceFactor	==	0)
					cTEXT::Put(iX,iY+=16,WHITE,_ms("          가격 : +%d",lpPrefix->m_dwPrice));
			else	cTEXT::Put(iX,iY+=16,WHITE,_ms("          가격 : *%d",lpPrefix->m_dwPrice));
			break;

		case	1	:
			if (lpPrefix->m_wPriceFactor	==	0)
					cTEXT::Put(iX,iY+=16,WHITE,_ms("          가격 : +[%d~%d]",lpPrefix->m_aValue[0][0],lpPrefix->m_aValue[0][1]));
			else	cTEXT::Put(iX,iY+=16,WHITE,_ms("          가격 : *[%d~%d]",lpPrefix->m_aValue[0][0],lpPrefix->m_aValue[0][1]));
			break;

		case	2	:
			if (lpPrefix->m_wPriceFactor	==	0)
					cTEXT::Put(iX,iY+=16,WHITE,_ms("   가격 계산식 : +[%d~%d]*%d",lpPrefix->m_aValue[0][0],lpPrefix->m_aValue[0][1],lpPrefix->m_dwPrice));
			else	cTEXT::Put(iX,iY+=16,WHITE,_ms("   가격 계산식 : *[%d~%d]*%d",lpPrefix->m_aValue[0][0],lpPrefix->m_aValue[0][1],lpPrefix->m_dwPrice));
			break;
		...
		case	4	:
			if (lpPrefix->m_wPriceFactor	==	0)
					cTEXT::Put(iX,iY+=16,WHITE,_ms("   가격 계산식 : +[%d~%d]*%d/100",lpPrefix->m_aValue[0][0],lpPrefix->m_aValue[0][1],lpPrefix->m_dwPrice));
			else	cTEXT::Put(iX,iY+=16,WHITE,_ms("   가격 계산식 : *[%d~%d]*%d/100",lpPrefix->m_aValue[0][0],lpPrefix->m_aValue[0][1],lpPrefix->m_dwPrice));
			break;

		case	5	:
			if (lpPrefix->m_wPriceFactor	==	0)
					cTEXT::Put(iX,iY+=16,WHITE,_ms("   가격 계산식 : +%d*[%d~%d]/[%d~%d]",lpPrefix->m_dwPrice,lpPrefix->m_aValue[1][0],lpPrefix->m_aValue[1][1],lpPrefix->m_dwPrice,lpPrefix->m_aValue[0][0],lpPrefix->m_aValue[0][1]));
			else	cTEXT::Put(iX,iY+=16,WHITE,_ms("   가격 계산식 : *%d*[%d~%d]/[%d~%d]",lpPrefix->m_dwPrice,lpPrefix->m_aValue[1][0],lpPrefix->m_aValue[1][1],lpPrefix->m_dwPrice,lpPrefix->m_aValue[0][0],lpPrefix->m_aValue[0][1]));
			break;
...
cSET_ITEM_PREFIX::DecodeEffectScriptFile(HWND _hDlg)
{
	...
	if (!cFILE::LoadBox(strFn,"텍스트 화일 {*.txt}\0*.txt\0\0","txt",NULL,"접두사 스크립트 데이터 화일 불러오기"))
	{
		SetLogFolder();
		return	FALSE;
	}
...
cSET_ITEM_PREFIX::DecodeEffectScriptFile(char *_strFile)
{
	...
	while(1)
	{
		...
		{	
			//	식별 코드/접두사 중복 확인
			{
				...
				{
					ERRMSG("식별 코드 혹은 접두사가 중복 됩니다!!","%s/%s\n\n식별 코드 %d",lpPrefix->m_str1stPrefix,lpPrefix->m_str2stPrefix,lpPrefix->m_wDiscernmentCode);
					iOverlappedCount++;
				}
			}	//	식별 코드/접두사 중복 확인
		}
	...
	cMSG::Put("데이터 화일 분석 결과","%d개의 접두사를 추가 했습니다.\n\n%d개의 접두사가 중복 되었습니다.",g_itemPrefix.getCount()-iCount,iOverlappedCount);
...
cSET_ITEM_PREFIX::DecodeFile(HWND _hDlg)
{
	...
	if (g_itemPrefix.getCount() > 0)
		if (cMSG::YESNO(_ms("'%s' 화일을 디코딩 합니다.",strFn),"디코딩 하기 전에 기존의 접두사 데이터를 초기화 시키시겠습니까?")==IDYES)
			g_itemPrefix.reset();
...
cSET_ITEM_PREFIX::DecodeScript(char *_strScript)
{
	...
	{	
		...
		if (iGrade	==	1)
		{
			if	(strstr(lpStream,"딜럭스 접두사"))
				s_makePrefix.m_wIsDXPrefix		=	TRUE;
			if	(strstr(lpStream,"얼티미트 접두사"))
		...
		switch(iIndex)
		{
			case	0	:
			{
				...
				if (iPrefixType	==	0xffff)
					return	cMSG::Error("얼래? 등록되지 않은 접두사 타입인디... -_-a",
								"'%s'\n\n제작자-_-a한테 문의 하세요~",lpStream);
			...
			case	7	:
				if (stricmp("명사",lpStream)==0)	s_makePrefix.m_wPrefixForm		=	0;
				else								s_makePrefix.m_wPrefixForm		=	1;
				break;

//			case	8	:
//				cMSG::Put("형태",lpStream);
//				break;
...
//	무기 효과 리스트
	{
		...
		cCOMBOBOX::Add(hComboWEL,"효과 없음",0xff);
...
//	방어구 효과 리스트
	{
		...
		cCOMBOBOX::Add(hComboAEL,"효과 없음",0xff);
...
cSET_ITEM_PREFIX::DecodeScript(HWND _hDlg)
{
	...
	{	
		...
		switch(iIndex)
		{
			...
			case	7	:
				if (stricmp("명사",lpStream)==0)	CheckRadioButton(_hDlg,IDC_NOUN,IDC_ADJECTIVE,IDC_NOUN);
