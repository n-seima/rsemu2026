...
//	착용 이미지
	...
	if (s_lpSelectItem->m_wEquippedShape	!=	0xffff)
		cTEXT::Put(x+20,y+=16,WHITE,_ms("착용 이미지 : %s",g_strViewEquipmentName[s_lpSelectItem->m_wKind][s_lpSelectItem->m_wEquippedShape]));
...
//	드랍 이미지
	...
	if (s_lpSelectItem->m_aValue[0][1])
	{
		if (s_lpSelectItem->m_aValue[0][0]==s_lpSelectItem->m_aValue[0][1])	
				cTEXT::Put(x,y+=16,WHITE,_ms("생성 수치 1 : %d",s_lpSelectItem->m_aValue[0][0]));
		else	cTEXT::Put(x,y+=16,WHITE,_ms("생성 수치 1 : %d~%d",s_lpSelectItem->m_aValue[0][0],s_lpSelectItem->m_aValue[0][1]));
	}
	if (s_lpSelectItem->m_aValue[1][1])
	{
		if (s_lpSelectItem->m_aValue[1][0]==s_lpSelectItem->m_aValue[1][1])	
				cTEXT::Put(x,y+=16,WHITE,_ms("생성 수치 2 : %d",s_lpSelectItem->m_aValue[1][0]));
		else	cTEXT::Put(x,y+=16,WHITE,_ms("생성 수치 2 : %d~%d",s_lpSelectItem->m_aValue[1][0],s_lpSelectItem->m_aValue[1][1]));
	}

	if (s_lpSelectItem->isDamageItem())
	{
		cTEXT::Put(x,y+=16,WHITE,_ms("최소 데미지 : %d",s_lpSelectItem->m_wMinDamage));
		cTEXT::Put(x,y+=16,WHITE,_ms("최고 데미지 : %d",s_lpSelectItem->m_wMaxDamage));
	}

	if(s_lpSelectItem->m_wSpeed)
		cTEXT::Put(x,y+=16,WHITE,_ms("  공격 속도 : %d.%.2d",s_lpSelectItem->m_wSpeed/100,s_lpSelectItem->m_wSpeed%100));

	if(s_lpSelectItem->m_wRange)
		cTEXT::Put(x,y+=16,WHITE,_ms("  사정 거리 : %d Pixel",s_lpSelectItem->m_wRange));

	if(s_lpSelectItem->m_wDamageRange)
		cTEXT::Put(x,y+=16,WHITE,_ms("데미지 범위 : %d Pixel",s_lpSelectItem->m_wDamageRange));

	switch(s_lpSelectItem->m_wPriceComputeMethod)
	{
		case	0	:
			cTEXT::Put(x,y+=16,WHITE,_ms("       가격 : %d",s_lpSelectItem->m_dwPrice));
			break;
		case	1	:
			if (s_lpSelectItem->m_aValue[0][0]==s_lpSelectItem->m_aValue[0][1])
					cTEXT::Put(x,y+=16,WHITE,_ms("       가격 : %d",s_lpSelectItem->m_aValue[0][0]));
			else	cTEXT::Put(x,y+=16,WHITE,_ms("       가격 : %d~%d",s_lpSelectItem->m_aValue[0][0],s_lpSelectItem->m_aValue[0][1]));
			break;
		case	2	:
			if (s_lpSelectItem->m_aValue[0][0]==s_lpSelectItem->m_aValue[0][1])
					cTEXT::Put(x,y+=16,WHITE,_ms("       가격 : %d*%d",s_lpSelectItem->m_dwPrice,s_lpSelectItem->m_aValue[0][0]));
			else	cTEXT::Put(x,y+=16,WHITE,_ms("       가격 : %d*[%d~%d]",s_lpSelectItem->m_dwPrice,s_lpSelectItem->m_aValue[0][0],s_lpSelectItem->m_aValue[0][1]));
			break;
		case	3	:
			if (s_lpSelectItem->m_aValue[1][0]==s_lpSelectItem->m_aValue[1][1])
					cTEXT::Put(x,y+=16,WHITE,_ms("       가격 : %d*%d",s_lpSelectItem->m_dwPrice,s_lpSelectItem->m_aValue[1][0]));
			else	cTEXT::Put(x,y+=16,WHITE,_ms("       가격 : %d*[%d~%d]",s_lpSelectItem->m_dwPrice,s_lpSelectItem->m_aValue[1][0],s_lpSelectItem->m_aValue[1][1]));
			break;
		case	4	:
			if (s_lpSelectItem->m_aValue[0][0]==s_lpSelectItem->m_aValue[0][1])
					cTEXT::Put(x,y+=16,WHITE,_ms("       가격 : %d*(%d+1)",s_lpSelectItem->m_dwPrice,s_lpSelectItem->m_aValue[0][0]));
			else	cTEXT::Put(x,y+=16,WHITE,_ms("       가격 : %d*([%d~%d]+1)",s_lpSelectItem->m_dwPrice,s_lpSelectItem->m_aValue[0][0],s_lpSelectItem->m_aValue[0][1]));
			break;
		...
		case	5	:
		{
			...
			cTEXT::Put(x,y+=16,WHITE,_ms("       가격 : %s*%s/%d",strValue1,strValue2,s_lpSelectItem->m_dwPrice));
			break;
		}

		case	6	:
			cTEXT::Put(x,y+=16,WHITE,_ms("       가격 : 10개에 %d원",s_lpSelectItem->m_dwPrice));
			break;
	...
	cTEXT::Put(x,y+=16,WHITE,_ms("  드랍 레벨 : %d",s_lpSelectItem->m_wDropLevel));

	cTEXT::Put(x,y+=16,WHITE,_ms("  스택 제한 : %d개",s_lpSelectItem->m_wStackLimit));
	cTEXT::Put(x,y+=16,WHITE,_ms("     내구력 : %d",s_lpSelectItem->m_wDurability));
	cTEXT::Put(x,y+=16,WHITE, _ms("  현재 단계 : %d", s_lpSelectItem->m_bf4EternalItemCurrentLevel));		//  현재 단계 09.08.25
	cTEXT::Put(x,y+=16, WHITE, _ms("  다음단계 아이템 시리얼 : %d", s_lpSelectItem->m_wNextEternalItemSerial));// 다음단계아이템시리얼 09.08.25
	cTEXT::Put(x,y+=16, WHITE, _ms("  강화패널티아이템 시리얼 : %d", s_lpSelectItem->m_wUpgradePenaltyItemSerial));// 강화패널티아이템시리얼 09.08.26
	x-=20;
	...
	{
		cTEXT::SetBoldFont();
		cTEXT::Put(x,y,LTYELLOW,"━━ 아이템 생성 정보");
		cTEXT::SetNormalFont();
	...
	{
		iLimitCount++;
		cTEXT::Put(x,y+=16,WHITE,_ms("성향제한 : %s성향 캐릭터만 착용/사용 가능",g_strAllignment[s_lpSelectItem->m_wRequireAllignment]));
	}

	if (s_lpSelectItem->m_wRequireLevel)
	{
		if(s_lpSelectItem->m_wRequireMaxLevel)
			cTEXT::Put(x,y+=16,WHITE,_ms("    레벨 : %d ~ %d", s_lpSelectItem->m_wRequireLevel,s_lpSelectItem->m_wRequireMaxLevel));//	사용/장비를 위한 최소 레벨
		else
			cTEXT::Put(x,y+=16,WHITE,_ms("    레벨 : %d",s_lpSelectItem->m_wRequireLevel));//	사용/장비를 위한 최소 레벨
		iLimitCount++;
	}
	...
	{
		cTEXT::Put(x,y+=16,WHITE,_ms("      힘 : %d",s_lpSelectItem->m_wRequireStrength));//	사용/장비를 위한 최소 힘
		iLimitCount++;
	}
	if (s_lpSelectItem->m_wRequireDexterity)
	{
		cTEXT::Put(x,y+=16,WHITE,_ms("  민첩성 : %d",s_lpSelectItem->m_wRequireDexterity));//	사용/장비를 위한 최소 민첩성
		iLimitCount++;
	}
	if (s_lpSelectItem->m_wRequireConstitution)
	{
		cTEXT::Put(x,y+=16,WHITE,_ms("    건강 : %d",s_lpSelectItem->m_wRequireConstitution));//	사용/장비를 위한 최소 체질
		iLimitCount++;
	}
	if (s_lpSelectItem->m_wRequireWisdom)
	{
		cTEXT::Put(x,y+=16,WHITE,_ms("    지혜 : %d",s_lpSelectItem->m_wRequireWisdom));//	사용/장비를 위한 최소 지혜
		iLimitCount++;
	}
	if (s_lpSelectItem->m_wRequireIntelligence)
	{
		cTEXT::Put(x,y+=16,WHITE,_ms("    지식 : %d",s_lpSelectItem->m_wRequireIntelligence));//	사용/장비를 위한 최소 지식
		iLimitCount++;
	}
	if (s_lpSelectItem->m_wRequireCharisma)
	{	
		cTEXT::Put(x,y+=16,WHITE,_ms("카리스마 : %d",s_lpSelectItem->m_wRequireCharisma));//	사용/장비를 위한 최소 카리스마
		iLimitCount++;
	}
	if (s_lpSelectItem->m_wRequireLuck)
	{	
		cTEXT::Put(x,y+=16,WHITE,_ms("      운 : %d",s_lpSelectItem->m_wRequireLuck));//	사용/장비를 위한 최소 운
		iLimitCount++;
	}
	...
	x-=20;
	cTEXT::SetBoldFont();
	cTEXT::Put(x,y+=24, LTYELLOW, "━━ 아이템 착용 갯수 제한");
	cTEXT::SetNormalFont();
	x+=20;
...
cSET_BASIC_ITEM::DecodeItemScriptFile(HWND _hDlg)
{
	...
	if (!cFILE::LoadBox(strFn,"텍스트 화일 {*.txt}\0*.txt\0\0","txt",NULL,"아이템 스크립트 데이터 화일 불러오기"))
	{
		SetLogFolder();
		return	FALSE;
	}
	...
	if (cBASIC_ITEM::s_iCount > 0)
		if (cMSG::YESNO(_ms("'%s' 화일을 디코딩 합니다.",strFn),"디코딩 하기 전에 기존의 아이템 데이터를 초기화 시키시겠습니까?")==IDYES)
			cBASIC_ITEM::Reset();
...
//	s_makeItem.m_wEquippedShape;						//	착용하고 있을때의 모양
	...
	SetDlgItemText(hwnd,IDC_SPEED,_ms("%d.%.2d",s_makeItem.m_wSpeed/100,s_makeItem.m_wSpeed%100));	//	최소 데미지
	...
	SetDlgItemText(hwnd,IDC_CORRECT_DROP_CHANCE,_ms("%d.%.1d",s_makeItem.m_wCorrectDropChance/10,s_makeItem.m_wCorrectDropChance%10));	//	최소 데미지
...
//	아이템 분류
	...
	if	(s_makeItem.m_wStackLimit	>	255)
	{
		cMSG::Put("에러","아이템 최대 스택 개수는 255개입니다.\n\n강제로 스택 제한을 255로 설정합니다.");
		s_makeItem.m_wStackLimit	=	255;
	}
...
cSET_BASIC_ITEM::CopyItemData(HWND _hDlg)
{
	...
	if (!lpBasicItem)
		return	ERRMSG("에러!","'%s'라는 이름의 아이템은 없는데요",strItem);
...
cSET_BASIC_ITEM::CopyBaseItemData(HWND _hDlg)
{
	...
	if (iBaseItem	==	0xffff)
		return	cMSG::Put("에러!","먼저 아이템을 선택 하셔요.");
...
cSET_BASIC_ITEM::SetCheckDataList(HWND _hDlg)					// 09.08.07
{
	...
	if(s_makeItem.m_attr.isGuildBattleItem){
		cLISTBOX::Add(hListBox,"길드전");
	}

	if(s_makeItem.m_attr.isBeginnerItem){
		cLISTBOX::Add(hListBox,"초보자용");
	}
	...
	if(s_makeItem.m_attr.isBadge){
		cLISTBOX::Add(hListBox,"배지");
	}

	if(s_makeItem.m_bf1IsEternalItem){		// 09.08.25
		cLISTBOX::Add(hListBox,"이터널");
	}

	if(s_makeItem.m_attr.isCanNotTradeItem){
		cLISTBOX::Add(hListBox,"거래불가");
	}

	if(s_makeItem.m_attr.isCanNotDropItem){
		cLISTBOX::Add(hListBox,"버리기 불가");
	}
	
	if(s_makeItem.m_attr.isCanNotDestroyItem){
		cLISTBOX::Add(hListBox,"파괴 불가");
	}
	
	if(s_makeItem.m_attr.isCanNotUseableItem){
		cLISTBOX::Add(hListBox,"사용불가");
	}
	
	if(s_makeItem.m_attr.isCanNotStoreBank){
		cLISTBOX::Add(hListBox,"은행 보관 불가");
	}

	if(s_makeItem.m_attr.isCanNotSelllItem){
		cLISTBOX::Add(hListBox,"상점 판매 불가");
	}

	if(s_makeItem.m_bf1IsBlockToFeedPet){
		cLISTBOX::Add(hListBox,"펫 먹이 불가");
	}
								
	if(s_makeItem.m_attr.isUseAbleBulletSubstituteArrow){
		cLISTBOX::Add(hListBox,"탄환을 화살 대신 사용");
	}
	
	if(s_makeItem.m_attr.isDurabilityToUseCount){
		cLISTBOX::Add(hListBox,"내구력 -> 사용횟수");
	}
	
	if(s_makeItem.m_attr.isUseToItem){
		cLISTBOX::Add(hListBox,"아이템에 사용");
	}
	
	if(s_makeItem.m_attr.isCanUseCorpseStatus){
		cLISTBOX::Add(hListBox,"죽은후에 사용");
	}

	if(s_makeItem.m_attr.isUseToCorpse){
		cLISTBOX::Add(hListBox,"시체에게 사용");
	}
	
	if(s_makeItem.m_attr.isUseToCharacter){
		cLISTBOX::Add(hListBox,"케릭터에게 사용");
	}
	
	if(s_makeItem.m_attr.isUseToMiniPet){
		cLISTBOX::Add(hListBox,"미니펫에게 사용");
	}
	
	if(s_makeItem.m_bf1IsUseOnlyGuildMaster){
		cLISTBOX::Add(hListBox,"길마만 사용 가능");
	}
								
	if(s_makeItem.m_bf1IsBuyOnlyGuildMaster){
		cLISTBOX::Add(hListBox,"길마만 구입 가능");
	}							

	if(s_makeItem.m_bf1IsItemNotUsePet){				// 09.09.04
		cLISTBOX::Add(hListBox,"펫/소환수 사용불가");
	}	
	
	
	if(s_makeItem.m_attr.isEquipableToBelt){
		cLISTBOX::Add(hListBox,"벨트에 착용 가능");
	}								
	if(s_makeItem.m_attr.isEffectInInventoryItem){
		cLISTBOX::Add(hListBox,"인벤토리에서 효과발휘");
	}
	if(s_makeItem.m_bf1IsDestroyWhenMoveField){
		cLISTBOX::Add(hListBox,"필드 이동시 파기");
	}							
	if(s_makeItem.m_attr.isCanNotOwnSameItem){
		cLISTBOX::Add(hListBox,"같은 아이템 무시");
	}							
	if(s_makeItem.m_bIsExceptionItem){
		cLISTBOX::Add(hListBox,"예외 아이템");
	}									
	
	if(s_makeItem.m_bf1IsOccurEatPotionEffect){
		cLISTBOX::Add(hListBox,"포션 먹기 효과 발생");
	}									

	if(s_makeItem.m_attr.isRequireSaveUseLog){
		cLISTBOX::Add(hListBox,"사용 로그 남김");
	}							
	if(s_makeItem.m_bf1IsRequireLogWithGuildInfo){
		cLISTBOX::Add(hListBox,"로그에 길드 정보 추가");
	}

	if(s_makeItem.m_bf1IsUseAbleByThrowPotionSkill){
		cLISTBOX::Add(hListBox,"물약");
	}						
	if(s_makeItem.m_bf1IsUseAbleByThrowDrugSkill){
		cLISTBOX::Add(hListBox,"치료약");
	}						
	if(s_makeItem.m_bf1IsUseAbleByThrowFlowerSkill){
		cLISTBOX::Add(hListBox,"꽃");
	}
	if(s_makeItem.m_bf1IsUseAbleByThrowCandySkill){
		cLISTBOX::Add(hListBox,"사탕, 과자, 케잌");
	}

	if(s_makeItem.m_attr.isMagicCarpetEmblem){
		cLISTBOX::Add(hListBox,"양탄자 엠블렘");
	}
	if(s_makeItem.m_attr.isMergeToCarpetEmblem){
		cLISTBOX::Add(hListBox,"사용시 양탄자 엠블럼에 병합");
	}
	if(s_makeItem.m_bf1IsRequestSummonCarpet){
		cLISTBOX::Add(hListBox,"양탄자 소환 의뢰");
	}
	if(s_makeItem.m_bf1IsCanSummonMagicCarpet){
		cLISTBOX::Add(hListBox,"양탄자 소환 가능");
	}
	if(s_makeItem.m_bf1RemeberPlace0){
		cLISTBOX::Add(hListBox,"장소기억");
	}
	if(s_makeItem.m_bf1RemeberPlace1){
		cLISTBOX::Add(hListBox,"장소기억1");
	}
	if(s_makeItem.m_bf1RemeberPlace2){
		cLISTBOX::Add(hListBox,"장소기억8");
	}
	if(s_makeItem.m_bf1IsFreePastPortal){
		cLISTBOX::Add(hListBox,"무료 패스트 포탈");
	}
	if(s_makeItem.m_bf1IsHalfPastPortal){
		cLISTBOX::Add(hListBox,"반값 패스트 포탈");
	}
	if(s_makeItem.m_bf1IsFreeTeleport){
		cLISTBOX::Add(hListBox,"무료 타운 텔레포트");
	}
	if(s_makeItem.m_bf1IsAddGetMysticStone){
		cLISTBOX::Add(hListBox,"신비석 추가 획득");
	}
	if(s_makeItem.m_bf1IsAddGetTantalissRelic){
		cLISTBOX::Add(hListBox,"탄틸리스 유물 추가 획득");
	}
	if(s_makeItem.m_bf1IsIgnoreGuildHallLevelForEntryTantalisExile){
		cLISTBOX::Add(hListBox,"제한없이 길드던전 입장");
	}
	if(s_makeItem.m_bf1IsInfinitySeriousUpgradeChanceUp){
		cLISTBOX::Add(hListBox,"이터널 시리즈 진화 확률 상승");
	}

	if(s_makeItem.m_bf1EternalItemUpgradeNotPenalty){		// 09.08.25
		cLISTBOX::Add(hListBox,"이터널아이템 강화 패널티 제거");
	}
	if(s_makeItem.m_bf1IsUpgradeMaterials){		// 09.08.26
		cLISTBOX::Add(hListBox,"강화제");	
	}
	if(s_makeItem.m_bf1IsUseShop){		// 상점 이용하기
		cLISTBOX::Add(hListBox,"상점 이용하기");	
	}
	if(s_makeItem.m_bf1IsUseBank){		// 은행 이용하기
		cLISTBOX::Add(hListBox,"은행 이용하기");	
	}

	if(s_makeItem.m_bf1IsNotConsume)		// "사용시 소모되지 않음"
		cLISTBOX::Add(hListBox,"사용시 소모되지 않음");

	if(s_makeItem.m_bf1IsUseItemAfterTransformation)		// 몬스터 변신후 사용
		cLISTBOX::Add(hListBox,"몬스터 변신후 사용");

	if(s_makeItem.m_bf1IsRingOfInfinity)			// 무한의 반지
		cLISTBOX::Add(hListBox,"무한의 반지");

	if(s_makeItem.m_bf1IsUseToEnemy)			// 적에게 사용
		cLISTBOX::Add(hListBox,"적에게 사용");

	if(s_makeItem.m_bf1IsUseAfterWear)			// 착용 후 기능 사용
		cLISTBOX::Add(hListBox,"착용 후 기능 사용");

	if(s_makeItem.m_bf1IsUsePremiumInventory)			// 유료아이템 전용 인벤토리 사용
		cLISTBOX::Add(hListBox,"유료아이템 전용 인벤토리 사용");

	if(s_makeItem.m_bf1IsNotApplyCopyEffect)			// 복사 효과 적용안됨
		cLISTBOX::Add(hListBox,"복사 효과 적용안됨");

	if(s_makeItem.m_bf1IsNotApplyReversionEffect)			// 거래불가해제 효과 적용안됨
		cLISTBOX::Add(hListBox,"거래불가해제 효과 적용안됨");

	if(s_makeItem.m_bf1IsNotGetUsedSkill)			//스킬로 줍기 불가
		cLISTBOX::Add(hListBox,"스킬로 줍기 불가");

	if(s_makeItem.m_bf1IsNotUseToGVG)			// GVG에서 사용불가
		cLISTBOX::Add(hListBox,"GVG에서 사용불가");
