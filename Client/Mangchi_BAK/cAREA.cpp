		...
		switch(m_wKind)
		{
			...
			case	dAREA_PORTAL		:
			{
				...
				if	(m_pString)
				{
					...
					// 09.12.18
					cTEXT::Put(rect.x1+5,rect.y1,cSET_FONT::s_OldColorOption[eCOLOR_PORTAL_EXIT]		,"출구 ");
				...
				if (m_gateShape	==	ePAS_HIDDEN)
				{
					rect.y1	+=	16;
					// 09.12.18
					cTEXT::Put(rect.x1+5,rect.y1,cSET_FONT::s_OldColorOption[eCOLOR_PORTAL_HIDDEN],"감춰진 포탈");
				...
				if (m_gateShape	==	ePAS_HIDDEN_INNER_PORTAL)
				{
					rect.y1	+=	16;
					// 09.12.18
					cTEXT::Put(rect.x1+5,rect.y1,cSET_FONT::s_OldColorOption[eCOLOR_PORTAL_HIDDEN_IN],"감춰진 내부 이동 포탈");
				...
				if	(m_gateShape	==	ePAS_ONE_WAY_GATE)
				{
					rect.y1	+=	16;
					// 09.12.18
					cTEXT::Put(rect.x1+5,rect.y1,cSET_FONT::s_OldColorOption[eCOLOR_PORTAL_ONE_WAY],"일방 통행 포탈");
			...
			case	dAREA_TRAP			:
				// 09.12.18
				cTEXT::Put(rect.x1+5,rect.y1+3 ,cSET_FONT::s_OldColorOption[eCOLOR_AREA_NAME],m_strName);
				cTEXT::Put(rect.x1+5,rect.y1+15 ,cSET_FONT::s_OldColorOption[eCOLOR_AREA_TRAP],"함정");
				//cTEXT::Put(rect.x1+5,rect.y1+3 ,WHITE,m_strName);
			...
			case	dAREA_EVENT_AREA	:
			{
				// 09.12.18
				cTEXT::Put(rect.x1+5,rect.y1+3 ,cSET_FONT::s_OldColorOption[eCOLOR_AREA_NAME],m_strName);
				cTEXT::Put(rect.x1+5,rect.y1+15 ,cSET_FONT::s_OldColorOption[eCOLOR_EVENT_AREA],"이벤트 영역");
				//cTEXT::Put(rect.x1+5,rect.y1+3 ,WHITE,m_strName);
			...
			case	dAREA_ARCA			:
			{
				...
				if (lpArca == NULL)
					cTEXT::Put(rect.x1+5,rect.y1+15 ,cSET_FONT::s_OldColorOption[eCOLOR_ARCA],"보물 상자 : 링크 문제 있음");
					//cTEXT::Put(rect.x1+5,rect.y1+15 ,LTYELLOW,"보물 상자 : 링크 문제 있음");
				else
					cTEXT::Put(rect.x1+5,rect.y1+15 ,cSET_FONT::s_OldColorOption[eCOLOR_ARCA],_ms("보물 상자 : %s",lpArca->m_strName));
...
cAREA::checkTwoWayLinkGate(char *_lpstrMapname)
{
	...
	for (int i=0;i<dMAX_AREA;i++)
	{
		...
		{
			int	iResult	=	cMSG::YESNO("계속 진행 하시겠습니까?","다음 영역들이 쌍방향 링크가 아닙니다.\n\n필드 '%s' - '%s'\n필드 '%s' - '%s'",_lpstrMapname,lpArea->m_strName,lpArea->m_pString,lpArea->m_strMoveGateName);
