...
static	char *strToolbar[]	=
  {
  		"새로운 맵 추가","현재 작업중인 맵 초기화",
  		"",
  		"불러오기","맵 추가","저장하기","작업중인 맵 모두 저장",
  		"",
  		"건물 레이어","오브젝트 레이어","유닛 레이어","영역및 기타정보","문 레이어",
  		"",
  		"지나가지 못하는 영역 표시","격자 표시","마우스로 화면 스크롤","타일 인덱스 표시","그리기 금지",
  		"",
  		"맵 실행",
  		"",
  		"이동 불가 타일 설정",
...
{	// 마지막 수정일 : 09.10.30	
	...
	if	(!file.Open("mangchi.cfg","rb"))	
		return ERRMSG("mangchi.cfg 화일이 없어요!!");
...
cMAIN::linkAllFieldsGate(HWND hwnd,BOOL _bIsSave)
{
	...
	for (int i=0;i<lpFiles->Count;i++)
	{
		...
		{
			KILL(lpFiles);
			return	ERRMSG("에러!!","출구 영역 연결이 제대로 안되어 있습니다.");
		}
...
cMAIN::compareFields(HWND hwnd)
{
	...
	if (!IsFolder(strFolder1))	return	ERRMSG("에러!!","%s 폴더를 찾을 수 없습니다.",strFolder1);
	if (!IsFolder(strFolder2))	return	ERRMSG("에러!!","%s 폴더를 찾을 수 없습니다.",strFolder2);
	
	SetCurrentDirectory(strFolder1);
	
	cSTRINGS	*lpFiles	=	cFOLDER::GetFiles("*.rmd");
	
	if (!lpFiles)	return	ERRMSG("에러!!","맵이 하나도 없어요.  -ㅐ-");
	...
	for (int i=0;i<lpFiles->Count;i++)
	{
		...
		{
			ERRMSG("에러!!","'%s'의 필드 사이즈가 다릅니다.",lpFile->String);
		...
		for (int iY=0;iY<map1Size.y;iY++)
			for (int iX=0;iX<map1Size.x;iX++)
			{
				...
				{
					ERRMSG("에러!!","'%s'의 %d,%d의 타일 속성이 다릅니다.",lpFile->String,iX,iY);
