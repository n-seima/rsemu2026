...
cSetBook::InitData(HWND _hDlg)
{
	...
	//	바닥에 딸어지는 아이템 데이터/리스트
	{
		...
		s_listBook.AddColumn("이름"				,6*39	,eLCOM_CENTER);
	}

	{
		s_listCategory.Init		(hCategoryList	,4		,FALSE	,ProcBookCategoryList);		//	2개의 컬럼
		s_listCategory.AddColumn("Index"		,6*6	,eLCOM_CENTER);
		s_listCategory.AddColumn("이름"			,6*46	,eLCOM_CENTER);
		s_listCategory.AddColumn("수치"			,6*6	,eLCOM_CENTER);
...

	{
		...
		s_listPage.AddColumn("내용"				,6*21	,eLCOM_NORMAL);
		s_listPage.AddColumn("수치"				,6*6	,eLCOM_CENTER);
...
cSetBook::ApplyItemComment(HWND hDlg)
{
	...
	if	(strText	<=	0)
	{
		cMSG::Error("error!","책 아이템 설명을 입력하세요.");
		return;
	}

	if	(!g_book.setBookItemComment(s_iSelectBook,strText))
	{
		cMSG::Error("error!","먼저 책을 선택해야 합니다.");
		return;
	}
...
cSetBook::AddCategory(HWND hDlg)
{
	...
	if	(!lpBook)
	{
		cMSG::Error("error!","먼저 책을 선택해 주세요.");
		return;
	}
	...
	if	(bTemp	==	FALSE)
	{
		cMSG::Error("error!","관련 수치를 입력해 주세요.");
		return;
	}

	if	(strlen(strCategoryName) ==	0)
	{
		cMSG::Error("error!","카테고리 이름을 입력해 주세요.");
		return;
	}

	if	(!lpBook->addCategory(strCategoryName,bIsDefaultCategory,iValue))
	{
		cMSG::Error("error!","더 이상의 카테고리를 추가 할 수 없습니다.");
		return;
	}
...
cSetBook::ModifyCategoryName(HWND hDlg)
{
	...
	{
		cMSG::Error("error!","먼저 카테고리를 선택해 주세요.");
		return;
	}
...
cSetBook::RemoveCategory(HWND hDlg)
{
	...
	{
		cMSG::Error("error!","먼저 카테고리를 선택해야 합니다.");
		return;
	}
...
cSetBook::AddPage(HWND hDlg,BOOL _bIsInsert)
{
	...
	{
		cMSG::Error("error!","페이지 내용을 입력해 주세요.");
		return;
	}
	...
	{
		cMSG::Error("error!","더 이상의 페이지를 추가 할 수 없습니다.");
...
cSetBook::ModifyPage(HWND hDlg)
{
	...
	{
		cMSG::Error("error!","먼저 페이지를 선택해 주세요.");
		return;
	}
