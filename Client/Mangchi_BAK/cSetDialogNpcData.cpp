...
cSetDialogNpcData::DrawNpcInfo()
{
	...
	cTEXT::Put(5,150,WHITE, "행동 영역 안에서 생성 : ");
	cTEXT::Put(5,165,WHITE, "자동 리젠 안됨 : ");
	cTEXT::Put(5,180,WHITE, "이따끔 방향을 바꿈 : ");	
...
cSetDialogNpcData::RemoveSpeech(HWND _hDlg)
{
	...
	if(MessageBox(_hDlg, "선택된 캐릭터의 모든 대화가 삭제됩니다. 그래도 삭제하시겠습니까?", "대화 삭제", MB_YESNO) == IDYES)
	{
		int selectlist = 0;
		s_iSelOldList == ePAGE_LIST ? selectlist = s_iSelectNpcData : selectlist = s_iSelectFindNpcData;
...
cSetDialogNpcData::RemoveAllSpeech(HWND _hDlg)
{
	...
	if(MessageBox(_hDlg, "모든 캐릭터의 대화가 삭제됩니다. 삭제하시겠습니까?", "모든 대화 삭제", MB_YESNO) == IDYES)
	{
		cACTOR_SET	*lpActorSet	=	&g_lpActmap->m_setActor;
		g_lpActmap->m_isVIRGIN	=	FALSE;
...
cSetDialogNpcData::RemmoveNpc(HWND _hDlg)
{
	...
	if(MessageBox(_hDlg, "선택된 캐릭터가 삭제 됩니다. 삭제하시겠습니까?", "캐릭터 삭제", MB_YESNO) == IDYES)
	{
		int selectlist = 0;
		s_iSelOldList == ePAGE_LIST ? selectlist = s_iSelectNpcData : selectlist = s_iSelectFindNpcData;
...
cSetDialogNpcData::RemoveAllNpc(HWND _hDlg)
{
	...
	if(MessageBox(_hDlg, "모든 캐릭터가 삭제됩니다. 삭제하시겠습니까?", "모든 캐릭터 삭제", MB_YESNO) == IDYES)
	{
		cACTOR_SET	*lpActorSet	=	&g_lpActmap->m_setActor;
