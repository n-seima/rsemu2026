...
CShopItem				CSetShopItem::s_shopitemEdit;	//	편집중인 상점
cLISTCONTROL			CSetShopItem::s_listRegistPrefix;	//	접두사 리스트
int						CSetShopItem::s_iPrefixListWndPos;
int						CSetShopItem::s_iSelectRegistPrefix	=	0xffff;

cLISTCONTROL			CSetShopItem::s_listPrefix;	//	접두사 리스트
...
//	등록된 접두사 윈도우 초기화
	{
		...
		s_listRegistPrefix.AddColumn("식별코드"	,6*10);
		s_listRegistPrefix.AddColumn("1차 접두사"	,6*20);
		s_listRegistPrefix.AddColumn("접두사 타입",6*32);
...
//	접두사 윈도우 초기화
	{
		...
		s_listPrefix.AddColumn("식별코드"	,6*10);
