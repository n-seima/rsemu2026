...
//	드랍 이미지
	...
	cTEXT::SetBoldFont();
	cTEXT::Put(x,y,GOLDYELLOW,"━━ 상점 아이템용 정보");
	cTEXT::SetNormalFont();

	if (lpItem->m_dwPrice	==	0)
			cTEXT::Put(x,y+=16,LTGREEN,"        아이템 가격 : 기존의 가격 계산식 적용");
	else	cTEXT::Put(x,y+=16,LTGREEN,_ms("        아이템 가격 : %dGold",lpItem->m_dwPrice));

	cTEXT::Put(x,y+=16,LTGREEN,_ms("아이템 생성  퀄리티 : %d％",lpItem->m_wGenerateQuality));

	if (lpItem->m_bf10ChangeLimitLevel)
	{
		cTEXT::Put(x,y+=16,LTGREEN,_ms("착용 레벨 : %d시간",lpItem->m_bf10ChangeLimitLevel));
	//	접두사 변경 주기
	}
	else
	{
	//	접두사
		...
		if (iPrefixCount)
		{
			cTEXT::Put(x,y+=16,LTGREEN,_ms("접두사 생성  퀄리티 : %d％",lpItem->m_wPrefixQuality));

			for (int i=0;i<iPrefixCount;i++)
			{
				cITEM_PREFIX *lpPrefix	=	g_itemPrefix.get(lpItem->m_aPrefix[i].m_wPrefix);

				cTEXT::Put(x,y+=16,LTGREEN,_ms("           접두사 %d : %s",i+1,lpPrefix->m_str1stPrefix));
			}
		}
	}
	...
	else
	{
		...
		if (s_lpSelectItem->m_bf1IsMaleJobEquipAble)	job.Add("남성/");
		if (s_lpSelectItem->m_bf1IsFemaleJobEquipAble)	job.Add("여성/");
...
{	// 마지막 수정일 : 09.10.16
	...
	if (stricmp(s_shopEdit.m_strName,"")==0)	strcpy(s_shopEdit.m_strName,"이름 좀 지어 줘요!! ㅠ.ㅜ");
