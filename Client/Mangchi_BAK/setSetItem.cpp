...
cSetItemSet::DecodeTextScriptFile(char *_lpstrFn)
{
	...
	while(1)
	{
		...
		if (iResult	==	eDSITSR_READ_ITEM_SET_END)
		{
			...
			if (iSerial	==	0xffff)
			{
				ERRMSG("아이템 세트 이름이 이름이 중복 됩니다.","%s",s_makeGroup.m_strName);
				iOverlappedCount++;
			}
		}
	}

	cMSG::Put("데이터 화일 분석 결과","%d개의 아이템 세트 추가 했습니다.\n\n%d개의 아이템 세트 이름이 중복 되었습니다.",g_itemSetManager.getCount()-1,iOverlappedCount);
			...
			switch(iCase)
			{
				case	0	:
				{
					...
					if (lpSetEffect->m_wEffect	==	0xffff)
						return	ERRMSG("에러!!","'%s' 라는 아이템 효과는 없습니다.",lpStream);
					break;
				}
...
cSetItemSet::LoadingTextScript(HWND _hDlg)
{
	...
	if (!cFILE::LoadBox(strFn,"텍스트 화일 {*.txt}\0*.txt\0\0","txt",NULL,"세트 아이템 텍스트 스크립트 데이터 파일 불러오기"))
	{
		SetLogFolder();
		return;
	}
...
cSetItemSet::RemoveGroup(HWND _hDlg)
{
	...
	if (cMSG::YESNO("세트 아이템 제거","정말로 선택한 세트 아이템을 제거 하시겠습니까?") == IDNO)
		return;
