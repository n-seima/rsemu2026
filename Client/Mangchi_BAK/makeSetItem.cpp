...
cMakeItemSet::InitDlg(HWND _hWnd)
{
	...
	for (int i=0;i<dMAX_SET_ITEM_EFFECT_GRADE_COUNT;i++)
		cCOMBOBOX::Add(hComoboEffctByCount,_ms("%2d개 모았을때 효과",i+1),i);
...
cMakeItemSet::FinishWork(HWND _hDlg)
{
	if (s_makeItemSet.getItemCount() < 2)
		return	ERRMSG("세트 아이템 제작","에러!! 세트 구성 아이템이 두개 이상 필요합니다.");

	if (s_makeItemSet.getSetEffectCount() < 1)
		return	ERRMSG("세트 아이템 제작","에러!! 세트 효과가 하나 이상 필요합니다.");
	...
	if (lpGroup)
		if (s_makeItemSet.m_wSerial	!=	lpGroup->m_wSerial)
			return	ERRMSG("세트 아이템 제작","에러!! 이미 동일한 이름의 세트 아이템이 있습니다.");
		...
		case	WM_COMMAND	:
			switch( LOWORD( wParam ) )
            {
				...
				case	IDC_ADD_SET_EFFECT		:
				{
					...
					{
						ERRMSG("에러!!","더 이상의 세트 효과를 추가 할 수 없습니다.");
						break;
					}
				...
				case	IDC_REMOVE_SET_EFFECT	:
				{
					...
					if (cMSG::YESNO("세트 효과를 제거 합니다.","선택한 세트 효과를 리스트에서 제거 하시겠습니까?")==IDNO)
						break;
				...
				case	IDC_ADD_ITEM			:
				{
					...
					{
						cMSG::Put("경고!!","더 이상의 아이템을 추가 할 수 없습니다.");
						break;
					}
				...
				case	IDC_REMOVE_ITEM			:
				{
					...
					if (cMSG::YESNO("아이템을 제거 합니다.","선택한 아이템을 세트 아이템 리스트에서 제거 하시겠습니까?")==IDNO)
						break;
