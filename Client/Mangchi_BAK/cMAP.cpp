...
cMAP::RemarkCrashMap()
{
	MSGOUT("모든 건물 충돌맵 재 설정");
	remarkAllBuildingInfo(TRUE,FALSE,FALSE);
	MSGOUT("모든 오브젝트 충돌맵 재 설정");
...
cMAP::SaveKarma(cFILE *_lpFile,CKarma *_lpKarma)
{
	...
	if (_lpKarma->m_pstrTitle)
	{
		if (_lpKarma->m_wTitleLength	== 0)
			ERRMSG("error in cMAP::SaveKarma","... 어째서인지 스트링 사이즈는 0인데 스트링은 있다.. -_-?");
...
cMAP::SaveKarmaItem(cFILE *_lpFile,CKarmaItem *_lpKarma)
{
	...
	if (_lpKarma->m_strString)
	{
		if (_lpKarma->m_wStringSize	== 0)
			ERRMSG("error in cMAP::SaveKarmaItem","... 어째서인지 스트링 사이즈는 0인데 스트링은 있다.. -_-?");
...
cMAP::Save(char *fn,int index,char *_lpstrSaveFileName,BOOL _bIsTempSave,char *_lpstrTargetFolder,BOOL _bIsLocalize)
{
	...
	{
		int		iLocalizeNation	=	eNationKorea;
	...
	if (lpMap->m_aBgmList[0] ==	0xffff	)
	{
		if	(g_bIsLocalizing	==	FALSE)
			ERRMSG("뷁!!!!!","BGM이 설정되어 있지 않습니다.!!\n\n%s",fn);
	}
	if	(lpMap->m_bf4FieldType==	0		)
	{
		if	(g_bIsLocalizing	==	FALSE)
			ERRMSG("뷁!!!!!","필드 타입이 설정되어 있지 않습니다.!!\n\n%s",fn);
	}
			...
			if (value != 0xffff)
			{
				...
				if	(lpMap->m_object.m_pFixedObjectImage[lpObjectInfo->m_wObject].isDoor())
				{
					if	(lpArea->isExistDoorAreaPos(x*dTILE_WIDTH+dTILE_WIDTH/2,y*dTILE_HEIGHT+dTILE_HEIGHT/2))
					{
						...
						{
							ERRMSG("error in save map","문수가 너무 많아요. (max %d)\n\n%s",dMAX_DOOR_IN_MAP,fn);
						}
					}
				}
				continue;
...
////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if	(_bIsLocalize	==	FALSE)
	{
		...
		{
			if	(g_bIsLocalizing	==	FALSE)
				cMSG::Put("필드 저장 경고!!","'%s' 화일에 연결되지 않은 출구 영역이 있습니다.",fn);
		}
	}
		...
		if	(lpArea->IsAvail(i))
		{
			...
			if	(lpArea->m_aArea[i].isEventArea()	||	lpArea->m_aArea[i].isGuildCrest())
			{
				...
				if	(lpEventArea->m_wLinkObject	==	0xffff)
					cMSG::Put("필드 저장 경고!!","'%s' 영역에 링크된 오브젝트가 없습니다.",lpEventArea->m_strName);
			}
...
////////////////////////////////////////////////////////////////////////////////////////////////
	MSGOUT("오브젝트 로딩 - 1");
	{
		...
		for (i=0;i<iObjectCount;i++)
		{
			...
			if (lpInfo->m_wObject		>=	lpMap->m_object.m_iTinyObjectImageCount)
			{
				...
				return	ERRMSG("에러!!","맵셋에 없는 바닥 오브젝트가 찍혀 있습니다.\n\n인덱스 %d",lpInfo->m_wObject);
			}
	...
	MSGOUT("오브젝트 로딩 - 2");
	{
		...
		if(iObjectCount	>=	dMAX_OBJECT_COUNT)
			cMSG::Put("얼래? 이거 오브젝트 수가 장난이 아닌걸. -o-","%s",fn);
		...
		for (i=0;i<iObjectCount;i++)
		{
			...
			if (object.m_wObject	>=	lpMap->m_object.m_iFixedObjectImageCount)
			{
				...
				return	cMSG::Put("에러!!","맵셋에 없는 고정 오브젝트가 찍혀 있습니다.\n\n인덱스 %d",object.m_wObject);
			}
...
////////////////////////////////////////////////////////////////////////////////////////////////
	{
		...
		for (i=0;i<iCount;i++)
		{
			...
			if (saveBuilding.m_wBuilding	>=	lpMap->m_building.m_iDataCount)
			{
				...
				return	ERRMSG("에러!!","맵셋에 없는 건물이 찍혀 있습니다.\n\n인덱스 %d",saveBuilding.m_wBuilding);
			}
...
{
	if	(getLock()	==	FALSE)
		cMSG::Put("경고!!","락을 거는데 실패 했습니다.\n\n방금 편집한 내용은 저장 할 수 없게 됩니다. 먼저 락을 거시기 바랍니다.");
...
{
	if	(IsVirgin()	==	FALSE)
		if	(cMSG::YESNO("서버로 전송","편집 중인 파일에 저장하지 않은 내용이 있습니다.\n\n저장후 서버에 전송 하시겠습니까?")	==	IDYES)
			cPROJECT::SaveMap();

	if	(CommitFileSVN(m_strFName,_bIsByCommand)	==	TRUE)
	{
		cMSG::Put("현재 파일 서버에 전송","성공했습니다.");

		return	TRUE;
	}
	else
		cMSG::Error("현재 파일 서버에 전송","실패했습니다.");
