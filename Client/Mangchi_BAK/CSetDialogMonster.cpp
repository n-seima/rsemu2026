...
CSetDialogMonster::InitSetDialogListBoxData(int sel)
{
	...
	case ePAGE_LIST:
		...
		s_listMonster.AddColumn("이름"			,6*36);
		break;
	case ePAGE_FIND_LIST:
		...
		s_listFindMonster.AddColumn("이름"		,6*36);
		break;
	}
...
CSetDialogMonster::DrawPageMonsterBaseInfo()
{
	...
	cTEXT::Put(5, 10,WHITE,"이름      : ");
	cTEXT::Put(5, 25,WHITE,"분류      : ");
	cTEXT::Put(5, 40,WHITE,"리젠 주기 : ");
	cTEXT::Put(5, 55,WHITE,"행동 주기 : ");
	cTEXT::Put(5, 70,WHITE,"직시 방향 : ");
	cTEXT::Put(5, 85,WHITE,"행동 영역 : ");
	cTEXT::Put(5,100,WHITE,"전투 성향 : ");

	cTEXT::Put(5,130,WHITE, "행동 영역 안에서 생성 : ");
	cTEXT::Put(5,145,WHITE, "자동 리젠 안됨 : ");
	cTEXT::Put(5,160,WHITE, "이따끔 방향을 바꿈 : ");	
...
CSetDialogMonster::DrawPageMonsterGeneralInfo()
{
	...
	cTEXT::Put(5, ypos + height * 0,YELLOW,"생성 레벨        : ");		
	cTEXT::Put(5, ypos + height * 1,YELLOW,"스케일           : ");
	cTEXT::Put(5, ypos + height * 2,YELLOW,"스킬사용빈도보정 : ");
	cTEXT::Put(5, ypos + height * 3,YELLOW,"최종 대미지 보정 : ");
	cTEXT::Put(5, ypos + height * 4,YELLOW,"체력 보정        : ");
	cTEXT::Put(5, ypos + height * 5,YELLOW,"경험치 보정      : ");
	cTEXT::Put(5, ypos + height * 6,YELLOW,"공격력 보정      : ");
		
	cDRAW::Box(_LTYELLOW,5,ypos + height * 8 - 5,410, ypos + height * 15);

	cTEXT::Put(10, ypos + height * 8,YELLOW,"물리 발동 확률  : ");				cTEXT::Put(200, ypos + height * 8,YELLOW,"흡수량  : ");
	cTEXT::Put(10, ypos + height * 9,YELLOW,"  불 발동 확률  : ");				cTEXT::Put(200, ypos + height * 9,YELLOW,"흡수량  : ");
	cTEXT::Put(10, ypos + height *10,YELLOW,"바람 발동 확률  : ");				cTEXT::Put(200, ypos + height *10,YELLOW,"흡수량  : ");
	cTEXT::Put(10, ypos + height *11,YELLOW,"  빛 발동 확률  : ");				cTEXT::Put(200, ypos + height *11,YELLOW,"흡수량  : ");
	cTEXT::Put(10, ypos + height *12,YELLOW,"  물 발동 확률  : ");				cTEXT::Put(200, ypos + height *12,YELLOW,"흡수량  : ");
	cTEXT::Put(10, ypos + height *13,YELLOW,"대지 발동 확률  : ");				cTEXT::Put(200, ypos + height *13,YELLOW,"흡수량  : ");
	cTEXT::Put(10, ypos + height *14,YELLOW,"어둠 발동 확률  : ");				cTEXT::Put(200, ypos + height *14,YELLOW,"흡수량  : ");
	
	cDRAW::Box(_LTYELLOW, 5, ypos + height *16+1, 15, ypos + height *17-5);		cTEXT::Put(20, ypos + height *16,YELLOW,"지능 공식 정상화");
	cDRAW::Box(_LTYELLOW, 5, ypos + height *17+1, 15, ypos + height *18-5);		cTEXT::Put(20, ypos + height *17,YELLOW,"조련 불가");
	cDRAW::Box(_LTYELLOW, 5, ypos + height *18+1, 15, ypos + height *19-5);		cTEXT::Put(20, ypos + height *18,YELLOW,"돈 안줌");
	cDRAW::Box(_LTYELLOW, 5, ypos + height *19+1, 15, ypos + height *20-5);		cTEXT::Put(20, ypos + height *19,YELLOW,"아이템 안줌");
	cDRAW::Box(_LTYELLOW, 5, ypos + height *20+1, 15, ypos + height *21-5);		cTEXT::Put(20, ypos + height *20,YELLOW,"경험치 안줌");
	cDRAW::Box(_LTYELLOW, 5, ypos + height *21+1, 15, ypos + height *22-5);		cTEXT::Put(20, ypos + height *21,YELLOW,"무적");
	cDRAW::Box(_LTYELLOW, 5, ypos + height *22+1, 15, ypos + height *23-5);		cTEXT::Put(20, ypos + height *22,YELLOW,"키 몬스터");
	cDRAW::Box(_LTYELLOW, 5, ypos + height *23+1, 15, ypos + height *24-5);		cTEXT::Put(20, ypos + height *23,YELLOW,"세트 보스");
	
	cDRAW::Box(_LTYELLOW, 185, ypos + height *16+1, 195, ypos + height *17-5);	cTEXT::Put(200, ypos + height *16,YELLOW,"유혹 면역");
	cDRAW::Box(_LTYELLOW, 185, ypos + height *17+1, 195, ypos + height *18-5);	cTEXT::Put(200, ypos + height *17,YELLOW,"경직 면역");
	cDRAW::Box(_LTYELLOW, 185, ypos + height *18+1, 195, ypos + height *19-5);	cTEXT::Put(200, ypos + height *18,YELLOW,"석화 면역");	
	cDRAW::Box(_LTYELLOW, 185, ypos + height *19+1, 195, ypos + height *20-5);	cTEXT::Put(200, ypos + height *19,YELLOW,"열기 면역");
	cDRAW::Box(_LTYELLOW, 185, ypos + height *20+1, 195, ypos + height *21-5);	cTEXT::Put(200, ypos + height *20,YELLOW,"녹백 면역");
	cDRAW::Box(_LTYELLOW, 185, ypos + height *21+1, 195, ypos + height *22-5);	cTEXT::Put(200, ypos + height *21,YELLOW,"마법 면역");
	cDRAW::Box(_LTYELLOW, 185, ypos + height *22+1, 195, ypos + height *23-5);	cTEXT::Put(200, ypos + height *22,YELLOW,"물리 연역");
	cDRAW::Box(_LTYELLOW, 185, ypos + height *23+1, 195, ypos + height *24-5);	cTEXT::Put(200, ypos + height *23,YELLOW,"혼란 면역");

	cTEXT::Put(5, ypos + height *26,GREEN,"HP          : ");
	cTEXT::Put(5, ypos + height *27,GREEN,"EP          : ");
	cTEXT::Put(5, ypos + height *28,GREEN,"민첩        : ");
	cTEXT::Put(5, ypos + height *29,GREEN,"운          : ");			cTEXT::Put(250, ypos + height *29,GREEN,"컨 셉     : ");
	cTEXT::Put(5, ypos + height *30,GREEN,"최소 공격력 : ");			cTEXT::Put(250, ypos + height *30,GREEN,"스킬 포함 : ");		
	cTEXT::Put(5, ypos + height *31,GREEN,"최대 공격력 : ");			cTEXT::Put(250, ypos + height *31,GREEN,"스킬 포함 : ");	
	cTEXT::Put(5, ypos + height *32,GREEN,"기본 방어력 : ");			
...
CSetDialogMonster::DrawPageMonsterJob()
{
	...
	if(s_nMonsterCount != -1)
	{
		...
		cTEXT::Put(rxpos + 90, ypos + height * 6,WHITE, _ms("%d 단계",_lpEditJob->m_wVariation));
		
		cTEXT::Put(lxpos + 90, ypos + height * 8,WHITE, g_strBattleInclination[_lpEditJob->m_wBattleInclination]);
		cTEXT::Put(200, ypos + height * 9,WHITE, _ms("%d 회",_lpEditJob->m_bf3RequireRebirthCountForTame));
		...
		cTEXT::Put(rxpos + 90, ypos + height * 20,WHITE,_ms("%d.%.2d 초",_lpEditJob->m_wAttackSpeed/100,_lpEditJob->m_wAttackSpeed%100));
