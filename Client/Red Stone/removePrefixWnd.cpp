...
cRemovePrefixWnd	g_gwRemovePrefix;

//	열기
void
cRemovePrefixWnd::open(int _iSrcItem,int _iDestItem)
{
	...
	if	(!lpSrcItem->checkFirstEffect(eIE_REMOVE_PREFIX)	&&	!lpSrcItem->checkFirstEffect(eIE_CH5_REMOVE_PREFIX))
		return;
	...
	s_iPopupInterface			=	ePIW_REMOVE_PREFIX;
	s_rectInterfaceSize.set(g_iScreenWidth-iWindowWidth, g_iCorrectBI_Y/2, g_iScreenWidth, g_iCorrectBI_Y/2+iWindowTitleHeight+iWindowHeight);
	m_iSelectPrefix				=	0;
...

	{
		...
		iX						-=	(iBarWidth+2);
		m_bmMenu.addImageBar(dMSG_REMOVE_PREFIX	,IDOK		,iX,iY,eBUTTON_NORMAL,eBUTTON_ACTIVE,eBUTTON_ACTIVE,0xffff,0xffff);
...
int
cRemovePrefixWnd::getFocusPrefix()
{
...
void
cRemovePrefixWnd::updateBarmenu()
{
	cItem	*lpDestItem		=	g_hero.getInventoryItem(m_iDestItem);
	...
	switch(dwMenu)
	{
		case	IDOK		:
		{
			...
			sprintf(strText,dMSG_CONFIRM_REMOVE_PREFIX_FORM,lpPrefix->m_str1stPrefix,lpstrComment,m_aEquipLevel[m_iSelectPrefix]);

			g_msgBox.cPopup("",strText,dMSG_YES,dMSG_NO);
			g_msgBox.setButtonId(eMBI_REMOVE_SELECT_PREFIX);
...
void
cRemovePrefixWnd::update()
{
	if	(s_iPopupInterface	!=	ePIW_REMOVE_PREFIX)
		return;
