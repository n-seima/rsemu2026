...
CActiveSkill::castGasTypeSkill()
{
	...
	setGasTypeSkillUpkeepTime(timeGetTime()+iUpkeepTime*10);
...
//	폭발형 스킬 업데이트
BOOL
CActiveSkill::updateGasTypeSkill()
{
	...
	if	(m_lpSkill->m_bf1IsZoomInEffectSkill && isExplosionWhichDefendOnImageSkillZoomming())
	{
		...
		if (iScale	<	m_lpSkill->m_wImageScale)
			return	FALSE;
...
//	프레임 갱신
	...
	if	(isGasTypeSkillFinishSkillTime(timeGetTime()))
	{
