...
void
cTREEVIEW::SetText(HTREEITEM item,char *_strText)
{	if (!hWND) return;

	TV_ITEM	tvItem;

	tvItem.mask		=	TVIF_TEXT;
	tvItem.pszText	=	_strText;
	tvItem.hItem	=	item;
...
void
cTREEVIEW::Delete(HTREEITEM item)
{
	TreeView_DeleteItem(hWND,item);
}	//	cTREEVIEW::Delete(HTREEITEM item)
...
int
cLISTVIEW::getColumnWidth(int _iColumn)
{
	return ListView_GetColumnWidth(hWND,_iColumn);
}
...
void
cLISTVIEW::SetItem(int index,int subItem,int iValue)
{	if (!hWND)	return;

	LV_ITEM			item;

	item.mask		= LVIF_TEXT;
	item.pszText	= _ms("%d",iValue);
	item.iItem		= index;
	item.iSubItem	= subItem;
...
int
cLISTVIEW::getData(int index,int subItem)
{
	LVITEM	item;
