...
/*	MSGOUT("PCX file : %s", fname);
	...
	MSGOUT("   pallete[48]:");
	MSGOUT("      reserved: %d", PCX_HEADER.reserved);
...
void
encode(FILE *fp,BYTE *buffer, int xs,int ys)
{
	BYTE	runchar,runcount,ch;
	...
	for (int y=0; y<ys; y++)
    {
		...
		for (int x=0; x<xs; x++)
		{
			...
			else
			{
				...
				{
					if ((runcount > 1) || ((runchar & 0xC0) == 0xC0))	fputc(0xC0 | runcount, fp);
					fputc(runchar,fp);
					runcount = 1;
					runchar = ch;
				}
...
BOOL
cPCX::Save(char * fname,BYTE *image,int xs,int ys,BYTE *lpPlt)
{
	if	(xs%2)	return FALSE;
	...
	PCX_HEADER.bytes_per_line	=	xs;
	PCX_HEADER.pallete_type		=	1;

	cFILE	file;

	if (!file.Open(fname,"wb"))	return ERRMSG("Error in cPCX::Load","can't find '%s' file",fname);

	file.Write(&PCX_HEADER, sizeof(PCX_HEADER));

	encode(file.fp,image,xs,ys);

	BYTE	dummy	=	12;
	file.Write(&dummy,1);			//	skip

	file.Write(lpPlt,256 * 3);
