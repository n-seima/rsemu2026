	...
	{	switch(workMode)
		{	case	dMODE_NORMAL		:
			...
			{	WORD color;
				...
				cTEXT::Put(4,4 ,color,"If the selected image is correct, click the left mouse button on the ",32);
				cTEXT::Put(4,18,color,"Press it and if it's wrong, press the right button.",32);
				cTEXT::OutLine(TRUE,0);
				break;
			}
		}
...
{	int index	=	0,fpos=0,size=strlen(_BMPWND.BMPName),count=0;
	...
	{	ImageFolder[0]		=	0;
		...
		bQUICKOPEN	=	FALSE;
		_MAIN.Draw();
		cMSG::Put("Image processing complete!!",
				"A total of %d image files were opened to generate %d images:\n\n 08bit data files: %d files\n 16bit data files: %d files",totalFile,totalImage,file8,file16);
...
cBMPWND::openImageAndShadow()
{
	...
	cMSG::Put("Image processing complete!!",
		"A total of %d files were opened:\n\n Images: %d\n Shadows: %d",iFileCount,iImageCount,iShadowCount);
}
...
cBMPWND::operateLoadCharacterImage()
{
	...
	if	(cMSG::YESNO("Is the information below correct?","Job: %s\n\nParts: %s",g_strJobName[iJob],strPartsDetailName)	==	IDNO)
		return;
...
cBMPWND::OPENALL()
{
	...
	{	EnableWindow(hPROGRESS	,TRUE);
		...
		SendMessage(hSTATUS,WM_SETTEXT,0,(LPARAM)"Searching for subfolders");
			...
			{	char cur[1024];
				...
				if (files)
				{
					...
					if (!bOPENFILES)
					{
						...
						cMSG::Put("The operation has stopped.","A total of %d image files were opened to create %d images:\n\n 08bit data files: %d files\n 16bit data files: %d files",totalFile,totalImage,file8,file16);
						_MAIN.Draw();
						return;
					}
		...
		if	(!g_isGetImageAndShadow || g_iLoadPart)
		{
			if (bGETSHADOW)
				cMSG::Put("Image processing complete!!",
				"A total of %d files were opened to create %d shadows:\n\n Current shadows: %d\n Current images: %d",totalFile,totalImage,totalImage,cIMG::Count);
			else
			cMSG::Put("Image processing complete!!",
				"A total of %d image files were opened to generate %d images:\n\n 08bit data files: %d files\n 16bit data files: %d files",totalFile,totalImage,file8,file16);
		}
	...
	{	if (!bGETSHADOW)
		...
		{	if (loadBmp(file->String)) totalFile++;
			...
			SendMessage(hINFO,WM_SETTEXT,0,(LPARAM)_ms("'%s'Converting dollars.",file->String));
			file	=	file->pNEXT;
		}
