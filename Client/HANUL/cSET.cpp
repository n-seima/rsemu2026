...
char		*strAnmDirect[]	=	{"No Direction", "Left and right 2 Directions", "Up and down 2 Directions", "4 Directions", "8 Directions", "16 Directions", "32 Directions", "64 Directions"};
...
{	int		i,j,k;
	...
	if (cANM::AnmCount > anmCount)
		for (i=anmCount;i<dMAX_ANM;i++)	ANM[i].ClearData(anmType);
...
//	crashType	=	cANM::CrashType;
	...
	//	전체 설정..
	for (i=0;i<8;i++)	cCOMBOBOX::Add(GetDlgItem(hDlg,IDC_ANI_KIND),_ms("%s Animations",strAnmType[i]));
...
ANMLIST.AddColumn(IDC_INDEX,40 ,"Number");
	ANMLIST.AddColumn(IDC_ANIMATION,140,"Animation Name");
	ANMLIST.AddColumn(IDC_DIRECT,80 ,"Direction");
	ANMLIST.AddColumn(IDC_MAX_FRAME,66 ,"Total Frames");
...
void
cSET::AddAnm(HWND _hDlg)
{
	tempAnm[anmCount].anmtype		=	anmType;
	tempAnm[anmCount].directCount	=	_directCount[anmType];

	ANMLIST.AddItem(anmCount,IDC_INDEX,_ms("%3d",anmCount));
	ANMLIST.SetItem(anmCount,IDC_ANIMATION,tempAnm[anmCount].Name);
	ANMLIST.SetItem(anmCount,IDC_DIRECT,strAnmDirect[tempAnm[anmCount].anmtype]);
	ANMLIST.SetItem(anmCount,IDC_MAX_FRAME,_ms("%d",tempAnm[anmCount].FrameCount));
...
void
cSET::InsertAnm(HWND _hDlg)
{
	int	curAnm	=	ANMLIST.GetSelectItem();

	anmCount++;

	for (int i=anmCount-1;i>=curAnm;i--)
	{
		strcpy(tempAnm[i].Name,tempAnm[i-1].Name);

		tempAnm[i].anmtype		=	tempAnm[i-1].anmtype;
		tempAnm[i].directCount	=	tempAnm[i-1].directCount;
		tempAnm[i].FrameCount	=	tempAnm[i-1].FrameCount;
	}

	tempAnm[curAnm].anmtype		=	anmType;
	tempAnm[curAnm].directCount	=	_directCount[anmType];
	tempAnm[curAnm].FrameCount	=	10;
	strcpy(tempAnm[curAnm].Name,"_Insert");

	ANMLIST.AddItem(anmCount,IDC_INDEX,_ms("%3d",anmCount-1));
	ANMLIST.SetItem(anmCount,IDC_ANIMATION,tempAnm[anmCount-1].Name);
	ANMLIST.SetItem(anmCount,IDC_DIRECT,strAnmDirect[tempAnm[anmCount-1].anmtype]);
	ANMLIST.SetItem(anmCount,IDC_MAX_FRAME,_ms("%d",tempAnm[anmCount-1].FrameCount));

	for (i=curAnm+1;i<anmCount;i++)
	{
		ANMLIST.SetText(i,IDC_ANIMATION,tempAnm[i].Name);
		ANMLIST.SetText(i,IDC_DIRECT,strAnmDirect[tempAnm[i].anmtype]);
		ANMLIST.SetText(i,IDC_MAX_FRAME,_ms("%d",tempAnm[i].FrameCount));
...
void
cSET::RemoveAnm(HWND _hDlg)
{
	int	curAnm	=	ANMLIST.GetSelectItem();

	for (int i=curAnm;i<anmCount;i++)
	{
		strcpy(tempAnm[i].Name,tempAnm[i+1].Name);
	...
	for (i=curAnm;i<anmCount;i++)
	{	ANMLIST.SetText(i,IDC_ANIMATION,tempAnm[i].Name);
		ANMLIST.SetText(i,IDC_DIRECT,strAnmDirect[tempAnm[i].anmtype]);
		ANMLIST.SetText(i,IDC_MAX_FRAME,_ms("%d",tempAnm[i].FrameCount));
	}

	ANMLIST.DelItem(anmCount);
}
	...
	{	case WM_INITDIALOG	:
		...
		case WM_COMMAND :
			...
			{	case	IDOK			:
				...
				case	IDB_SAVE		:
					if (cMSG::YESNO("Save animation settings data","We need to apply the current state before saving, do you want to continue?")==IDNO) break;
...
{	int i;
	...
	CheckDlgButton	(hDlg,IDC_RELEASE_POINT	,tempAnm[curAnm].bSaveReleasePoint);
	CheckDlgButton	(hDlg,IDC_REFIT_FRAME	,tempAnm[curAnm].bSaveRefitPos);

	if (anmType==dANI_CUSTOM)
		for (i=0;i<8;i++) EnableWindow(GetDlgItem(hDlg,IDR_SINGLE+i),FALSE);
}
...
{	int		i;
	...
	if (anmType!= dANI_CUSTOM)
		for (i=IDR_SINGLE;i<=IDR_64WAY;i++)	
	...
	tempAnm[curAnm].bSaveReleasePoint	=	IsDlgButtonChecked(hDlg,IDC_RELEASE_POINT);
	tempAnm[curAnm].bSaveRefitPos		=	IsDlgButtonChecked(hDlg,IDC_REFIT_FRAME);
