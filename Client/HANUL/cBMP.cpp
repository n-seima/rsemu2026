...
{	FILE	*fp;
	...
	fp=fopen(fn,"rb");
	if (fp==NULL) return MSGOUT("Error!! in class cPICTURE::Load(... - '%s' Tuesday was not found",fn);
...
{	pKILL(p16IMAGE);
    ...
    if (bmFileHeader.bfType != 0x4d42)  // 비트맵인지 확인한다.(BMP화일인지..
		return MSGOUT("Error!! in cBMP::Load(... - BMP Not Tuesday.");
	...
	{	p16IMAGE	=	new WORD [Width*Height+2];
		...
		for (i=0;i<Height;i++)
		{
			...
			if (InfoHd->biHeight<0)	addr+=bytePerLine;
			else					addr-=bytePerLine;
		}
	...
	return	MSGOUT("ERROR!! in cBMP::Load(... - '%d' bit Image is not supported.",BPP);
}