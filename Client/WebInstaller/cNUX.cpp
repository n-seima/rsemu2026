...
{	bMERGE	=	merge;
	...
	if (!outFile) return cMSG::Error("Compress Error!!","'%s' File Create Error!!",fn);
...
//	outFile	=	NULL;
	...
	{	int read	=	read_header();
		...
		{	SetCurrentDirectory(processFolder);
			...
			{	if (!cFOLDER::Create(folderName,TRUE)) return cMSG::Error("Error in cNUX::PressExtract","Don't Create Follow Folder\n\n%s",folderName);
			...
			}	else SetCurrentDirectory(folderName);
...
//	초기화
	...
	{	fclose(arcFile);
		arcFile	=	NULL;
		if (ismerge==ISPRESSFILE)	return PressExtract(fn,destFolder,strLogFile);
...
cNUX_INFO::Update()
{
	...
	if(m_dwTotalSize		!=	0)	m_iProgressRate		=	(int)(((UINT64)m_dwProgressSize*1000)/m_dwTotalSize);
	if(m_dwFileSize			!=	0)	m_iProgressFileRate	=	(int)(((UINT64)m_dwProgressFileSize*1000)/m_dwFileSize);
	if(m_dwProgressTime		!=	0)	m_dwBps				=	m_dwProgressSize / m_dwProgressTime;
}