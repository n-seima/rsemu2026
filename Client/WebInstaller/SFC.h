...
public:
	...
	BOOL		AddAndData	(char *str,int data);
...
public:
	...
	BOOL				readString(char *_lpstrString,int _iStringSize);
	BOOL				Write(void *buffer,DWORD size,DWORD items=1);
	BOOL				Write4(DWORD	value);
	BOOL				Write2(WORD		value);
	BOOL				Write1(BYTE		value);
	BOOL				Rewind();
	BOOL				End();
	BOOL				IsOpen();
...
//
//	NUX화일 인코딩이나 디코딩을 위해 필요한 루틴들. 사용시 cNUX.cpp를 링크해야 한다. 
//
	static	BOOL		bSTOP;
	static	BOOL		bMERGE;
...
inline	BOOL		SetLogFolder()
{
	return	SetCurrentDirectory(_logFolder);
}
...
#define	mCHECKER(err,method)	if (err) method;			//	err이 체크 되면 method를 실행
