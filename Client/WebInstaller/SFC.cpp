...
BOOL
cSTRINGS::AddAndData(char *str,int data)
{
	if (!pHEAD)
	{	pHEAD		=	new cSTRING();
		pTAIL		=	new cSTRING();
...
{	cSORT_STRING_ITEM	*pStrings;
	...
	for (i=0;i<iCount;i++)
	{	AddAndIndex(pStrings[i].m_strString,pStrings[i].m_iValue);
	}
...
BOOL
cFILE::readString(char *_lpstrString,int _iStringSiz)
{
	if (!fp) return FALSE;

	if (!fgets(_lpstrString,_iStringSiz,fp))	return	FALSE;

	return TRUE;
}	//	cFILE::readString(char *_lpstrString){
...
//	else							return;
	...
	{	if (timeGetTime() < idleTime1 + 300 )	LEFTDBLCLICK	=	TRUE;
		idleTime1	=	timeGetTime();
	}
	if ( KEY[dKEY_RBUTTON] && !oldkey[dKEY_RBUTTON])
	{	if (timeGetTime() < idleTime2 + 300 )	RIGHTDBLCLICK	=	TRUE;
		idleTime1	=	timeGetTime();
	}
