...
CBSCallbackImpl::CBSCallbackImpl(cDOWNLOAD *_lpDownload,void (*_lpUpdateFunction)(cDOWNLOAD *))
{
	...
	m_ulObjRefCount		=	1;
}	//	CBSCallbackImpl::CBSCallbackImpl(cDOWNLOAD_INFO *lpInfo,BOOL *lpCancelWork,void	(*update)(cDOWNLOAD_INFO*))
...
CBSCallbackImpl::OnProgress(ULONG ulProgress,ULONG ulProgressMax,ULONG ulStatusCode,LPCWSTR szStatusText)
{
	...
	if (ulStatusCode	==	BINDSTATUS_CACHEFILENAMEAVAILABLE)	strcpy(m_lpDownloadInfo->m_strFileName,W2CT(szStatusText));

	m_lpDownloadInfo->update(ulProgress,ulProgressMax);

	if (m_lpfpUpdate)	m_lpfpUpdate(m_lpDownloadInfo);
	if (m_lpDownloadInfo->m_bIsCancelWork	==	TRUE)	return E_ABORT;  // canceled by the user
...
cDOWNLOAD::update(DWORD dwReceivedSize,DWORD dwDataSize)
{
	...
	if(m_dwProgressTime != 0)
		m_dwBps			=	(m_dwReceivedSize+dwReceivedSize) / m_dwProgressTime;	//	천분에 일초동안 다운받은 양-_-
}

//
//	다운로드 받기
BOOL
cDOWNLOAD::downLoad(char *strUrl,void (*update)(cDOWNLOAD*))
{
	m_dwCurrentFileReceivedSize	=	0;
	...
	HRESULT	hr	=	URLDownloadToCacheFile(NULL,m_strUrl,m_strFileName,URLOSTRM_USECACHEDCOPY,0,&bsc);
...
cDOWNLOAD::IsValidAddress(char *address)
{
	...
	if (::IsValidURL(NULL,T2CW(address),0)	!=	S_OK)	return TRUE;
