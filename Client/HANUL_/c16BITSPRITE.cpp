...
{	int   x1, y1, x2, y2 ,xs ,ys;
	...
	{	count=sprite[bufferPoint++];
		...
		{	xPoint	+=	sprite[bufferPoint++];
			...
			if (xPoint	< 0		)	beginX= -xPoint;			//	왼쪽 클리핑..
			if (size	> beginX)	memset(p2SCREEN+addr+xPoint+beginX,(WORD)alpha,size-beginX);
	...
	{	count	=	sprite[bufferPoint++];
		...
		{	xPoint	+=	sprite[bufferPoint++];
			...
			if (xPoint+ size>  WIDTH)	endX	= WIDTH-xPoint;		//	오른쪽 클리핑..
			if (endX		>	0	)	memset(p2SCREEN+addr+xPoint,(WORD)alpha,endX);
	...
	{	count	=	sprite[bufferPoint++];
		...
		{	xPoint	+=	sprite[bufferPoint++];
			size	=	sprite[bufferPoint++];
			memset(p2SCREEN+addr+xPoint,(WORD)alpha,size);
			bufferPoint	+=	size;
			xPoint		+=	size;
		}
		addr+=WIDTH;
