...
char	strAnmType[10][40]		=	{	"일반",	"좌우 2 방향","상하 2 방향","4 방향","8 방향","16 방향","32 방향","64 방향"};
...
{	{"한방향"},
	...
	{"북","북북동","북동","동북동",
	 ...
	 "남","남남서","남서","서남서",
	 "서","서북서","북서","북북서"},
	{	"방향 01","방향 02","방향 03","방향 04","방향 05","방향 06","방향 07","방향 08","방향 09","방향 10","방향 11","방향 12",
		"방향 13","방향 14","방향 15","방향 16","방향 17","방향 18","방향 19","방향 20","방향 21","방향 22","방향 23","방향 24",
		"방향 25","방향 26","방향 27","방향 28","방향 29","방향 30","방향 31","방향 32"},

	{	"방향 01","방향 02","방향 03","방향 04","방향 05","방향 06","방향 07","방향 08","방향 09","방향 10","방향 11","방향 12",
		"방향 13","방향 14","방향 15","방향 16","방향 17","방향 18","방향 19","방향 20","방향 21","방향 22","방향 23","방향 24",
		"방향 25","방향 26","방향 27","방향 28","방향 29","방향 30","방향 31","방향 32","방향 33","방향 34","방향 35","방향 36",
		"방향 37","방향 38","방향 39","방향 40","방향 41","방향 42","방향 43","방향 44","방향 45","방향 46","방향 47","방향 48",
		"방향 49","방향 50","방향 51","방향 52","방향 53","방향 54","방향 55","방향 56","방향 57","방향 58","방향 59","방향 60",
		"방향 61","방향 62","방향 63","방향 64"}
};
...
cANM::SaveSD(char *fn,BOOL _bIsSaveCodeSprite)
{
	...
	if	(_bIsSaveCodeSprite)
	{
		...
		cMSG::Put("완료","%s 파일에 저장되었습니다.",strFileName);
	}
...
cANM::LoadSD(char *fn)
{
	...
	if	(!fp)
		return cMSG::Error("스프라이트 화일 불러오기 에러!!","'%s' 화일을 찾을 수 없습니다.",fn);
...
cANM::SaveSAD(char *fn,BOOL bInfoOut)
{
	...
	if (!fp)
	{
		...
		if	(!fp)
			return cMSG::Put("스프라이트 에니메이션 화일 저장 에러!!","'%s' 화일 쓰기 에러입니다.",fn);
	}
...
cANM::DamageFrame(int frame)
{
	...
	if (ANM[CurAnm].isEvent[frame]&dEVENT_DAMAGE)
	{
		if (frame	>=	ANM[CurAnm].FrameCount-1)
		{
			...
			cMSG::Error("데미지 프레임 설정 에러!!","마지막 프레임에는 데미지 프레임이 설정 될 수 없습니다!");
		}
		else
		{
			if (ANM[CurAnm].isEvent[frame+1]&dEVENT_DAMAGE)
			{
				cMSG::Error("데미지 프레임 설정 에러!!","데미지 프레임에은 연속으로 설정 될 수 없습니다!");
...
cANM::TriggerFrame(int frame)
{
	...
	if (ANM[CurAnm].isEvent[frame]&dEVENT_TRIGGER)
	{
		if (frame	>=	ANM[CurAnm].FrameCount-1)
		{
			...
			cMSG::Error("트리거 프레임 설정 에러!!","마지막 프레임에는 트리거 프레임이 설정 될 수 없습니다!");
		}
		else
		{
			if (ANM[CurAnm].isEvent[frame+1]&dEVENT_TRIGGER)
			{
				cMSG::Error("트리거 프레임 설정 에러!!","트리거 프레임에은 연속으로 설정 될 수 없습니다!");
...
{	if (!anm->IsEnable())
	{	ERRMSG("에니메이션 동기화 에러!!","로딩된 데이터가 없습니다.");
		return;
	}
