...
*/
	...
	{	
		...
		if	(CurrentImageFolder == ImageFolderCount + 1)
			cTEXT::Put(8,4,_WHITE,"모든 이미지 표시");
		else
		{
			if	(imageFOLDERS)
			{
				...
				if	(name)
				{
					cTEXT::Put(8,4,_WHITE,_ms("현재 폴더 : [%s]",name));
					iCurrentFoldersSize	=	strlen(name)*7 + 100;
				}
			}
		}
		cTEXT::Put(8+iCurrentFoldersSize,4,_WHITE,_ms("이미지 갯수 : [%d]",ImgCount));
...
{	int		result;
	if (all)	result = cMSG::YESNO("모든 스프라이트의 그림자를 지웁니다.","계속하시겠습니까?");
	else		result = cMSG::YESNO("선택된 스프라이트의 그림자를 지웁니다.","계속하시겠습니까?");
