...
{	"새로운 화일",
	"",
	"불러오기(F3)","이미지 불러오기(F4)","그림자 불러오기(F5)","저장하기(F2)",
	"",
	"확대(+)","축소(-)",
	"",
	"스프라이트 편집","에니메이션 편집","충돌박스/데미지박스 설정","선택 박스 설정","이미지 얻어오기",
	"",
	"이전 에니메이션(Q)","다음 에니메이션(W)","시계 반대 방향으로 회전(A)","시계 방향으로 회전(S)",
	"",
	"여러개의 이미지 잡기","투명컬러 설정",
	"",
	"에디터 설정","에니메이션 설정",
...
{
	char	*folder	=	cFOLDER::Tree(hWND,TRUE,"볼 파일들이 있는 폴더를 선택해 주세요.",_cfg.dataDir[0]);
...
{	int i;
	...
	ofn.hwndOwner		=	hWND;
	ofn.lpstrFilter		=	"에니메이션 데이터 {*.sad}\0*.sad\0스프라이트 데이터 {*.sd}\0*.sd\0\0";
...
{	
	...
	if	(_bIsParts)
	{
		...
		sprintf(strText,"%s 파츠 선택",strPartType[what]);

		if	(!cFILE::LoadBox(fn,"Red Stone 에니메이션 데이터 {*.sad}\0*.sad\0\0","sad",l_strPartsDataFolder[what],strText))
			return FALSE;
		...
		if	(g_aPartsData[what].Count	!=	cIMG::Count)
		{
			...
			ERRMSG(_ms("%s 로딩 에러",strPartType[what]),"먼저 작업중인 데이터와 이미지의 수가 완전히 같아야 합니다.\n\n로딩한 화일의 이미지 수는 %d개 입니다.",g_aPartsData[what].Count);
		...
		{	
			...
			ERRMSG(_ms("%s 로딩 에러",strPartType[what]),"이 화일은 %s가 아닙니다.",strPartType[what]);
	...
	if	(!cFILE::LoadBox(fn,"Red Stone 에니메이션 데이터 {*.sad}\0*.sad\0\0","sad",l_dataFolder[what],"Load data"))
		return FALSE;
	...
	if	(addDATA[what].Count	!=	cIMG::Count)
		ERRMSG(_ms("%s 로딩 에러",datatype[what]),"먼저 작업중인 데이터와 이미지의 수가 완전히 같아야 합니다.\n\n로딩한 화일의 이미지 수는 %d개 입니다.",addDATA[what].Count);
	
	if (addDATA[what].Kind	!=	what)
	{	
		ERRMSG(_ms("%s 로딩 에러",datatype[what]),"이 화일은 %s가 아닙니다.",datatype[what]);
...
{	
	...
	if	(!cFILE::LoadBox(fn,"에니메이션 데이터 {*.sad}\0*.sad\0\0","sad",l_dataFolder[g_wAnmDataType],"Load data"))
		return FALSE;
...
{
	char	*folder	=	cFOLDER::Tree(hWND,TRUE,"컨버트할 화일들이 있는 폴더를 선택해 주세요.");	//	폴더트리 쉘
...
*/
	...
	cMSG::Put("컨버트 작업 완료","%d개의 화일을 컨버트 하였습니다.",iFileCount);
}
...
cMAIN::loadPlayerLayerData(BOOL _bByMenu)
{
	...
	if	(_bByMenu)
		cMSG::Put("완료","플레이어 직업 [%s]의 레이어 출력 순서 정보를 로딩 했습니다.",g_strJobName[g_wPlayerJob-1]);
}
