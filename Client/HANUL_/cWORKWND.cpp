...
{	barMenu[0 ].Set("외곽선"	,5			,HEIGHT-22	,71		,HEIGHT-4	,IDM_SET_OUTLINE);
	...
	barMenu[4 ].Set("충돌박스"	,5			,HEIGHT-102	,71		,HEIGHT-84	,IDM_SET_CRASHBOX);
	barMenu[5 ].Set("인덱스"	,5			,HEIGHT-122	,71		,HEIGHT-104	,IDM_PUT_INDEX);
	barMenu[6 ].Set("Lock"		,5			,HEIGHT-142	,71		,HEIGHT-124	,IDM_LOCK);

	barMenu[8 ].Set("불꽃"		,WIDTH-71	,HEIGHT-22	,WIDTH-5,HEIGHT-4	,IDM_ALPHA_SOFTDODGE);
	barMenu[9 ].Set("중독"		,WIDTH-71	,HEIGHT-42	,WIDTH-5,HEIGHT-24	,IDM_ALPHA_DIFFEREMT);
	barMenu[10].Set("그을림"	,WIDTH-71	,HEIGHT-62	,WIDTH-5,HEIGHT-44	,IDM_ALPHA_DODGEBURN);
	barMenu[11].Set("연기"		,WIDTH-71	,HEIGHT-82	,WIDTH-5,HEIGHT-64	,IDM_ALPHA_SOFT);
	barMenu[12].Set("빛마법"	,WIDTH-71	,HEIGHT-102	,WIDTH-5,HEIGHT-84	,IDM_ALPHA_DODGE);
	barMenu[13].Set("라이트"	,WIDTH-71	,HEIGHT-122	,WIDTH-5,HEIGHT-104	,IDM_ALPHA_LIGHT);
	barMenu[14].Set("라이트EX"	,WIDTH-71	,HEIGHT-142	,WIDTH-5,HEIGHT-124	,IDM_ALPHA_EXLIGHT);
	barMenu[16].Set("반투명"	,WIDTH-71	,HEIGHT-162	,WIDTH-5,HEIGHT-144	,IDM_ALPHA_HALF);
	barMenu[15].Set("보통"		,WIDTH-71	,HEIGHT-182	,WIDTH-5,HEIGHT-164	,IDM_ALPHA_NORMAL);

	barMenu[17].Set(" 8 비트"	,WIDTH-71	,4         	,WIDTH-5,22        	,IDM_IMAGE_8BIT);
	barMenu[18].Set("16 비트"	,WIDTH-71	,24        	,WIDTH-5,42        	,IDM_IMAGE_16BIT);
	...
	barMenu[34].Set("2:1 좌표"		,WIDTH-71	,HEIGHT-124	,WIDTH-5,HEIGHT-106	,IDM_MOVE_OVAL);
	barMenu[35].Set("1:1 좌표"		,WIDTH-71	,HEIGHT-144	,WIDTH-5,HEIGHT-126	,IDM_MOVE_CIRCLE);

	barMenu[36].Set("에니종류"		,WIDTH-141	,HEIGHT-147	,WIDTH-75	,HEIGHT-129	,IDM_ANM_KIND);
	barMenu[37].Set("캐릭터"		,WIDTH-141	,HEIGHT-122	,WIDTH-75	,HEIGHT-104	,IDM_DATA_CHARACTER);
	barMenu[38].Set("무기"			,WIDTH-141	,HEIGHT-102	,WIDTH-75	,HEIGHT-84	,IDM_DATA_WEAPON);
	barMenu[39].Set("무기효과"		,WIDTH-141	,HEIGHT-82	,WIDTH-75	,HEIGHT-64	,IDM_DATA_WEAPONEFFECT);
	barMenu[40].Set("방패"			,WIDTH-141	,HEIGHT-62	,WIDTH-75	,HEIGHT-44	,IDM_DATA_SHIELD);
	barMenu[41].Set("방패효과"		,WIDTH-141	,HEIGHT-42	,WIDTH-75	,HEIGHT-24	,IDM_DATA_SHIELDEFFECT);
	barMenu[42].Set("추가효과"		,WIDTH-141	,HEIGHT-22	,WIDTH-75	,HEIGHT-4	,IDM_DATA_EFFECT);

	barMenu[43].Set("릴리즈 포인트"	,WIDTH-141	,HEIGHT-22	,WIDTH-75	,HEIGHT-4	,IDM_RELEASE_POINT);

	{
		...
		barMenu[eParts_title].Set("파츠 분류"		,iX	,iY+=20,iX+iDX	,iY+20+18);
		iY	+=	5;
		barMenu[eParts_head_back].Set("머리(뒤)"	,iX	,iY+=20,iX+iDX	,iY+20+18);
		barMenu[eParts_head_front].Set("머리(앞)"	,iX	,iY+=20,iX+iDX	,iY+20+18);
		barMenu[eParts_body_back].Set("몸통(뒤)"	,iX	,iY+=20,iX+iDX	,iY+20+18);
		barMenu[eParts_body_front].Set("몸통(앞)"	,iX	,iY+=20,iX+iDX	,iY+20+18);
		barMenu[eParts_arm_left].Set("왼팔"			,iX	,iY+=20,iX+iDX	,iY+20+18);
		barMenu[eParts_arm_right].Set("오른팔"		,iX	,iY+=20,iX+iDX	,iY+20+18);
		barMenu[eParts_leg_left].Set("왼다리"		,iX	,iY+=20,iX+iDX	,iY+20+18);
		barMenu[eParts_leg_right].Set("오른다리"	,iX	,iY+=20,iX+iDX	,iY+20+18);
		barMenu[eParts_weapon].Set("무기"			,iX	,iY+=20,iX+iDX	,iY+20+18);
		barMenu[eParts_shield].Set("방패"		,iX	,iY+=20,iX+iDX	,iY+20+18);
	}
	...
	barMenu[106].Set("기본 공격"	,WIDTH-141	,HEIGHT-42	,WIDTH-75	,HEIGHT-24	,IDM_ATTACK_ACTION);
	barMenu[107].Set("기본 마법"	,WIDTH-141	,HEIGHT-62	,WIDTH-75	,HEIGHT-44	,IDM_MAGIC_ACTION);
	barMenu[108].Set("이따금 휴식"	,WIDTH-211	,HEIGHT-62	,WIDTH-138	,HEIGHT-44	,IDM_OCCASIONALLY_REST);

	barMenu[IDM_COMPARE_DATA].Set("장비 이미지 출력",5			,HEIGHT-102	,71		,HEIGHT-84	,IDM_COMPARE_DATA);

	int i;

	for (i=IDM_ANM_FIRST;i<=IDM_ANM_LAST;i++)
	{	
		barMenu[i].Set("에니메이션",WIDTH-71	,HEIGHT-22	,WIDTH-5,HEIGHT-126	,i);
...
*/
	...
	{	
		...
		barMenu[IDM_COMPARE_DATA].Set("장비 이미지 출력",5		,HEIGHT-2-20*(index+1)-10	,110,HEIGHT-4 - index*20-10);
		barMenu[IDM_COMPARE_DATA].SetActive(TRUE);

		if (ANM[cANM::CurAnm].bSaveReleasePoint)
		{
			barMenu[IDM_RELEASE_POINT].SetActive(TRUE);
			barMenu[IDM_RELEASE_POINT].Set("릴리즈 포인트 설정"	,5+5+maxSize+34	,HEIGHT-2-64,5+5+maxSize+124+34,HEIGHT-48);
		}
		else
		...
		barMenu[IDM_ATTACK_ACTION	].Set("기본 공격"	,5+5+maxSize+34			,HEIGHT-2-20,5+5+maxSize+124+34			,HEIGHT- 4);
		barMenu[IDM_MAGIC_ACTION	].Set("기본 마법"	,5+5+maxSize+34			,HEIGHT-2-40,5+5+maxSize+124+34			,HEIGHT-24);
		barMenu[IDM_OCCASIONALLY_REST].Set("이따금 휴식",5+5+maxSize+124+34+5	,HEIGHT-2-20,5+5+maxSize+124+34+5+80	,HEIGHT-4);
...
{	int	anm	=	cANM::CurAnm;
	...
	{	
		if	(cANM::GetMaxFrame() > 0)
		{
			...
			cIMG::pCURRENT->Put(x-cANM::ReformPosX()*Zoom/100,y-cANM::ReformPosY()*Zoom/100,Zoom,cANM::isFlip(),alpha);	//	나
		}
		return;
	}
	...
	{	
		...
		y		=	y-cANM::ReformPosY()*Zoom/100;	//	나
...
cWORKWND::Draw()
{
	...
	if (cIMG::pCURRENT)
	{
		...
		cTEXT::Put(tx,ty+=20,_LTYELLOW,_ms ("전체교정 :     ,"));
		...
		if (_MAIN.workMode==	dWORK_SET_CRASH	||	_cfg.bDRAWCRASHBOX)
		{
			if (_MAIN.workMode==	dWORK_SET_CRASH)
			{
				...
				cTEXT::Put(202,12,0x7fff,_ms("충돌 박스 사이즈 : %d Pixel",g_wCrashSize));
				cTEXT::Put(202,12,_LTGREEN,_ms("                   %d",g_wCrashSize));
				cTEXT::Put(202,29,0x7fff,"'+','-'키로 붉은써클의 사이즈를 조절하세요.");
			}
			...
			switch(current)
			{
				...
				case	IDM_IMAGE_8BIT		:
					if (cANM::BPP==16 && cIMG::Count > 0)
						switch(cMSG::YESNO("경고!! 16비트에서 8비트로 변경하면 기존의 데이터는 초기화 됩니다.","계속 하시겠습니까?"))
						{	case	IDNO	:	break;
							case	IDYES	:	
								cANM::BPP=8;
								cIMG::Reset();
				...
				case	IDM_IMAGE_16BIT		:
					if (cANM::BPP==8 && cIMG::Count > 0)
						switch(cMSG::YESNO("경고!! 8비트에서 16비트로 변경하면 기존의 데이터는 초기화 됩니다.","계속 하시겠습니까?"))
						{	case	IDNO	:	break;
							case	IDYES	:	
								cANM::BPP=16;
								cIMG::Reset();
